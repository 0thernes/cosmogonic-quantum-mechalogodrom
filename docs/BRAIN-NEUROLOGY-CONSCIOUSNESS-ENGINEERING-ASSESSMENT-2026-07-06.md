<!-- reviewed: 2026-07-06 | v0.21.7 MASTER brain/neuro/consciousness assessment | merges 4 AI reports | canonical: docs/VERIFICATION-ANALYTICAL-DATA.md -->
...
**Version under review:** `v0.21.7` (package + all truth surfaces) · **Assessed:** 2026-07-06 · **Windows Bun 1.3.14 checkout**
...
**Method:** four independent AI reports (Gemini Antigravity 3.5 Flash, Composer 2.5, Devin SWE 1.6, plus repo-internal audits) merged and reconciled against the canonical single-source-of-truth ledger [`docs/VERIFICATION-ANALYTICAL-DATA.md`](docs/VERIFICATION-ANALYTICAL-DATA.md) §1, `scripts/canonical-receipts.ts`, and a fresh `git ls-files` measurement. Where reports disagreed, the code / test / lab-JSON receipt wins; stale `v0.21.6` / `4.22` numbers are treated as superseded and reconciled in §1.
...
| **Tsotchke corpus**                    | **20** projects (~16 wired/harvested; 3 fenced) · **22** registry slugs                     | `tsotchke-registry.ts` + integration map                |
...
`git ls-files` snapshot (2026-07-06), excluding `node_modules`, `dist`, `coverage`, native build output, and `.git`. **Total tracked: 730 files.**

| Extension               |   Count |                Lines | Role                                                    |
| ----------------------- | ------: | -------------------: | ------------------------------------------------------- |
| `.ts` (TypeScript)      | **584** |              135,780 | app source + tests + scripts + bench (no `.tsx`/`.jsx`) |
| `.md` (Markdown)        |  **64** |               15,292 | living docs (non-legacy tracked)                        |
| `.html`                 |   **8** |               12,907 | diagrams / labs / spec surfaces                         |
| `.json`                 |  **11** |                9,706 | config + lab receipts                                   |
| `.svg`                  |  **12** |                  173 | diagram assets                                          |
| `.xml`                  |   **4** |                  708 | master steering personas + roadmap                      |
...
| `.css`                  |   **2** |                2,462 | app + docs styling                                      |
| `.ps1` (PowerShell)     |   **2** |                  515 | Windows-side scripts                                    |
| `.png` (binary)         |   **4** |  (18,716 blob-lines) | visual receipts / screenshots                           |
| `.txt` + config + other |      62 |              ~30,557 | lockfiles, `.gitattributes`, `.githooks`, misc          |
| **TOTAL**               | **730** | **~226,382 tracked** | —                                                       |
...
| ------------------ | ----: | ------------------: | ------------------------------------------------------------------------------------------ |
| `src/`             |   288 |             135,780 | app source (`.ts` only): `main.ts`, `server.ts`, sim, math, core, workers                  |
| `tests/`           |   255 |             ~57,500 | the 2,360-test / 2,866,429-`expect()` suite                                                |
| `docs/`            |    65 |              15,292 | living narrative + ADRs + dated reports (64 non-legacy MDs synced)                         |
| `scripts/`         |    27 |              ~9,200 | build / verify / sync / bench (gate-critical: `canonical-receipts.ts`, `sync-surfaces.ts`) |
| `bench/`           |    16 |              ~1,800 | deterministic benchmarks + P1 quantum-vs-classical                                         |
| `native/`          |    12 |              ~1,600 | C++ / GLFW / Jolt engine (optional tier)                                                   |
| `.github/`         |    11 |                 685 | CI workflows, templates, CODEOWNERS                                                        |
| `lab/`             |     5 |             ~12,900 | consciousness + sentience falsifier labs (HTML + JSON)                                     |
| `masters/`         |     3 |                 437 | XML steering personas (EXECUTOR / ARCHITECT / PHYSICIST)                                   |
| `legacy/`          |     5 |               1,433 | preserved verbatim (opus-4.7 origin); **immutable — never edited**                         |
| `output/`          |     3 | (18,715 blob-lines) | visual receipts (PNG-embedded screenshots)                                                 |
| Root MD            |     9 |              ~5,000 | `README`, `CHANGELOG`, `CLAUDE.md`, `THIRD-PARTY-NOTICES` (name-pinned)                    |
...
**(a) Line count: 186,498 vs 200,428 vs 226,382.** These are not contradictory measurements — they are three different _scopes_ of the same tree.
...
- **226,382** (this master edition, fresh `git ls-files` 2026-07-06) — **total tracked**, _including_ the 4 `.png` visual receipts (~18,716 + ~18,715 blob-lines under `output/`) and all config/lockfiles.

Honest reconciliation: the canonical **total tracked = 226,382 lines**; the "pure code + docs" number (code + tests + docs, excluding PNG receipts and vendored lockfiles) is **~200k–206k**, which is what the Gemini/Devin reports report at their respective scopes. Devin's `v0.21.6` _header_ is a typo — its body uses `v0.21.7` facts; no real version drift exists.
...
### 2.1 What is real now (verified against v0.21.7)

- **A gate-enforced, deterministic artifact — with the full gate currently RED.** The 8-stage `bun run check` (prettier → tsc strict → oxlint → verify:receipts → sync:check → verify:facts → build) is designed to hold every surface honest, and the four measurement sub-gates pass right now: **typecheck, lint, sync:check, and verify:receipts are green** — canonical receipts floor **2,360 tests / 0 fail** across **255 test files** with **≥2,866,429 `expect()` calls** (the measured pass-count runs _higher_ as suites grow — latest local observed ~2,376; 2,360 is the pinned floor, never an exact), portable coverage floor **84.64 % line / 82.21 % func** (Windows checkout **92.02 % / 89.65 %**), version pinned at **0.21.7** in `package.json` and propagated to every surface by `scripts/sync-surfaces.ts`. **But the full `check` is red**: `format:check` fails on the unformatted report forks and — the P0 blocker — `bun run build` fails at `src/styles/app.css:0` (a verified toolchain regression, not content; see §II.9.1). Any claim that `bun run check` is fully green today is stale. Codebase scale (git-tracked, 2026-07-07): **730 tracked files · 584 `.ts` (583 excl. 1 `.d.ts`) · ~135,780 TS lines · 8 C/C++ · 255 `*.test.ts`**.
...
Both sections are complete and ready to merge into the master document. Key grounding notes for the compiler: §2's honesty spine (`meanNullGap ≈ 0`, `ablationRate 0.40625`, Butlin 8+6, coupling floor 0.188) and §3's two mermaid blocks (full-system data-flow + acyclic layering) all trace to the grounded fact sheet — `super-mind.ts:49–57`, `consciousness-kernel.ts:28–32`, `coupling-audit.test.ts:170`, `lab/sentience-data.json`, `lab/consciousness-data.json`, and canonical v0.21.7 receipts (2,360 tests / 84.64 % / 82.21 %).
...
Beyond the eight abstract brain architectures, four **world-entity classes** carry their own deterministic behavioral cognition — game-theoretic, predatory, authority-projecting, and trophic. None is a `SuperMind`-class deliberator, but each **reads world state and writes it back** (the _coupling > count_ law restated at the ecology layer), and each is real, seeded, falsifiable code — this is why §II.7 can say cognition spans the living world, not only the apex tier.

| Entity                  | File                    | Behavioral cognition                                                                                                                                                                                                                           | Key constants (`file:line`)                                                                         | Reads → Writes                                              |
| ----------------------- | ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| **Titans ×20**          | `sim/titans.ts`         | Game-theoretic economy — 10 territorial + 10 central breeders, **C(20,2) = 190** pairwise war-matrix payoffs; an energy ledger zeroed at the matrix mean; bankruptcy mutates strategy via **replicator dynamics** over a 5-strategy population | `TITAN_COUNT=20` (:52), `PAIR_COUNT=190` (:54), `RESOURCE_CAP=1000` (:77), `replicatorStep` (:42)   | rival payoffs + resource field → strategy, light, territory |
...
| **Puppet-Masters ×100** | `sim/puppet-masters.ts` | World-authority projection — 3 named cores (AETHON / SELENE / KRONOS) + 97 lesser hands on a deterministic **golden-angle** layout (no rng); each reshapes the world on a long stagger                                                         | `PUPPET_COUNT_DESKTOP=100` (:79), `PUPPET_COUNT_MOBILE=14` (:80), golden-angle (:83)                | arena state → weather / chaos / mutation reshapes           |
...
- **Determinism everywhere.** Every equation above is a pure function of seeded inputs. The kernel, IIT proxy, predictive coding, Izhikevich, and Hopfield use **no RNG at all**; active inference / Thaler / GWT draw only from the seeded `Rng`. Re-running any seed reproduces the trace bit-for-bit.
...
This is the **honest** number and it is deliberately reported instead of the inflated `tsotchkeWiringCoverage()` value (which averages _only over already-wired entries_ and therefore reports a misleading `1.0`). We publish **~16 wired/harvested** as the round-number headline and **85.7%** as the precise substrate fraction.
...
| **World / sim**             | **2** | asteroids, simple_mnist                                                                                                                | Real code drives **entity-scale physics & baselines** — Newtonian motility seeds and the perceptron nutrient field. Not in the apex cognition frame, but load-bearing in the world step.                      | 1.0 each      |
| **Ported / telemetry**      | **3** | PINN, PIMC, quantum-quake                                                                                                              | Real primitives ported and **read as observables** — Gray–Scott metabolism residual, path-integral "soul" weights, QGE aliveness. Feed vitality / exploration / aliveness telemetry, not decision commitment. | 1.0 each      |
| **License-gated / studied** | **2** | ulg, logo-lab                                                                                                                          | Real code, wired into world-law-graph and procedural morphogenesis, but **held behind a copyright-assignment gate** (ubernaut). Kept minimal pending relicensing.                                             | 1.0 each      |
| **API / toolchain**         | **2** | homebrew-eshkol, Quantum-RNG-API                                                                                                       | **Build/harvest tooling and REST wrappers** — the `.esk` compiler catalog (source of 1,365 heritable DNA programs) and a QRNG REST façade (redundant; core is ported natively).                               | 1.0 (harvest) |
...
> Class-sum note: 9 + 2 + 3 + 2 + 2 = **18 wired/harvested slugs** across scientific classes (`classical_rng` and `classical-contrast` are counted inside the 9 Deep; denominator 21 after removing org-`.github`). The 3 fenced slugs are the honest `0`-wiring remainder. This is why the headline is **~16 wired** and the precise fraction is **18/21 = 85.7%** — never "all 20 deeply wired."
...
| Wiring breadth   | "**~16 of 20** projects wired/harvested; precise substrate fraction **18/21 = 85.7%**; 3 fenced by design; 1 GPL-quarantined."          | "**All 20 deeply wired at full depth.**"                                                |
...
flowchart TD
    ...
    E --> G
    F --> G
    G --> H["#1/113 breadth (z=+2.83 grounded)<br/>· 8.3/10 scrutiny · 486 PASS<br/>· coupling 0.27 (weakest, 6.0/10)"]
```
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| Test suite                 | **2,360 pass / 0 fail** across **255 test files**                                                                                                                                                  | `scripts/canonical-receipts.ts` |
| Assertion density          | **2,866,429** `expect()` calls                                                                                                                                                                     | measured 2026-07-06             |
| Coverage (portable floor)  | **84.64 % line / 82.21 % func**                                                                                                                                                                    | canonical ledger (Ubuntu)       |
| Coverage (Windows receipt) | **92.02 % line / 89.65 % func**                                                                                                                                                                    | this checkout                   |
| Type discipline            | `tsc --noEmit` strict, **0 errors**                                                                                                                                                                | `bun run check`                 |
| Lint                       | oxlint over `src server.ts tests bench scripts`, **0 errors**                                                                                                                                      | gate                            |
| Gate stages                | prettier → tsc → oxlint → verify:receipts → sync:check → verify:facts → build — **sub-gates green; full `check` currently RED** (`format:check` + `build` fail at `app.css:0`, see §2.1 / §II.9.1) | `bun run check`                 |
| Codebase scale             | **730 tracked files** (584 `.ts`, ~135,780 LOC)                                                                                                                                                    | `git ls-files`                  |
...
**Cosmogonic Quantum Mechalogodrom v0.21.7 is a strong PhD-lab prototype and a genuine white-paper-grade artifact. It is NOT prize-level evidence — and it does not claim to be.**
...
graph TB
    ...
    I -->|"2,360 tests<br/>show behavior →<br/>generalize"| C
...
| **90° Inductive**  | bottom-up empirical          | Generalize from 2,360 passing tests / 2,866,429 `expect()` calls: survival, foraging, clustering, opponent-plan prediction, ablation traces                                     | The system **IS behaviorally intelligent** at A-Life scale — 50,000-entity coordinated pursuit/evasion, ToM menace tracking, SR look-ahead — grounded in tests, **not** speculation. Rank #1/113, breadth 4.44/5.                                           | `tests/super-mind.test.ts`, `tests/tom-pantheon.test.ts`, `tests/butlin-indicators.test.ts` |
...
```mermaid
flowchart LR
    A["<b>CLAIM</b><br/>2,360 tests · 84.64% line<br/>100 faculties · 8/14 Butlin"]
...
| 3   | **Test / receipt integrity**              | **8.7** ★★★★½ | 2,360 tests / 0 fail · 2,866,429 `expect()` · 255 files; 84.64% line / 82.21% func portable floor gate-enforced. Deep and honest (floor is a floor, not a headline).  |
...
The canonical test corpus (**2,360 tests / 0 fail · 255 files · 2,866,429 expect() calls** at v0.21.7) is the _floor_, not the ceiling: it proves the mechanisms run deterministically. This benchmark suite proves whether they are _load-bearing indicators_ — a strictly stronger, and currently only partially satisfied, bar.
...
| Dimension                | Current (v0.21.7)                                                      | Target                                                                   | Gating step                |
| ------------------------ | ---------------------------------------------------------------------- | ------------------------------------------------------------------------ | -------------------------- |
...
Both sections are complete and grounded. Key file:line refs verified live against source this session: organ classes and Meta-Paradox clauses at `apex-brain.ts:23–40, 92–1266, 1458–1466`; STDP constants at `mechalogodrom-brain.ts:35–40`; consciousness-kernel `iters=6` / `squash01` / `O(iters·10²)` at `consciousness-kernel.ts:360, 447, 451, 471`. All numbers reconciled to canonical v0.21.7. The full markdown for §9 and §16 is above, ready to paste directly into the master document at the assigned headings.

---

## §17 — Issues / Bugs / Gaps / Dilemmas — The Honest Liability Register

This section is the adversarial ledger. Every entry is a live risk against the credibility, correctness, or publishability of the Cosmogonic Quantum Mechalogodrom v0.21.7 assessment. Nothing here is cosmetic; each row is tied to a code / test / lab-JSON / doc surface and carries a concrete handling protocol. All consciousness/sentience claims remain **indicatorOnly** — these issues are exactly the fault lines where that discipline is most easily broken.
...
flowchart TD
  ...
  B -- no --> D{Numeric fact?}
  C --> D
  D -- yes --> E[Trace to code/test/lab-JSON; reconcile to canonical v0.21.7]
  D -- no --> F[Ship]
...
| 8   | **Appendix**     | Canonical receipts (v0.21.7 · 2,360 tests / 0 fail · 255 files · 2,866,429 expect() · 84.64%/82.21% portable, 92.02%/89.65% Windows); file inventory (730 tracked); full coupling matrix W; complete lexicon / honesty attestation; reproduction commands (`bun run check`, `bun run bench`).                                                                                                                                           | canonical-receipts.ts; VERIFICATION-ANALYTICAL-DATA.md                                                            |
...
- [ ] **Reconcile every `4.22`** → `4.44` self / `3.68` code-grounded; every `v0.21.6`/`v0.18.0` current-tense pointer → `v0.21.7`; every stale test floor (`2295`, `2372`, `1984`, `1477`) → `2,360`.
...
| #   | Document : line                                                                       | Stale item                                            | Stale value   | Correct value (canonical v0.21.7)                                        | Fix                                                                                                                                                            | Severity              |
| --- | ------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
...
| 2   | `docs/reports/README.md:8`                                                            | Canonical-receipts version pointer in prose           | `v0.21.6`     | `v0.21.7 · 2,360-test floor · 84.64% / 82.21%`                           | Rewrite the "Current gate receipts" line to point at `VERIFICATION-ANALYTICAL-DATA.md §1` + `canonical-receipts.ts` at v0.21.7                                 | **P1 surface-sync**   |
| 3   | `docs/KANBAN-2026-06-26.md:17`                                                        | Tsotchke corpus count                                 | `19` projects | `20` projects (~16 wired, 3 fenced); 22 registry slugs                   | Rewrite: "registry maps all **20** projects (~16 wired, 3 fenced)"; matches `TSOTCHKE-INTEGRATION-MAP` + `tsotchke-registry.ts`                                | **P2 count-accuracy** |
| 4   | `docs/BRAIN-NEUROLOGY-…-2026-07-06.md:649`                                            | Version marker context ("V123 optimization complete") | `v0.21.6`     | `v0.21.7` (v0.21.6 is a past tag)                                        | Confirm historical framing is explicit ("v0.21.6 is superseded"); no change if clearly past-tense                                                              | **P1 doc-accuracy**   |
| 5   | `docs/reports/2026-06-20-ROADMAP-TO-NHSI-AND-SENTIENCE.xml:5`                         | XML roadmap version baseline                          | `v0.18.0`     | (current package `v0.21.7`)                                              | **EXEMPT** — point-in-time snapshot dated 2026-06-20; v0.18.0 is correct _for that date_ per `docs/reports/README.md` worldline-snapshot rule. Do NOT rewrite. | P2 (exempt)           |
...
| 7   | _(sweep)_ any surface still on `v0.21.6` / `4.22` / `2295` / `14-of-14` current-tense | version / breadth / test-floor / Butlin               | as found      | `v0.21.7` / `4.44` (+3.68) / `2,360` / `8-of-14 + 6-partial`             | Fix at SOURCE (`package.json`, `canonical-receipts.ts`), then `bun run sync` propagates                                                                        | P1                    |
...
**Section files (this assignment):** the three sections above (§17, §18, §19) are the deliverable text, returned inline as required — no separate report file was written. All numbers reconcile to canonical **v0.21.7** (2,360 tests / 0 fail · 84.64%/82.21% portable · 92.02%/89.65% Windows · 730 tracked files · Butlin 8/14 MET + 6/14 PARTIAL · A-Life 4.44 self / 3.68 code-grounded · Tsotchke 18/21 wired = 85.7%). Every claim is **indicatorOnly** — computational indicators, never phenomenal consciousness, sentience, or a solved hard problem.

---

## §20 — Four-Report Reconciliation

The MASTER EDITION merges four upstream AI-authored assessments into a single strictly-larger super-report. Two of the four (**A** — Gemini Antigravity 3.5 Flash `.txt`, and **E** — Codex GPT `.md`) are **byte-identical** and together constitute the _current canonical brain doc_ checked into the repo (`docs/BRAIN-NEUROLOGY-CONSCIOUSNESS-ENGINEERING-ASSESSMENT-2026-07-06.md`, compiled at `125406d1`). This edition supersedes and expands **all four**. Where a source cites a stale number, the canonical v0.21.7 receipt (verified against `package.json`, `scripts/canonical-receipts.ts`, and the lab JSONs on 2026-07-06) **wins** — no exception.
...
| #   | Claim as stated in a source                                       | Which report(s) said it                                                   | Current v0.21.7 reconciliation (canonical, binding)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --- | ----------------------------------------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Version is **v0.21.6**                                            | C (header); A/E (a "V123 optimization complete" prose marker at line 649) | **Superseded → v0.21.7.** `package.json` = `0.21.7`; `scripts/canonical-receipts.ts` floors against it. v0.21.6 is a _past release tag_, historically correct at its own commit; every report **body** already uses 0.21.7 facts, so the drift is a header/prose typo, not a logic error. Living surfaces say v0.21.7.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 5   | Total line count **186,498**                                      | C                                                                         | **Superseded → ~200,428 / 226,382 measured.** Devin _excluded_ `output/` PNG receipts. Gemini (D) counted 200,428 _including_ config/binary blobs. Canonical measured total (2026-07-06) = **226,382 tracked lines**; "pure code" (code + tests + docs, excluding PNG/lockfile noise) ≈ **200k–206k**. All three are internally consistent once scope is fixed; the variance is a scope difference, not an error.                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 10  | Tsotchke: "20 all deeply wired" / "19 repos"                      | KANBAN prose (19); some report summaries ("all wired")                    | **Superseded → 20 corpus projects; ~16 wired/harvested; 22 registry slugs.** Honest accounting via `tsotchkeWiredSubstrateFraction()` = **18 wired / 21 scientific = 85.7%** (NOT 1.0). Depth classes: 9 deep + 7 wired + 2 harvest + 3 fenced + 1 org-meta. KANBAN's "19" is a stale outlier (fix: 20). **Tsotchke is REAL MIT-licensed quantum math — never fake/decorative.**                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| 18  | Test suite size                                                   | A/E, B, C, D                                                              | **Confirmed, no drift → 2,360 pass / 0 fail · 255 files · 2,866,429 expect().** All four reports agree once versioned to 0.21.7 (B explicitly confirms). Coverage: **84.64% line / 82.21% func** portable floor; **92.02% / 89.65%** Windows receipt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
graph TD
    ...
    A --> MASTER
    CANON --> MASTER["MASTER EDITION v0.21.7<br/>(strictly larger; supersedes + expands all four)"]
    MASTER -->|reconciles to| RECEIPTS["canonical-receipts.ts · package.json · lab JSONs<br/>(2026-07-06 verified)"]
```

**Net reconciliation verdict.** Zero _substantive_ contradictions survive once every source is versioned to v0.21.7 and scoped consistently. All divergences reduce to one of four benign classes: **(a) version-tag lag** (v0.21.6 header → 0.21.7 body), **(b) scope difference** (186k vs 200k vs 226k lines; 64 vs 89 md — different inclusion sets), **(c) method difference** (4.44 self-scored vs 3.68 code-grounded), or **(d) framing shorthand** ("5 brains," "14/14," "< 2%") that the canon expands into the honest fuller statement. The MASTER EDITION adopts the most conservative honest reading in every case: **8/14 Butlin, 4.44 self / z=+2.83 code-grounded, 85.7% Tsotchke wired, 1.99 ms apex think(), indicatorOnly throughout.**
...
Cosmogonic Quantum Mechalogodrom v0.21.7 occupies a specific and defensible position: it is **not sentient, and it is not a toy.** It is not sentient because every metric it produces is a **computational indicator** — a load-bearing, seeded, falsifiable proxy for a _mechanism_ named by a consciousness theory, never a measurement of phenomenal experience. The hard problem is untouched; the honesty law is stamped into source (`super-mind.ts:49-57` "NOT SENTIENT"; `consciousness-kernel.ts:28-32` `claim: 'indicatorOnly'`), and the labs _prove their own limits_ — the headline index null-gap ≈ 0, the ablation gate fails at 0.406, and both facts are published rather than buried. But it is equally **not a toy**: 2,360 green tests across 255 files with 2.87M assertions, an 84.64%/82.21% portable coverage floor, a gate-enforced single-source-of-truth sync pipeline, real MIT-licensed quantum math (Eshkol AD, Moonlab Clifford/VQE, QGT Fubini-Study, libirrep SO(3), spin-glass Hopfield) running as exact deterministic simulation, and a **#1-of-113 A-Life breadth ranking** (z = +4.02 population, +2.83 code-grounded) with **solo-leader** substrate pluralism (z = +8.05) and consciousness-theory wiring (z = +10.04). It is a rigorously-engineered, fully-auditable, deterministic multi-theory artificial-life research prototype — an instrument for _testing_ consciousness hypotheses, not a claim to have _answered_ them.
...
5. **Reproducible lab data.** Publish the seeded lab JSONs (`consciousness-data.json` seed 539363075, `sentience-data.json` rootSeed 0x20260704) with a one-command regenerator and the surrogate/shuffle controls, so any external reviewer reproduces the exact 5-event / 90-event singularity traces bit-for-bit.
...
| SUPER-REPORT ULTIMATE-MEGA (manifesto)              |    232 | **C+ / 6.0**                 | **"~400 TS files" (→584), "~50,000 lines" (→135,780 .ts / 186,498 authored), ApexBrain conflation, "7 C/C++" (→8)** | the manifesto-of-the-gods voice + the brain×scrutiny checkmark matrix concept                                                                                                              | supersede (keep voice only) |
...
| -------------------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| **Titans**                             | 10 (some) / 20 (others)                       | **20** — `TITAN_COUNT = 20` (10 territorial + 10 central breeders); war matrix 20×20; C(20,2)=190 pairs (an older header comment saying "10 / 45 pairs" is stale doc-vs-code drift)                                                                          | `titans.ts:51-54,608`                                                             |
...
| **Total LOC / files**                  | "~50,000 lines / ~400 TS files" (ULTIMATE)    | **135,780 .ts LOC · 584 .ts files · 730 tracked · 186,498 authored (pure code+docs) / ~200,428 excl binary PNG receipts / 226,382 total tracked incl the 4 PNG receipts**                                                                                    | `git ls-files`                                                                    |
...
| Valence-steering               | felt affect _causally_ steers behaviour (not epiphenomenal)                                                                                    | `valence-steering.ts:98`                         |
...
| 2   | VERIFICATION-ANALYTICAL-DATA                        | SSOT ledger         | CURRENT (0.21.7 fix landed)                                                  | **KEEP** — canonical facts; the "latest receipt" line is a **floor** (measured count grows as tests are added) |
| 3   | NHSI-PROGRESS-DASHBOARD                             | dashboard           | CURRENT (stylized)                                                           | **KEEP** — but fix stale ".esk 1,436+" → 1,365                                                                 |
...
A direct probe on 2026-07-07 (`git status` + `prettier --check docs/*.md`) shows the working tree is **dirty and NOT full-gate green**: **`format:check` fails on 8 docs** (the unformatted report forks + this file pre-format). Any report that says "`bun run check` green" right now is **stale** — it was green at a clean commit, not in this dirty tree. `verify:receipts` still passes (the **2,360 floor** holds; the measured pass-count runs _higher_ as suites grow — a floor, never an exact). **Correct wording:** _sub-gates (typecheck/lint/sync/receipts) pass; the full `check` is red until the docs are formatted and the forks resolved._
...
- **A real source drift to fix:** `specs.html` still says "10 Titans" in one public sentence while `titans.ts` sets `TITAN_COUNT = 20` — a browser-surface stale count worth correcting at source.
...
1. **Format the docs** (`prettier --write docs/*.md`) → clears `format:check`.
...
3. **Fix the two stale source counts** (NHSI `.esk 1,436+`→1,365; `specs.html` "10 Titans"→20).
4. **Keep** this master MD + the Brain Atlas HTML + VERIFICATION + NHSI + CONTROLS.
5. **`bun run check`** (now green) → **ship to `main`.**

_II.8 compiled 2026-07-07 · Claude Fable 5 · total-16-file audit + live-gate truth._

---

## II.9 — The 22-file update, the P0 build break, and the gaps worth using

**Added 2026-07-07 (third pass).** The set grew to **22** (GPT 5.5 Extra High added `CONSOLIDATED-16-MASTER-ASSESSMENT` md+html, `CONSOLIDATED-16-FILE-AUDIT`, `MASTER-ASSESSMENT` md+html, `FILE-AUDIT-16-FILES`). A live re-probe surfaced a finding that outranks every doc issue in the fleet.

### II.9.1 — P0: THE APP DOES NOT BUILD (verified)

`bun run build` **fails**: generators run, then `Bun.build` dies at `src/styles/app.css:0` with **"Arguments contain a value that is out of range of code points."** Verified 2026-07-07 (reproduced twice). **Root cause = a TOOLCHAIN regression, NOT app.css content.** I tested the obvious hypothesis — stripped **all 117 non-ASCII characters** (box-drawing / em-dash / ellipsis) out of app.css and rebuilt: **it failed identically at `app.css:0`**, so the CSS content is exonerated (change reverted). The fault is in the **`bun-plugin-tailwind ^0.1.2` + Bun 1.3.14 + Tailwind v4** bundling path throwing an out-of-range `String.fromCodePoint`-class error while processing the CSS entry. **This is the real gate blocker — bigger than `format:check`.** Any report claiming a green build is stale; the repo is red until this is fixed. **The fix is dependency-level, not a doc/CSS edit:** test a `bun-plugin-tailwind` version bump (or a Bun version change) in isolation, then rebuild — it needs a deliberate toolchain pass, not a content normalization (that path is now disproven).
...
1. **P0 — fix the build** (§II.9.1). Nothing ships until `bun run build` is green.
...
7. **Fix the two stale source counts** — `specs.html` "10 Titans"→20; NHSI `.esk 1,436+`→1,365.

### II.9.4 — Final consolidation directive (supersedes II.8.4)

1. **FIX THE BUILD first** (§II.9.1) — the P0.
2. `prettier --write docs/*.md` → clears `format:check`.
...
6. `bun run check` (now green) → **ship to `main`.**

_II.9 compiled 2026-07-07 · Claude Fable 5 · 22-file audit · P0 build break verified · convergent with GPT 5.5._
