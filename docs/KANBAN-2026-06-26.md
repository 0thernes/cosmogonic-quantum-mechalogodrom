...
**GOAL 5 + TSOTCHKE CORPUS (depth-ledger wiring - Digital Biologics Growth Phase)**: 5 Archons + **23 public Tsotchke repositories censused and 22 causal/runtime entries accounted for** (Eshkol AD/inference/GWT/.esk proxy language, Moonlab Clifford/MPS/QEC/VQE, spin/Hopfield, QGT, libirrep, physics and morphogenesis facades, RNG controls, and harvests) with **8 deep, 7 wired, 2 harvest, 4 fenced, 1 meta** and `17/21` non-meta integrated. `homebrew-moonlab` is census-only deployment metadata. `OBLITERATUS` is one of the four deliberate fences; `classical-contrast` is a separate internal control outside both denominators. These are computational substrates and indicators, not evidence of sentience or physical quantumness. Super Creature is beginning only — primordial soup grows new independent digital biologics ("Grow What Thou Wilt"). Not LLM/tokenizer. Gate + push.
...
**Goal:** Account for the complete Tsotchke public surface (23 public repositories across the user and organization, plus the local corpus and sites) while keeping the 22-entry causal/runtime ledger explicit by depth class: wired where licensed/ported, harvested where used as DNA/toolchain substrate, fenced where legal or mandate boundaries require it, and metadata where it is inert. `homebrew-moonlab` is public-census-only deployment metadata. Maximal but contract-strict, det, measured, audited. See [docs/TSOTCHKE-INTEGRATION-MAP-2026-06-26.md](./TSOTCHKE-INTEGRATION-MAP-2026-06-26.md).

**Status (v0.23.0):** FULL DEEP DIVE COMPLETE — Tsotchke local corpus plus GitHub studied. The public census contains 23 repositories; the causal/runtime registry maps 22 entries as `8 deep`, `7 wired`, `2 harvest`, `4 fenced`, `1 meta`, with `17/21` non-meta integrated. `homebrew-moonlab` is census-only deployment metadata, `OBLITERATUS` is fenced, and `classical-contrast` is an internal control outside both denominators. Petri/soup + brutal god tier live. Gate **3,299 tests · 84.64% / 82.21%** (receipts enforced); latest Windows-local pass may exceed the floor. "Grow What Thou Wilt." Performance targets consolidated into [BENCHMARKS-2026-06-26.md](./BENCHMARKS-2026-06-26.md).
...
**Tsotchke corpus depth ledger (reconciled 2026-07-10):** all 23 public repositories are censused; the causal/runtime ledger intentionally contains 22 entries. Depth-classed catalysis and the shared organism-intelligence field consume bounded integrated channels. Direct ports, deterministic facades, and harvests remain distinct. Current ledger: `8 deep`, `7 wired`, `2 harvest`, `4 fenced`, `1 meta`; `17/21` non-meta integrated. `homebrew-moonlab` is census-only deployment metadata. The four inert fences include `OBLITERATUS`; `classical-contrast` remains an internal control outside both denominators.
...
- Ralph loop: deepen Tsotchke wiring per [TSOTCHKE-INTEGRATION-MAP-2026-06-26.md](./TSOTCHKE-INTEGRATION-MAP-2026-06-26.md); NHSI targets tracked in [NHSI-PROGRESS-DASHBOARD-2026-06-26.md](./NHSI-PROGRESS-DASHBOARD-2026-06-26.md). Gate GREEN (**3,299 tests**, receipts enforced).
- **PERFORMANCE OPTIMIZATION:** The comprehensive Gemini 3.1 Pro High comparison and the evidence-backed current-state audit are consolidated in [BENCHMARKS-2026-06-26.md](./BENCHMARKS-2026-06-26.md). The live POWER loop remains primarily CPU/FP32 with no general WebGPU-compute or WebAssembly path, and intentionally keeps the wilderness Web Worker lane dormant: worker-enabled runs produced multi-second delivered-frame stalls hidden by the nominal FPS label. The ineffective instanced-motion lane was also removed after review because both interpolation endpoints carried the current transform. Quantization, priority cascades, workers, WebGPU, and WASM remain proposals until production-shaped delivered-frame benchmarks prove their benefit while preserving cognition and visual fidelity.
