...
- **Truth-sync (2026-06-17, audit pass — independent cold-shell verification).** Re-ran the full gate from a cold shell (Bun 1.3.14, commit `a9bb4dc`) and corrected every public receipt to the measured ground truth: **1,050 tests / 0 fail / 99 files / 1,738,688 assertions** and **94.61 % line / 91.19 % function** coverage (surfaces variously read 1,159/1,165/1,166 and 97.38/93.35). Apex params re-summed from the running constructor (10,081 + 1,444 + 25,700 = **37,225**). The "five orders of magnitude" line corrected: that ratio is the whole-world 3.5 M mass (~1/50,000th of GPT-3); the apex (37,225) is **~6.7 orders / ~4.7 million×** smaller. `think()` reported with machine context (≈ 289 µs author HW; ≈ 443 µs 2-core cloud Xeon; CI < 5 ms). Citations: Butlin 2023 = exactly 14 indicators; Kanerva 2009 = _Cognitive Computation_; Ferrante 2025 "double-bind" flagged as shorthand for the asymmetric "challenges key tenets of both"; Lidayan 2025 corrected to the complementary-roles finding (all 13 citations verified to exist). 2026-06-16 reports get a SUPERSEDED banner, bodies preserved. No source changed; gate green.
...
- **Launcher reverted to its prior layout + the dock float killed at the source (V74)** — user feedback on
  ...
  three-zone "dead-centre" split of `center-hud.ts` back to the V72 launcher — the **‹ › cycle arrows
