...
**Gate:** 3,324 tests · 84.64% line / 82.21% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).
...
| Passing tests                           | **3,324** (exact tracked suite; 0 failing)                            |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**3,324 tests, 0 fail**) →
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (3,324 tests, 84.64% line / 82.21% func coverage),
