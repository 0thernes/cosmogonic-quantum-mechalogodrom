...
> z `+2.954`, peer z `+3.090`). Batches 21–25 gate-lock this receipt: differential-survival GA supports
> Open-endedness `2.4`; exact Eshkol-AD policy learning plus live flora-gradient chemotaxis support
> Cognition/Learning `4.1`; and the audit sync allowlist gap is closed. A fresh disjoint fixed-family
...
| **Test floor (portable)**              | **2,507** pass / **0** fail                                                                                          | `scripts/canonical-receipts.ts`                         |
| **Latest local receipt (Windows)**     | **2,507** completed cases / 0 fail · **281** test files · **2,838,975** `expect()` calls                             | run 2026-07-11                                          |
| **Coverage floor — portable (Ubuntu)** | **84.64%** line / **82.21%** func                                                                                    | `scripts/canonical-receipts.ts` (gate floor, ±6 pp tol) |
| **Coverage — local (Windows)**         | **92.85%** line / **90.55%** func                                                                                    | this checkout                                           |
...
| 11  | **Reservoir Computing** (Maass 2002; Jaeger 2001)                                         | Reservoir novelty read in the coupling audit's 100-faculty subset; echo-state dynamics feed novelty                                                                                                | **wired** (novelty signal), tracked in coupling audit                                                                                      | A fixed random dynamical reservoir; novelty ≠ creativity ≠ experience                                                                                                        |
...
| 2   | `docs/reports/README.md:8`                                                            | Canonical-receipts version pointer in prose           | `v0.21.6`     | `v0.21.13 · 2,507-test floor · 84.64% / 82.21%`                                                                    | Point current gate text at `VERIFICATION-ANALYTICAL-DATA.md §1` + `canonical-receipts.ts`; let `bun run sync` propagate it                               | **P1 surface-sync**   |
