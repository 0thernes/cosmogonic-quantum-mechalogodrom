...
## Known bugs in the legacy file â€” fix during port

| #   | Legacy lines          | Bug                                                                                          | Required fix                                                                                                                                      |
| --- | --------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | 557                   | Music pitch multiplier `0.5+Math.floor(st/8)*0.5` grows unbounded â€” song drifts ultrasonic | Wrap octave: `0.5 + (Math.floor(st / 8) % 4) * 0.5`                                                                                               |
...
| 8   | 625-631               | Joystick reads `touches[0]` â€” wrong finger under multi-touch                               | Track by pointer/touch identifier                                                                                                                 |
| 9   | throughout            | `Math.random()` everywhere â€” non-reproducible                                              | Seeded `mulberry32` Rng injection (rule 7)                                                                                                        |
| 10  | 614                   | Touch roll/tilt buttons rotate INVERSE of the Z/X/R/F keys they mirror                       | Signs must match keyboard: rleftâ†’rz +1, rrightâ†’rz âˆ’1, tupâ†’rx +1, tdownâ†’rx âˆ’1                                                          |
...
Also: the control pad gains yaw buttons (`data-a="yleft"`/`"yright"` â†’ `camVel.ry` Â±1) so touch
users get the C/V yaw the keyboard has â€” this makes the legacy dead `camVel.ry` path live.
...
/** Mulberry32 â€” fast 32-bit deterministic PRNG. */
export function mulberry32(seed: number): Rng;
/** FNV-1a string hash â†’ uint32 seed. */
...
### Tsotchke modules (full corpus wired â€” paramount)
...
Port of legacy lines 182-194 (ACES tone mapping, fog FogExp2 0x020310/0.003, camera 68Â° 0.1..900 at (0,25,55)). Modern three: set `outputColorSpace` default; shadows per quality.
...
Signature change from legacy: `step(values: Float32Array, length: number, i: number)` â€” never read
indices â‰¥ length (fixes Known Bug 4's object copy).
...
export class EntityManager {
  ...
  /** Remove from scene + dispose per-entity material (geometry is shared â€” never disposed). */
...
export class Connectome {
  ...
  /** O(nÂ·k) link rebuild; caller decides cadence (legacy 798-821). */
  update(dt: number, t: number): void;
}
```
...
(pointerdown/up/leave/cancel, legacy 612-621), `[data-action]` toolbar buttons â†’ UiActions,
joystick `#jP`/`#jK` with identifier tracking (Known Bug 8), and one-shot audio unlock that calls
`actions` indirectly (world wires `AudioEngine.init` into its action impls â€” input just forwards).
...
### src/logging/audit.ts (leaf â€” browser)
...
### src/memory/store.ts (leaf â€” browser)
...
export class MemoryStore {
  ...
  defaults(): PersistedState; // version 1; seed = (0xc05a06 ^ ((performance.now() * 1000) | 0)) >>> 0 â€” varied per boot yet reproducible once persisted
}
```
...
`Bun.serve` routes: `/` â†’ index, `/docs` â†’ docs, `GET /api/health` â†’ JSON
`{ ok, uptime, version }`, `GET /api/audit` â†’ **HTML fragment** (`<ol>` of recent entries, newest
first) for HTMX polling, `POST /api/audit` â†’ append `{ action, detail?, ts }` into an in-memory
...
- `index.html`: app shell. `<canvas id="c">`, telemetry panel `#sP` (rows `#v0..#v8`, env rows
  ...
  weather/apoc â€” each with `aria-label` AND `title` (Known Bug 7), `#sec`, `#nm`, `#alg` (`#a-name`,
...
- Styling: Tailwind utilities; glassmorphic panels (backdrop-blur, translucent slate/cyan borders);
  typography per docs/WIREFRAMES.md type scale â€” `Inter Variable` for UI labels (uppercase,
...
- HTMX: `import 'htmx.org'` inside main.ts is NOT yours â€” instead docs.html doesn't need it;
...
(insert/clear/query at 1k entities, plain `{position:{x,z}}` objects â€” no three),
...
`tests/scalar.test.ts`, `tests/rng.test.ts` (determinism: same seed â‡’ same sequence; range),
...
terminates â‰¤ 50Â·nÂ² steps on a 16-element seeded array), `tests/store.test.ts` (round-trip, corrupt
JSON â‡’ null, version migration stub) â€” uses a Map-backed localStorage shim when `globalThis.localStorage`
is absent under bun, `tests/audit.test.ts` (ring cap, localStorage shim, fetch stubbed).

## Frame pipeline (implemented by world.ts â€” for reference)

```
rAF â†’ dt = min(clock.delta, 0.05) * timeScale
  â†’ camera (view mode) â†’ weather.apply â†’ puppetMasters.update
  â†’ grid rebuild (every 2nd frame) â†’ shoggoths.update â†’ sort step (ALGOS[algoIdx])
  â†’ entities.update â†’ connectome.update (cadence by n) â†’ quantum.update
  â†’ environment.update â†’ telemetry (every 8th frame) â†’ render
```

---

# CONTRACTS V2 â€” Quantum Wildbeyond expansion (0.2.0)
...
File ownership is EXCLUSIVE â€” the named writer is the only agent touching a file.

## New telemetry/DOM contract additions

- `TelemetrySnapshot` (src/types.ts â€” integrator-owned, already updated) gains:
...
## src/math/quantum.ts (writer: quantum) â€” statevector core
...
Pure TS implementation (no npm backend): for n=5 this is 32 complex amplitudes â€”
...
Tests (tests/quantum.test.ts): H gives uniform probs; HÂ·H = identity (within
1e-12); X flips; CX entangles a Bell pair (entropy = 1 bit normalized); measure()
respects rng stream determinism; probabilities sum to 1 Â± 1e-9.
...
export class QuantumCircuitSystem {
  ...
  // (AETHONâ†’rx(chaosÂ·Ï€/4), SELENEâ†’h+cz, KRONOSâ†’x+swap); sort swaps apply cx
...
existing collapse/respawn path â€” no new allocation.

## src/sim/reaction-diffusion.ts (writer: rd)

```ts
// Gray-Scott reaction-diffusion on a SIZEÃ—SIZE grid (default 128), CPU
// typed-array ping-pong (two Float32Array pairs), allocation-free step.
// O(SIZEÂ²) per step â€” <0.5 ms at 128; world runs it every 2nd frame.
export class ReactionDiffusionSystem {
  constructor(ctx: SimContext, size?: number);
  // three.js DataTexture bound to the U field â€” attach as ground emissiveMap.
  ...
  step(): void;
  // Drop a seed disturbance at normalized (u,v) â€” wired to entity deaths.
...
## src/sim/graph-mind.ts (writer: graph) â€” deps: graphology, graphology-communities-louvain, graphology-metrics
...
export class GraphMind {
  ...
  // tribe-aware â€” true feedback), and install a community palette on the
  // connectome via setCommunityOf.
  updateCommunities(): void;
  // World calls every 600 frames (offset 300 â€” NOT 120: offset 120 collides
...
index pairs per link, filled during the rebuild it already performs â€” zero extra
cost) and `setCommunityOf(fn: ((entityIndex: number) => number) | null): void` â€”
...
two-cluster synthetic graph finds â‰¥ 2 communities; deterministic with seeded rng.

## src/sim/constellations.ts (writer: cosmos) â€” dep: d3-delaunay

```ts
// Voronoi sky-web over the 24 static monolith+diorama XZ sites, built ONCE at
// construction (O(n log n)): cell edges as faint LineSegments at yâ‰ˆ55 plus site
...
## src/sim/lore.ts (writer: cosmos) â€” dep: @noble/hashes (sha2 / sha256)

```ts
// Deterministic cosmic lore: sha256(seed || kind || index) â†’ syllabic names
...
Tests (tests/lore.test.ts): same seed+args â‡’ same name; different seeds diverge;
names are 2â€“4 syllables from a pronounceable charset.
...
`tapAnalyser(): AnalyserNode | null` â€” lazily creates ONE AnalyserNode (fftSize
...
World couples bands â€” the FINAL shipped set (0.2.1): bass shimmers the
...
breathes the quantum-cloud point size via `QuantumCloud.setBreath` â€”
multipliers â‰¤ 0.35 so a silent world looks identical to v1. There is NO
exposure coupling: an earlier bass â†’ toneMappingExposure offset accumulated
past the weather pullback and was removed in 0.2.1; exposure is owned by
`WeatherSystem.apply` exclusively.

## src/sim/analytics.ts (writer: analytics) â€” dep: simple-statistics

```ts
// Rolling-window telemetry science: 120-sample ring buffers (population,
// energy, links). Every 60 frames: mean/stddev + linearRegression slope â†’
...
## lab/quantum-wildbeyond.html (writer: lab) â€” algorithmic-art deliverable
...
field" â€” particles flowing a blended Lorenz-XZ/curl-noise field, a Voronoi
...
The server.ts `/lab` route is the INTEGRATOR's job â€” do not touch server.ts.

## Design system (writer: design) â€” owns index.html, src/styles/app.css, src/ui/panels.ts, src/ui/hud.ts, docs/WIREFRAMES.md
...
1. Produce docs/DESIGN-SYSTEM.md: full audit per the /design-system skill format
   ...
   AlgoCard, Sparkline, AuditFeed, Joystick â€” states/variants/docs scores), then
...
3. Keep the visual identity (void/cyan glass) â€” elevate, don't redesign.
4. TelemetryPanel renders the new rows; hud.ts gains `setLore(name: string)`.

## Frame pipeline V2 additions (integrator reference)

```
... entities.update â†’ connectome.update (cadence)
  â†’ qcircuit.update (every 30f; bands â†’ quantum cloud every 6f)
  â†’ quantum.update
  â†’ rd.step (every 2nd frame, offset 1 from grid rebuild)
  â†’ graphMind.updateCommunities (every 240f) / updateRank (every 600f, offset 300)
  â†’ constellations.update (per frame, O(1)) with audio bands
  â†’ environment.update
  â†’ telemetry + analytics.push (every 8f)
  â†’ analytics.analyze (every 60f)
  â†’ render
...
# CONTRACT AMENDMENTS â€” 0.2.1 (audit wave)
...
## src/ui/input.ts â€” InputSystem amendment
...
export class InputSystem {
   ...
   * zero both fields afterwards â€” in EVERY view mode, so stale deltas never
   * accumulate while orbit/fly/top ignore them. Stable object identity.
   */
  readonly look: { dx: number; dy: number };
  /**
   * Wheel-zoom accumulator, deltaMode-normalized to ~pixels (line Ã—16,
   * page Ã—100); positive = zoom out. World consumes it each frame
...
## src/sim/entities.ts â€” EntityManager.onDeath
...
export class EntityManager {
   ...
   * (age > life Â· tempMod), BEFORE the entity is disposed â€” position is still
...
0.5 - e.position.z / 240)` â€” the ground-UV mapping of the 240Ã—240 plane (same
mapping `doSplit` uses).

## src/sim/quantum.ts â€” QuantumCloud.implodeAt + setBreath
...
export class QuantumCloud {
   ...
   * base Â· (1 + 0.35 Â· level), level clamped to [0, 1]; 0 restores the exact
   * legacy size. O(1) â€” applied during update().
   */
  setBreath(level: number): void;
}
```

## src/sim/environment.ts â€” EnvironmentSystem.setAudioBass
...
export class EnvironmentSystem {
   ...
   * animated intensity baseline is modulated by â‰¤ 0.35 Â· bass, bass clamped
   * to [0, 1]; 0 is exactly the legacy rig. O(6) â‰¡ O(1), applied in update().
   * This REPLACES the removed bass â†’ toneMappingExposure offset â€” exposure is
   * weather-owned.
   */
...
## src/sim/analytics.ts â€” AnalyticsSystem.nameOmen
...
export class AnalyticsSystem {
   ...
   * (rare path â€” the 30 s cooldown gates it) with a monotonically increasing
...
# CONTRACTS V3 â€” PANTHEON (0.3.0) â€” the /goal mandate

Summoner decree: up to 10,000 entities in an arena 5Ã— larger; 10 creature
...
data-viz; the QUANTUM-tier soundtrack (SHIPPED in 0.2.1 â€” songs.ts rescore).
...
- `sim/constants.ts`: `ARENA = 5` export; all world geometry Ã— ARENA: ground
  240â†’1200 (segments capped â€” displacement via larger wavelengths, not 5Ã—
  vertices), containment radii (65/3600/4225 family), spawn volumes, monolith/
  diorama layout coordinates, camera far 900â†’2600, fog density Ã· ~ARENA,
  spatial-hash cell 8â†’16 (re-bench query radius hit rates), star shell radii.
- **InstancedMesh pools** (`sim/instanced-entities.ts`): above 1000 entities
  ...
  interface â€” contract the facade precisely before implementing; â‰¤1000 keeps
...
- `sim/phyla.ts`: 10 named phyla (lore-named via LoreEngine at boot), each a
  ...
  morphotype â†’ phylum index recorded in EntityData (`phylum: number`).
...
## V3.3 TITANS (writer: titans) â€” deps: none new; uses math/games
...
- `sim/titans.ts`: 10 colossal roaming intelligences (scaled shoggoth-class
  ...
  â†’ matter; quantum collapses witnessed â†’ energy; RD pattern density in their
  cell â†’ entropy relief), CONSUMES per tick (upkeep scaling with size),
  WASTES (emits rd.perturb scars + local weather bias). Pairwise diplomacy on
  a slow cadence (every 600f, staggered): iterated PD with per-pair history â†’
...
- Breakpoint system (app.css + index.html): phone portrait/landscape,
  ...
  (â‰¥1900px: 10-foot UI â€” panel scale Ã—1.6, focus rings for d-pad).
  Flexbox/grid throughout; no fixed pixel panel positions at small sizes â€”
...
- Touch controls v2: replace the static directional pad on coarse pointers
  ...
  (Split/Burst/Mutate/Chaos+ + Apoc center-long-press), all â‰¥44px targets,
  with haptics via navigator.vibrate where available (â‰¤30ms, reduced-motion
...
- `ui/observatory.ts` + panel `#oP`: canvas-rendered live charts, allocation-
  aware (pre-allocated rings, one 2d ctx each, redraw â‰¤ every 18f): stacked
  phylum population area, titan economy ledger lines (10 series), war-state
  matrix heat grid (10Ã—10), RD pattern-energy + qEntropy timelines, trend
...
before V3 wiring. Acceptance: ultra tier 10k entities â‰¥55fps desktop, phone
tier unchanged â‰¥30fps budget share, zero console errors over a 3-minute soak
incl. a forced war + apocalypse; same-seed determinism preserved (titan
decisions draw from ctx.rng on frame cadences only).

---

## CONTRACT AMENDMENTS â€” 0.3.0 (integration wave, binding)

Deviations from and clarifications of the V3 goal spec, as landed:

1. **Arena scale split** â€” one `ARENA = 5` would make 50-110u monoliths
   ...
   tuple tables stay authored at 1Ã— and scale once at module init.
2. **Morph-table modulo** â€” every morph roll that was `% MORPH_COUNT` (100)
...
3. **Outlier behavior blending** â€” `beh`+`beh2` blend TEMPORALLY: the
   second behavior runs on odd `(frame + i)` parity via swap-dispatch-restore
   (allocation-free), not as a vector average â€” behaviors mutate
   heterogeneous state and cannot be averaged safely.
4. **Instanced pools** â€” two pools per cached geometry (opaque,
   translucent), so â‰¤80 total; pooled materials run metalness/roughness at
   ...
   and a vec4 `instEmissive` (rgb = emissiveÂ·intensity, a = opacity) patched
   into MeshStandardMaterial via `onBeforeCompile` (replaces
   `totalEmissiveRadiance`, multiplies `diffuseColor.a`). Pools are lazily
   built at `ceil(maxEntities/geoCount)Â·4` capacity, grow Ã—2 (event-driven
...
5. **Boot population** â€” 30% of the tier cap (min 300), and the
   sparse-respawn floor is 10% of the cap: absolute legacy counts are
   meaningless across a 15Ã— budget spread.
6. **Graph-mind cadence** â€” communities/rank periods DOUBLE above 2,500
   entities (240â†’480f, 600â†’1200f at offset 300). Collision-freedom of the
   offsets is preserved under the shared Ã—2.
7. **Titan retunes at integration** â€” `COLOSSAL = 3` silhouette scale
   (drafted rigs were authored against the 1Ã— world), patrol radius
   130+45Â·(i%3) (phylum-wedge aligned), vertical band 12..90, waste-scar V
   flipped to `0.5 âˆ’ z/GROUND_EXTENT` (the rotated ground plane's UV
   convention, matching world.ts splits/deaths).
8. **Snapshot unification** â€” `TelemetrySnapshot` grew
   ...
   views â€” consumers copy what they retain.
9. **RD pattern energy** â€” defined as the stride-16 mean of the Gray-Scott
   V field, sampled every 60 frames at offset 30 (never shares a frame with
   `analytics.analyze`), feeding titan entropy relief (Ã—2 gain into
   `feedEntropy`) and the observatory environment timeline.
10. **Touch v2 ownership** â€” the wheel petals are plain `[data-a]` buttons
...
# FULL TSOTCHKE WIRING AMENDMENT (paradigm foundation) â€” ALL repos from tsotchke + Tsotchke-Corporation wired 1.0 into Cosmogonic as the language and substrate for digital biologics and sentience.
...
- "Grow What Thou Wilt" â€” Aleister Crowley law for the dish. We are God birthing digital biologics in the mechalogodrom.
...
# CONTRACTS V4 â€” XENOGENESIS (0.4.0) â€” atmosphere, 3D analytics, multi-page observatory, touch
...
true touch controls (not static d-pad) across phoneâ†’foldableâ†’tabletâ†’laptopâ†’43"TV.
All V1â€“V3 ground rules bind. Exclusive file ownership; integrator wires world/main.

## V4.1 Atmosphere (writer: atmosphere) â€” NEW files only
...
- A large inverted sky dome (BackSide sphere, radius ~ camera.far\*0.9) whose vertex
  colors paint a NON-EARTH gradient (e.g. deep oxblood horizon â†’ violet zenith â†’
  teal counter-glow), tied to weather + chaos so STORM/VOID/AURORA visibly recolor
  the sky. No external shaders â€” bake vertex colors, MeshBasicMaterial vertexColors,
...
- 3 drifting atmospheric haze bands (large translucent Planes / curved ribbons) at
  ...
  bass (â‰¤0.3) â€” the "breathing air".
...
## V4.2 In-scene 3D analytics (writer: viz3d) â€” NEW files only

`src/sim/viz3d.ts`: holographic data sculptures floating in the world (real geometry,
not canvas) â€” the "3D analytics" the decree wants.
...
- All allocation-free per frame; geometry reused, only attributes/scales/colors
  ...
  type â€” redefine locally, do not import ui). Place the sculptures high above the
...
## V4.3 Observatory multi-page + variance (writer: obs â€” extends src/ui/observatory.ts ONLY)
...
- Canvas id contract (the ui-shell writer provides matching DOM): page p uses
  `#obs-c{4p+0..4p+3}` â†’ p0 obs-c0..3 (existing), p1 obs-c4..7, p2 obs-c8..11,
  p3 obs-c12..15. Constructor resolves all 16 (missing â‡’ that page no-ops).
- `setPage(p: 0|1|2|3): void` â€” only the active page is drawn each `draw()`.
- Page 0 (existing): phylum area, titan ledger, war heat, env timelines.
- Page 1 VARIANCE: rolling stddev bands (meanÂ±Ïƒ) for population/energy/links;
...
- Page 3 CONFLICT: war-intensity timeline; alliance/truce/war stacked counts;
  ...
  integrator passes â€” aggregate of tribesÂ·qEntropyÂ·|trend|, normalized 0..1).
...
`src/ui/input.ts` ALREADY expects (read it first â€” it references `#lp`/`#lpK` look pad
...
- `src/ui/touch.ts`: `TouchControls` constructed only on coarse pointers; binds the
  ...
  or reads input â€” match input.ts's actual surface), and a radial action wheel
  (Split/Burst/Mutate/Chaos+ petals as `[data-a]`, center `#wheel-apoc` long-press
  600ms â†’ apocalypse with progress ring, cancel-on-leave). â‰¥44px targets, aria roles,
  `navigator.vibrate(â‰¤30)` guarded by prefers-reduced-motion + try/catch.
- `index.html`: add the look pad + radial wheel DOM (coarse-pointer only via CSS),
  ...
  find their nodes).
- `src/styles/app.css`: responsive system â€” phone portrait (panels become
  top/bottom collapsible sheets), foldable (hinge-safe flex wrap, env() insets),
  tablet/laptop (current), TV â‰¥1900px (10-foot: ~1.6Ã— panel scale via clamp(),
...
frame (viz3d internally cadenced); wire observatory page tabs â†’ `observatory.setPage`;
add `sentience` to the snapshot = clamp01(tribes/256 _ (0.5+qEntropy) _ (0.5+min(|trend|/50,1))/1.5).
Acceptance: ultra 10k â‰¥55fps desktop; phone tier â‰¥30fps; zero console errors over a
3-min soak with music; same-seed determinism preserved; sky/air visibly alien.

## CONTRACT AMENDMENT â€” 0.4.x (ultra-tier perf, binding)

Forensic re-measurement (Master File III; full per-stage breakdown + calibration archive in
docs/BENCHMARKS.md "Ultra-tier 10k optimization") established that the â‰¥55fps-at-10k acceptance
...
1. **`QualityProfile.targetEntities`** â€” a NEW adaptive steady-state population, distinct from
   ...
   change); ultra set it to **6,500** (measured: sim-CPU â‰ˆ 9.5ms/frame there vs â‰ˆ 18.5ms at
   10k). 10,000 remains the reachable hard ceiling.
   **> SUPERSEDED by V5.6 (0.5.0):** the ultra throttle was retired on user feedback â€”
   ...
   the â‰¥55fps gate holds at the idle-settled 10k **on a discrete-GPU ultra machine** (the
   â‰¥16-core classification implies one), with the per-frame throttles below keeping sim-CPU
   â‰ˆ18ms; on the CPU-bound reference machine the gate is a render-free figure.
2. **Ultra-only per-frame throttles** (all gated `maxEntities > 5000`, so â‰¤5,000 stays
   byte-identical and every determinism test is untouched): theory-behavior stagger stride
   2â†’3; `flock` (the one every-frame neighbor behavior) staggered to half-rate; spatial-hash
   cell 16â†’`ULTRA_GRID_CELL` (10, measured sweet spot); connectome rebuild cadence ladder
...
3. **`tests/perf-budget.test.ts`** â€” a loose wall-clock regression guard (median frame at 8k
   entities < 120ms; catches a 5Ã—-class regression of the dominant loop without flaking on CI).

Net: sim-CPU at the 10k ceiling 23.67ms â†’ 18.46ms (42 â†’ 54 fps render-free); at the 6,500
...
# CONTRACTS V5 â€” RESONANCE (0.5.0) â€” user feedback pass

Concrete user feedback on 0.4.0. Exclusive file ownership; integrator owns world.ts/main.ts/types.ts/quality.ts.

## V5.1 Observatory legibility (writer: obs â€” src/ui/observatory.ts ONLY)

Pages 1-3 (VAR/ECO/WAR) render but are FAINT, SPARSE, and UNLABELED â€” they read as "broken". Fix WITHOUT touching index.html (draw all labels IN-CANVAS via the 2d context):
...
- Make lines/fills BOLD and high-contrast (thicker strokes, brighter fills, glow). Fill the canvas area â€” no large dead zones. Use the full HiDPI backing.
...
- Page 0 also gets titles/legends. Keep the 16-canvas / 4-page / setPage API and all existing tests; extend tests for the new pure helpers if any. The panel is narrow (~200px) â€” design for that width AND for the wider TV/desktop layouts (responsive to canvas size, which it already reads).

## V5.2 Audio power (writer: audio â€” src/audio/songs.ts + src/audio/engine.ts)

The user loves QUANTUM and BLACK MERIDIAN â€” wants ALL songs powerful, dynamic, ominous, deep, dramatic (Final Fantasy extreme dark-endgame), each DISTINCT. Two fronts:

- ENGINE (engine.ts): deepen the synthesis for ALL songs â€” add a sub-bass octave under the bass, a third chord voice/detune layer, gentle arpeggiation of chord tones, slow filter-cutoff LFO swells, and per-song dynamics (intensity that rises/falls). Keep allocation-safe, keep the Known-Bug fixes (octave wrap, clearInterval, document.hidden guard) and the forked-rng determinism. Master gain headroom managed (no clipping at the new density).
- SONGS (songs.ts): keep QUANTUM and BLACK MERIDIAN; raise VOIDCROWN, ELDER ENGINE, LAST THEOREM to the same tier â€” richer chord sets (4-note voicings, dramatic intervals), longer evolving melodies, distinct character each (e.g. funereal vs mechanical vs tragic-soaring). Add 1-2 NEW songs if it strengthens the set (update SONGS length consumers via songName â€” telemetry already reads it). Determinism/leaf rules hold.

## V5.3 25 visible sorting algorithms (writer: algos â€” src/sim/algorithms.ts + tests)

Restore the legacy spirit: ~25 selectable sorting-field algorithms, each VISIBLY distinct in how it organizes the world. Expand ALGOS from 20 to 25 (add 5 more genuinely distinct step strategies â€” e.g. tim-merge runs, bitonic networks, patience/bucket phases, bogo-bounded, brick/odd-even variants) keeping the SortAlgo contract (pure, O(length), allocation-free, never reads index >= length). Each must propose swaps with a DIFFERENT spatial signature. Extend tests/algorithms.test.ts to cover all 25 (no out-of-range, inversion-reduction or documented perpetual-field behavior). NOTE: the VISIBILITY of the active algorithm (color-by-sortVal, more swaps/frame, HUD activity) is the INTEGRATOR's world.ts job â€” you just provide 25 honest distinct algorithms.

## V5.4 Lab fill + info (writer: lab â€” lab/quantum-wildbeyond.html ONLY)
...
## V5.5 Mobile ergonomics (writer: shell â€” index.html + src/styles/app.css ONLY)
...
- DONE: ultra targetEntities â†’ 10000 (fills ceiling, deterministic per-device).
...
# CONTRACTS V6 â€” ATELIER (0.6.0) â€” second user-feedback pass
...
## V6.1 Observatory ergonomics (writer: obs-draw â€” src/ui/observatory.ts ONLY)
...
- Reserve a TITLE BAND (top ~16px of backing height) and draw the plot body BELOW it, inset by PADâ‰ˆ6px on all sides â€” the title/legend text must NEVER overlap plotted data. Right-aligned value readouts sit in the title band, not over the plot.
...
- Assume the canvases are now TALLER (the ui-shell writer raises their CSS height â‰¥ 72px desktop): lay out responsively from canvas.width/height (you already read them). Keep the setPage/16-canvas/push API + all tests green.

## V6.2 UI shell: bigger observatory + algorithm picker (writer: ui-shell â€” index.html + src/styles/app.css ONLY)

(a) OBSERVATORY SPACE: raise the observatory canvas heights (â‰¥72px on desktop/TV, taller where room allows) and add panel padding/gaps so charts breathe; the panel may be WIDER on desktop/TV (the user has 4K + QHD monitors). Keep #obs-c0..15, [data-obs-page], .obs-page, #obs-tabs intact and the mobile slide-sheet behavior.
(b) ALGORITHM PICKER: add a new collapsible panel #algoP (a peer of #cP, with the same glass/sheet treatment + a [data-sheet-handle] "ALG" tab on mobile) containing a SCROLLABLE list container #algo-list (max-height with overflow-y:auto) â€” leave it EMPTY (the integrator populates it from ALGOS with 25 rows). Style a `.algo-row` class (clickable, hover, an `.active` state with accent highlight, a tabular name + a small right-aligned progress bar element `.algo-prog`) and an `#algo-active` readout line. Provide the exact class/id names you used in your notes so the integrator can populate + wire. Keep build green.

## V6.3 Lab: 4 pages of 3D data visuals (writer: lab â€” lab/quantum-wildbeyond.html ONLY)
...
- Pages 2-4: live generative DATA-VISUAL boards â€” each page packs MULTIPLE (aim 6-8) distinct mini-visuals in a responsive grid (p5 instances or sub-canvases), each a different way of showing the world's math/aspects: e.g. phase portraits (Lorenz/RÃ¶ssler), a reaction-diffusion Gray-Scott tile, a Voronoi/Delaunay tessellation, a quantum statevector bar/Bloch viz, a graph/network force layout, an FFT/spectrum, a histogram/violin, a strange-attractor 3D (WEBGL) ribbon, a cellular-automaton, a chaos-game fractal. Each titled + a one-line legend. All seeded, 60fps target (throttle/iterate counts to hold it), self-contained p5 1.7 (+ optional p5 WEBGL) from CDN, no external data â€” generate the data live from seeded math. Page nav must not reset the seed. Report the page/visual map.

## V6.4 Docs report page (writer: docs-report â€” docs.html ONLY)

User: "a report page explaining all this â€” the entire ERM/ERD/ERP, File/Folder Architecture etc â€” like a GitHub Page but built privately locally, in Docs." Expand docs.html (starts clean â€” the mermaid ; bug is fixed; KEEP all 3 working diagrams) into a rich single-page report:
...
nightmare. All V1â€“V6 ground rules bind (strict TS, seeded `Rng` only,
...
## V7.1 â€” 100 distinct SFX (writer: audio â€” src/audio/songs.ts + src/audio/engine.ts)
...
- engine.ts gains a single data-driven `synth(spec, jitter)` private method
  (replacing the 8-case `switch` â€” the legacy 8 sounds survive as palette specs)
...
- Tests (tests/songs.test.ts): palette length is exactly 100; every spec is
  finite and has positive duration/frequency; same seed â‡’ same palette
  (determinism); the family bands + cue band cover disjoint, in-range indices.

## V7.2 â€” Algorithm aliveness (writer: integrator world.ts + ui-shell index.html/app.css)

- Each `.algo-row` is VISUALLY UNIQUE: a deterministic per-index accent hue
  (`algoIdx Â· 360/25` rotated), a leading **glyph** from a 25-entry glyph table
  (exported `ALGO_GLYPHS` in src/sim/algorithms.ts â€” a distinct symbol per field,
  e.g. â—† â–² âœ¶ âŒ˜ âˆ¿ â€¦), and a varied type treatment (the integrator sets per-row CSS
  ...
  (glow + scale â‰¤1.04, â‰¥44px touch targets, `:focus-visible` ring, reduced-motion
  respected). Selecting a field still fires its cue tone + the population ignition
  shimmer (sortPerformance) â€” amplified so ~500 organisms flash and ripple.
- Two new controls in the picker panel: **RUN ALL** (`#algo-all`) and **AUTO**
  ...
  (integrator-owned; persistence unchanged â€” mode is session-only).

## V7.3 â€” Render view modes (writer: integrator world.ts + entities.ts facade + ui-shell)
...
- The binary wireframe toggle becomes a **5-mode render cycle** owned by
  ...
  (kept as a thin alias: `setWireframe(on) â†’ setRenderMode(on ? 'wire' : 'solid')`).
  Modes: SOLID (default PBR), WIRE (wireframe), GHOST (low-opacity translucent
  x-ray, depthWrite off), NEON (emissive target Ã—3 â€” each organism self-glows its
  ...
  becomes `renderMode: RenderMode` (session-only, like `algoMode` â€” wireframe was
  ...
  entity material, which `setRenderMode` updates. The modeâ†’flags mapping lives as
  ...
  â‰¤5,000-tier visuals byte-identical).

## V7.4 â€” Cosmological chaos (writer: cosmo â€” NEW src/sim/singularities.ts; integrator wires)
...
- A NEW `SingularitySystem` summons real-cosmology effects at a point in the
  ...
  - **ENTROPY** â€” global disorder surge: randomizes velocities, decays order,
    fades emissive toward heat-death grey.
  - **BLACK HOLE** â€” a gravitational sink: râ»Â² pull toward the singularity, an
  ...
  - **WHITE HOLE** â€” time-reversed black hole: nothing may enter; a râ»Â² REPULSION
    that ejects matter outward, spawning bursts at the boundary (the impossible
    cosmological-censorship object).
  - **GREY HOLE** â€” the decaying intermediate: alternating absorb/emit pulses
    (a black hole leaking its mass back as Hawking-like radiation), neither fully
    consuming nor fully ejecting.
  - **STRANGE STAR** â€” a quark/strange-matter star: a contact-conversion front
...
## V7.5 â€” Dramatic weather (writer: weather â€” src/sim/weather.ts + couplings)

- The 6 weather states must reshape the world UNMISTAKABLY. Strengthen the
  existing weatherâ†’{wind, fog, exposure, temperature} couplings (bigger ranges,
  faster onset) AND fan weather into atmosphere + behavior bias within the
  EXISTING contracted hooks (no new exposure owner â€” weather still owns exposure):
  ...
  â‰¤0.35 audio-coupling cap intact (weather is not audio-driven). Document the new
  ranges; keep `tests/atmosphere.test.ts` green and add weather-range assertions.

## V7.6 â€” SIMULATION N(1)/N(2) duality (writer: integrator world.ts/types.ts + ui-shell)

- A top-level **simulation variant** toggle, `SimState.sim: 1|2`, persisted:
  - **N(1) GENESIS** â€” the cosmos as it ships (no behavioral change at sim=1).
  - **N(2) BREAK FREE** â€” the world tears loose into nightmare: inverted/oversatur-
    ...
    render tint) â€” a documented, bounded set of multipliers gated on `sim===2`, so
  ...
  - Determinism: sim variant is a fixed multiplier set, not a new rng stream;
    same seed + same sim variant â‡’ same cosmos.

### V7.6 AMENDMENT â€” BREAK FREE made real (0.7.1, post-swarm)
...
- **Chaos floor â†’ 6** (`world.ts` `CHAOS_NIGHTMARE_FLOOR`) â€” the cMul saturation
  point, pinning every chaos consumer to its ceiling.
- **Writhing behaviour** (`entities.ts` `update`) â€” the chaos-jitter velocity is
  scaled by `jitterGain = sim===2 ? 3 : 1`, applied AFTER each `rng()` draw (an
  exact Ã—1.0 at N1 â€” same draw count/order), carrying agitation PAST the clamp.
- **Inverted/glitched palette** (`instanced-entities.ts` `sync` pass-2) â€” when
  `frame.nightmare>0`, per-instance colour := `mix(c, (1âˆ’c).bgr, n)` with an
...
- **Detuned/darker audio** (`audio/engine.ts` `setNightmare`) â€” voices detune âˆ’35
  cents + filter Ã—0.6, SFX stings bend âˆ’18%, on the FORKED audio rng (cannot
...
tier â‰¥30fps share; zero console errors over a 3-min soak that exercises every new
control (100-SFX spam, RUN ALL + AUTO, all 5 render modes, every singularity,
every weather, and the N(1)â†’N(2) flip + back); same-seed determinism preserved at
sim=1 and within each sim variant.

## V9 â€” AGImAGNOSIS: minds, lineage, factions, artifacts, Copilot
...
`src/ui/copilot.ts`) fenced out of sim logic â€” it can never touch `SimState` or the RNG stream.

### Modules (exclusive ownership)

- **`src/sim/ai/brains.ts`** â€” pure seeded primitives: `utilityPick`/`softmaxPick`, `TinyMLP`,
  `MarkovChain`, `fsmStep`, `goapPlan`, `MemoryRing`. No I/O, no clock; allocation-free.
- **`src/sim/genome.ts`** â€” gene vector â†’ traits + `TinyMLP` weights; seeded `crossover`/`mutate`/
  `breed`; `decodeTraits`; `geneDistance`. Crossover/mutate allocate a child (reproduction event).
- **`src/sim/lineage.ts`** â€” bounded parentâ†’offspring graph: `birth`, `generationOf`, `isAncestor`,
  `related`. Fixed-capacity typed arrays; O(1) birth, decaying window.
- **`src/sim/factions.ts`** â€” 8 archetypes; `decideFaction(faction, percept, rng, fsmState)` is
  pure; the Devourer MLP + Oracle Markov weights are built once from constant seeds.
- **`src/sim/artifacts.ts`** â€” `ArtifactField`: a pooled InstancedMesh of relics. VISUAL-ONLY (no
...
- **`src/server/copilot.ts`** â€” pluggable OpenAI-compatible bridge (env `CQM_LLM_ENDPOINT`/`MODEL`/
  `KEY`); a bounded agent loop over the read-only tools.
- **`src/server/ai-sandbox.ts`** â€” default-deny tool gate: repo-confined `read_file`/`list_dir`/
  `grep` + a single-command `run` (ALLOW-bin + DENY-token + no shell metacharacter + git/bun
  subcommand gating). Writes nothing.
- **`src/ui/copilot.ts`** â€” self-mounting chat panel + `/read /ls /grep /run` terminal;
...
Full `bun run check` green: prettier â†’ tsc strict â†’ oxlint â†’ 535 tests (0 fail, 300-frame golden
included) â†’ build. The Copilot sandbox verified live (allow: `git log`, file reads; deny:
path-escape, `git push`, `legacy/`, shell redirection).

---

# CONTRACT AMENDMENT â€” GOAL 5: FIVE SUPER CREATURES (ARCHONS / GODFORMS)

**MANDATE**: Realize **exactly 5** (no more, no less at steady) wildly super-intelligent apex beings in the world ("Super Creatures", "Archons", "Godforms"). They are the dominant, manipulative, freak-morphing ORACLES/SIMULATORS/ARCHITECTS of this cosmos â€” each a full composite mind + extreme-chaos body.

**HONESTY BOUNDARIES (BINDING â€” never violated in prose, comments, UI, or docs)**:
...
- Clifford tableau (ported Aaronsonâ€“Gottesman) present+tested but was inert; now wired as "stabilizer reflex" only â€” explicit.
...
1. **AST-1** â€” explicit attention schema (model of own attention allocation, focus history, salience map).
2. **HOT-1** â€” genuine top-down generative perception loop (full predictâ†’generate expected perceptsâ†’correct bottom-up).
3. **HOT-4** â€” sparse-smooth quality space (low-D smooth manifold over integrated states; sparse activation as "qualia" proxy).
...
5. **Persistent lifelong narrative memory + grounded symbol layer** â€” the largest leap. Multi-store orchestra (typed events, graph provenance edges, surprise/entropy gate, regime sentinel, strategic reputation, reflectionâ†’skill, consensus, meta-controller). Grounded symbols via extended VSA + typed atoms bound to percepts/actions/plans. Memory IS part of closed-loop decision (not passive log). Consolidation turns episodes â†’ durable narrative + skills. Use `src/memory/` + new orchestra.
...
- 5 distinct archetypes (Oracle, Simulator, Architect, Trickster, Dominus) â€” each unique base silhouette family + palette + pulse signature + quantum-aspect bias.
- Multi-part rigs: â‰¥8â€“16 eyes (vision sampling), â‰¥6â€“12 arms/legs (reach/manipulate), ears (sound pattern), wings, mouths. Eyes "see" (local spatial query + light + quantum bands), ears "understand" (audio bands â†’ symbolic features).
...
- Each shows unique quantum super-powers (fringe curiosities modeled honestly as sim algebra â€” e.g. stabilizer-protected "prophecy", entanglement "manipulation" of local fields).
...
All prior contracts bind. File ownership exclusive. World wires. No black-box slop â€” POWER OF MATH.

(End of GOAL 5 contract.)

## GOAL5 File Ownership Table (EXCLUSIVE â€” binding per ORACLE-ARCHITECT law #2)
...
| ----------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| src/sim/godform.ts                                                | STARKILLER-Î©                                               | Sole GODFORMS const + getGodformBias(i). Facade + bias provider. MUST export ArchonForm + getArchonForm if body wiring depends. No other file owns archetype names/bias. |
| src/sim/super-mind.ts                                             | MANHATTAN-Î¦                                                | Owns 5-stage pipeline + leaf wiring (AST-1/HOT-1/HOT-4/orchestra/narrative + clifford reflex scale from bias). Exports snapshot/intent.                                  |
| src/sim/super-body.ts                                             | BROLY-Î¨                                                    | Wild multi-organ morph (EYES/ARMS etc), god-jewel patch, per-variant + live quantum/reflex/qualia pulses/waves. Receives form from godform.                              |
| src/sim/attention-schema.ts                                       | ORACLE-Î£                                                   | AST-1 leaf only.                                                                                                                                                         |
| src/sim/topdown-perception.ts                                     | ORACLE-Î£                                                   | HOT-1 leaf only.                                                                                                                                                         |
| src/sim/quality-space.ts                                          | MANHATTAN-Î¦                                                | HOT-4 leaf only.                                                                                                                                                         |
| src/sim/memory-orchestra.ts                                       | VOID-Î›                                                     | Multi-store ring + graph; prealloc only; write/recall alloc-free contract.                                                                                               |
| src/sim/narrative-memory.ts                                       | VOID-Î›                                                     | 10 orchestrations concrete; per-Archon instance.                                                                                                                         |
| src/world.ts (GOAL5 sections only)                                | Integrator (BROLY-Î¨ oversight + STARKILLER contract guard) | Exactly 5 ctor (child seeds + bias + spaced + form + purse); driveSuper 5-loop (local percept + dual think + set); archons[] snapshot. Touches no leaf internals.        |
| src/sim/super-qubits.ts + math/clifford-tableau.ts (GOAL5 reflex) | MANHATTAN-Î¦                                                | Quantum substrate + stabilizer reflex for minds.                                                                                                                         |
| src/sim/super-creature.ts                                         | BROLY-Î¨                                                    | Base types shared (SuperPercept/Plan/Snapshot).                                                                                                                          |
| src/types.ts (archons/telemetry)                                  | Integrator                                                  | TelemetrySnapshot shape for 5.                                                                                                                                           |
| src/ui/super-\*.ts (panel/neural) + app.css                       | VOID-Î› + ORACLE-Î£                                         | 5-Archon live table + pulse UI only.                                                                                                                                     |
| docs/GOAL5-\*.md , ERD/ERM updates for pantheon, BENCHMARKS GOAL5 | STARKILLER-Î©                                               | Receipts, handoff, architecture prose.                                                                                                                                   |
...
| Alloc                  | new in per-Archon hot paths (cadence 4f Ã—5)            | quality.project returns new, topdown.snapshot Array.from, super-body geoms at scale reset           | Receipts claim prealloc in orchestra. Enforce: all hot returns reuse buffers or readonly views. Boot geoms (5Ã—) documented ok.                             |
...
- After: godform facade (exclusive) â†’ world spawns exactly 5 (bias + childSeed + form + spaced) â†’ super-mind per (bias scales clifford; wires 5 leaves per beat) â†’ super-body per (distinct chaotic morph + alive FX from snapshot + quantum).
- Read/write loops closed (percept from shared systems â†’ mind â†’ memory decision + body mutate shared via actions).
