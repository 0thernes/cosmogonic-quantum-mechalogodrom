...
**Version:** v0.19.1 · **Generated:** 2026-06-26 · **License:** Non-commercial research & play (© 0thernes; study / run / modify / share non-commercially, keep attribution, no for-profit use).
**Gate:** 1,984 tests · 92.13% line / 89.66% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).  
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,984 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 92.13% line / 89.66% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,984 tests, 92.13% line / 89.66% func coverage),
