...
**Gate:** 2,856 tests · 84.64% line / 82.21% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).
...
| Passing tests                           | **2,856** (exact tracked suite; 0 failing)                            |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**2,856 tests, 0 fail**) →
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (2,856 tests, 84.64% line / 82.21% func coverage),
