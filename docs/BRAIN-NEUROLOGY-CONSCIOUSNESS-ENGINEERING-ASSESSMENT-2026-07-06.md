<!-- reviewed: 2026-07-11 | v0.22.0 causal and A-Life addendum over historical v0.21.9 assessment | canonical: docs/VERIFICATION-ANALYTICAL-DATA.md -->

# Brain, Neurology, Consciousness & Sentience-Path Engineering Assessment — Master Edition

> **Superseding current-state addendum (2026-07-11).** The body below preserves the v0.21.9
> assessment and its historical sensitivity numbers. Current evidence is narrower: the corrected
> Tsotchke ledger contains 22 external repositories (`8 deep / 7 wired / 2 harvest / 4 fenced / 1 meta`),
> with `17/21` non-meta integrated; `classical-contrast` is an internal control. The code-grounded A-Life
> vector is `[4.0, 2.4, 3.2, 3.8, 4.3, 4.5, 4.3, 3.5, 4.0]` (breadth `3.778`, rank `#1/113`, population
> z `+2.990`, peer z `+3.131`). Batches 21–27 gate-lock and correct this receipt: differential-survival
> GA supports Open-endedness `2.4`; exact Eshkol-AD policy learning, flora-gradient chemotaxis, and a
> nonlinear AD-MLP with a live biologic self-model support Cognition/Learning `4.3`. Batch 26 found that the initial nearest-cell sampler made the shipped
...
Every value below is the current SSOT figure from `scripts/canonical-receipts.ts` / `package.json`, verified against the repo on 2026-07-11. These override the historical body and any half-remembered report claim.

| Fact                                   | Canonical value                                                                                                      | Source of truth                                   |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| **Version**                            | `0.22.0`                                                                                                             | `package.json`                                    |
| **Test floor (portable)**              | **3,275** pass / **0** fail                                                                                          | `scripts/canonical-receipts.ts`                   |
| **Latest local receipt (Windows)**     | **3,275** completed cases / 0 fail · **365** test files · **3,551,817** `expect()` calls                             | run 2026-07-14                                    |
| **Coverage floor — portable (Ubuntu)** | **84.64%** line / **82.21%** func                                                                                    | `scripts/canonical-receipts.ts` (one-sided floor) |
| **Coverage — local (Windows)**         | **93.58%** line / **91.60%** func                                                                                    | this checkout                                     |
...
| **Tsotchke corpus**                    | **22 external repos** = 8 deep / 7 wired / 2 harvest / 4 fenced / 1 meta; `17/21` non-meta integrated                | `tsotchke-registry.ts` + integration map          |
| **Butlin indicators**                  | **8/14 MET + 6/14 PARTIAL** (0/14 failed) · `indicatorOnly` — **never** 14/14                                        | `VERIFICATION-ANALYTICAL-DATA.md` §6              |
| **A-Life breadth**                     | code-grounded **3.778/5** · rank **#1 / 113** · z = **+2.990** population / **+3.131** peers; legacy self-score 4.44 | `docs/reports/assets/alife-codeground.json`       |
...
| **Tsotchke corpus** (22 external slugs, 17 integrated)                             | `src/sim/tsotchke-registry.ts`, `tsotchke-brain-intake.ts`                                 | code + current V3 fixed-family receipt                   | 8 deep + 7 wired + 2 harvest             |
...
**Slug accounting:** 22 external slugs → **8 deep · 7 wired · 2 harvest · 4 fenced · 1 meta**.
...
This is the **honest** number and is reported instead of the older `tsotchkeWiringCoverage()` interpretation that averaged only already-wired entries. We publish **17/21 integrated non-meta external repositories**, with four fences and metadata explicitly separated.
...
| Wiring breadth   | "**17/21** non-meta external repositories integrated; **8 deep / 7 wired / 2 harvest / 4 fenced / 1 meta**; `OBLITERATUS` fenced."      | "**All repositories deeply wired at full depth.**"                                      |
...
| **Tsotchke substrate**       | 22 external repos, 17/21 non-meta integrated (80.95%); every integrated row causal in current V3 fixed-family evaluation        | 4 fenced incl. OBLITERATUS; quantum-quake GPL-derived source quarantined; PINN/PIMC facade boundaries                         |
...
| 9   | **Tsotchke integration depth**            | **8.0** ★★★★☆ | 17/21 non-meta external repos integrated, every channel causal in V2. Docked for 4 fences including OBLITERATUS, GPL-derived-source quarantine, and facade-only boundaries.            |
...
| 6 | **TSO-DEPTH** | **P1** | Tsotchke depth is uneven: 22 external repositories, with 8 deep, 7 wired, 2 harvest, 4 fenced, and 1 meta. Honest integration is **17/21 = 80.95%**; `classical-contrast` is internal. | Uneven depth is valid when direct ports, deterministic facades, harvests, and fences are distinguished. Classical simulation does not imply physical-QPU speed, entropy, or correctness. | Cite the current registry and V3 fixed-family receipt: all 17 integrated rows changed behavior; the four fences including `OBLITERATUS` plus metadata were inert. GPL-derived `quantum-quake` source stays quarantined pending legal review. |
...
| 2   | **Architecture** | The brains (SuperMind ~10,081 weights, 5-stage PERCEIVE→IMAGINE→REASON→FEEL→ACT, 25 candidates/beat; EntityBrain 70 params ×50k; ApexBrain 11 organs; connectome + GraphMind); 25 Archons (5 apex + 20 echoes); 25 ToM organs / 6 families; 100/144 faculties (~30 deep-wired); Tsotchke substrate (17/21 non-meta integrated, per-repo license and representation boundaries).                                                         | super-mind.ts; godform.ts; tom-pantheon.ts; tsotchke-registry.ts                                                  |
...
- [ ] **Always publish the external integrated fraction `17/21 = 80.95%`** (TSO-DEPTH); keep `OBLITERATUS` fenced, `classical-contrast` internal, and GPL-derived `quantum-quake` source quarantined.
...
| #   | Document : line                                                                       | Stale item                                            | Stale value   | Correct value at the 2026-07-11 reconciliation                                                                     | Fix                                                                                                                                                      | Severity              |
| --- | ------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| 1   | `docs/BRAIN-NEUROLOGY-CONSCIOUSNESS-ENGINEERING-ASSESSMENT-2026-07-06.md:650`         | A-Life breadth score in current-tense prose           | `4.22 / 5`    | legacy self-score `4.44/5`; current code-grounded `3.778/5`, rank `#1/113`, population z=`+2.990`, peer z=`+3.131` | Keep the historical body labeled; publish the current code-grounded receipt from `docs/reports/assets/alife-codeground.json` in the superseding addendum | **P1 fact-drift**     |
| 2   | `docs/reports/README.md:8`                                                            | Canonical-receipts version pointer in prose           | `v0.21.6`     | `v0.22.0 · 3,275-test floor · 84.64% / 82.21%`                                                                     | Point current gate text at `VERIFICATION-ANALYTICAL-DATA.md §1` + `canonical-receipts.ts`; let `bun run sync` propagate it                               | **P1 surface-sync**   |
| 3   | `docs/KANBAN-2026-06-26.md:17`                                                        | Tsotchke corpus count                                 | `19` projects | 22 external repos; depth classes 8 deep / 7 wired / 2 harvest / 4 fenced / 1 meta; `17/21` non-meta integrated     | State that `OBLITERATUS` is fenced and `classical-contrast` is an internal control; match `TSOTCHKE-INTEGRATION-MAP` + `tsotchke-registry.ts`            | **P2 count-accuracy** |
| 4   | `docs/BRAIN-NEUROLOGY-…-2026-07-06.md:649`                                            | Version marker context ("V123 optimization complete") | `v0.21.6`     | current `v0.22.0`; `v0.21.6` and the assessed `v0.21.9` body are historical                                        | Keep the historical framing explicit; current facts live in the superseding addendum                                                                     | **P1 doc-accuracy**   |
| 5   | `docs/reports/2026-06-20-ROADMAP-TO-NHSI-AND-SENTIENCE.xml:5`                         | XML roadmap version baseline                          | `v0.18.0`     | current package `v0.22.0`                                                                                          | **EXEMPT** — point-in-time snapshot dated 2026-06-20; v0.18.0 is correct _for that date_ per the reports worldline-snapshot rule. Do NOT rewrite.        | P2 (exempt)           |
...
| 7   | _(sweep)_ any surface still on `v0.21.6` / `4.22` / `2295` / `14-of-14` current-tense | version / breadth / test-floor / Butlin               | as found      | `v0.22.0` / legacy self `4.44` + grounded `3.778` / `2,953` / `8-of-14 + 6-partial`                                | Fix at SOURCE (`package.json`, `canonical-receipts.ts`, A-Life artifact), then `bun run sync` propagates                                                 | P1                    |
...
**Section files (this assignment):** the three sections above (§17, §18, §19) preserve the v0.21.9 snapshot. Current v0.22.0 truth is `17/21 = 80.95%` non-meta external repositories integrated, with four fences including `OBLITERATUS`; the current code-grounded A-Life receipt is `3.778/5` after the batch-21–25 GA, Eshkol-AD learning, and flora-chemotaxis updates; and current causal claims remain governed by the superseding addendum. Every claim is **indicatorOnly** — computational indicators, never phenomenal consciousness, sentience, or a solved hard problem.
...
| 10  | Tsotchke: "all deeply wired" / "19 repos"                         | KANBAN prose (19); some report summaries ("all wired")                    | **Superseded → 22 external repositories.** Honest accounting is **17 integrated / 21 non-meta = 80.95%**, with 8 deep + 7 wired + 2 harvest + 4 fenced + 1 meta. `OBLITERATUS` is fenced and `classical-contrast` is internal.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **Tsotchke depth**                     | historical inconsistent tallies               | **8 deep / 7 wired / 2 harvest / 4 fenced / 1 meta** across 22 external registry entries; `17/21` non-meta integrated; `OBLITERATUS` fenced                                                                                                                  | `tsotchke-registry.ts` + integration map                                          |
