...
**Gate:** 1,514 tests · 90.80% line / 87.88% func (canonical synced via verify-receipts 2026-06-21, receipts law enforced).  
...
| Test : source ratio                        | 0.40 → **90.80% line / 87.88% func** coverage (`bun test --coverage`) |
...
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 90.80% line / 87.88% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,514 tests, 90.80% line / 87.88% func coverage),
