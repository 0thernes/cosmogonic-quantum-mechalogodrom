...
**Gate:** 1,498 tests · 90.83% line / 87.91% func (canonical synced via verify-receipts 2026-06-21, receipts law enforced).  
...
| Test : source ratio                        | 0.40 → **90.83% line / 87.91% func** coverage (`bun test --coverage`) |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,498 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 90.83% line / 87.91% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,498 tests, 90.83% line / 87.91% func coverage),
