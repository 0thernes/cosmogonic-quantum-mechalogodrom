...
| **Latest local receipt (Windows)**     | 2,380 completed cases / 0 fail · **256** test files · **2,867,137** `expect()` calls        | run 2026-07-07                                          |
...
| `tests/`           |   255 |        33,887 | the 2,360-test floor / 2,867,137 latest-local `expect()` receipt                           |
...
| Assertion density          | **2,867,137** `expect()` calls                                                                                                                                            | measured 2026-07-07             |
...
| **90° Inductive**  | bottom-up empirical          | Generalize from the 2,360-test floor / 2,867,137 latest-local `expect()` receipt: survival, foraging, clustering, opponent-plan prediction, ablation traces                     | The system **IS behaviorally intelligent** at A-Life scale — 50,000-entity coordinated pursuit/evasion, ToM menace tracking, SR look-ahead — grounded in tests, **not** speculation. Rank #1/113, breadth 4.44/5.                                           | `tests/super-mind.test.ts`, `tests/tom-pantheon.test.ts`, `tests/butlin-indicators.test.ts` |
...
| 3   | **Test / receipt integrity**              | **8.7** ★★★★½ | 2,360-test floor / 0 fail · 2,867,137 latest-local `expect()` · 255 files; 84.64% line / 82.21% func portable floor gate-enforced. Deep and honest (floor is a floor, not a headline). |
...
The canonical test corpus (**2,360-test floor / 0 fail · 255 files** at v0.21.8, with a latest-local **2,867,137 expect()** receipt) is the _floor_, not the ceiling: it proves the mechanisms run deterministically. This benchmark suite proves whether they are _load-bearing indicators_ — a strictly stronger, and currently only partially satisfied, bar.
...
| 8   | **Appendix**     | Canonical receipts (v0.21.8 · 2,360-test floor / 0 fail · 255 files · latest-local 2,867,137 expect() · 84.64%/82.21% portable, 92.01%/89.65% Windows); file inventory (730 tracked); full coupling matrix W; complete lexicon / honesty attestation; reproduction commands (`bun run check`, `bun run bench`).                                                                                                                         | canonical-receipts.ts; VERIFICATION-ANALYTICAL-DATA.md                                                            |
...
| 18  | Test suite size                                                   | A/E, B, C, D                                                              | **Confirmed current split → 2,360-test floor / 0 fail · 255 files · latest-local 2,867,137 expect().** All four reports agree once versioned to 0.21.8. Coverage: **84.64% line / 82.21% func** portable floor; **92.01% / 89.65%** Windows receipt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
