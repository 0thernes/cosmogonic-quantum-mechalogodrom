...
// TSOTCHKE CORPUS: canonical Moonlab tensor + Eshkol AD from facade (Z:\[Vibe Coded (AI)]\(Tsotchke) mirrors/moonlab + eshkol_repo ad/tape)
...
import {
  ...
  libirrepSymmetry,
  quakePerturb,
} from './tsotchke-facade'; // + eshkolDual, libirrepSym + quake from Tsotchke (Eshkol AD/tape, Moonlab, libirrep, quantum-quake) into quality HOT-4
...
export class QualitySpace {
  ...
  project(state: ArrayLike<number>): { tone: number; code: Float32Array; toneGrad?: number } {
    ...
    for (let d = 0; d < D; d++) {
      ...
      if (d === 4) {
        // second Moonlab tensor slice (deeper wiring of tensor contraction)
        this.tensorA[0] = state[0] ?? 0;
        ...
        // libirrep + quake in d=4 for more sym/alive
      ...
      if (d === 0) {
        ...
        toneGrad += adG * 0.1 + dTone.derivative * 0.05; // deeper AD/tape wire
      }
    }
