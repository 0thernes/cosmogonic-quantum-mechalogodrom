...
**Gate:** 1,771 tests · 91.97% line / 94.85% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).  
...
| Test : source ratio                     | 0.34 → **91.97% line / 94.85% func** coverage (`bun test --coverage`) |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,771 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 91.97% line / 94.85% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,771 tests, 91.97% line / 94.85% func coverage),
