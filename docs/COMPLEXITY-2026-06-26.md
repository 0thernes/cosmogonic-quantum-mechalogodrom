...
| PageRank pass                                           | `sim/graph-mind`              | O(i·(V + E) + V log K), K = 20       | every 600th frame, offset 300                                    | Top-`K`=20 entities get an emissive floor boost while rank holds; selected via a bounded min-heap (`math/heap.ts` `selectTopK`) in **O(V log K)** not a full O(V log V) sort — strict total order (rank desc, ties by ascending index) ⇒ same SET as a stable sort; offset 300 never shares a frame with the 240f Louvain pass |
