...
>
> **Habitat-denominator refresh (2026-07-10):** only the explicitly labeled flora/phyla constants row
> was refreshed for the expanded habitat. All gate receipts and trust judgments retain their
...
- Second audit update: publication-facing docs now treat Pass-1/2/3/Final-Hurrah
  mega drafts as local archive drafts, not public release links. Public truth is
  the tracked consolidated 22 master/audit pair plus the BRAIN/VERIFICATION/NHSI
  spine.
...
| Pages Bible link gap                 | High     | `bible.html`, `scripts/gen-bible-corpus.ts`, `scripts/build-pages.ts` | Fixed in second audit: root docs are copied to Pages; source/script references go to GitHub blob URLs; new `bible-public-links` test guards this. |
| Stale scale counts                   | Medium   | `SUPER-REPORT-ULTIMATE`, `OMNISCIENT`, `MASTER` family                | Current tracked scale: 585 TS files, 135,957 TS lines, 8 C/C++ files, 741 tracked files, 198,769 tracked authored lines.                          |
...
| A-Life comparative matrix           | `NHSI`, `BRAIN`                             | Include code-grounded breadth `3.844`, population `z=+3.220`, peer `z=+3.373`, and the 129-system comparison as a measured comparative claim requiring citation. The `4.44` self-score is quotable only as a labelled historical baseline.                                                                                                                                                           |
...
| Flora and phyla constants                         | Active `AlienFlora` is exactly `60,000` desktop / `20,800` mobile across the expanded 4× land area; inactive legacy `Vegetation.PLANT_COUNT=10,000` is not a live-world denominator. Preserve `PHYLUM_COUNT=10`, `MORPHS_PER_PHYLUM=25`, and `OUTLIER_RATE=0.01`. Non-plant population ceilings remain unchanged.                                                                                         |
