...
**Version:** current (Tsotchke full) Â· **Generated:** 2026-06-20 Â· **License:** Proprietary ” All Rights Reserved.
**Gate:** 1617 tests · 95.58% line / 92.69% func (canonical synced, receipts enforced).
...
| Native C++ engine (separate, ADR-0007)     | 1,644 lines Â· 6 files                                                  |
| Files                                      | **331**                                                                 |
| Folders                                    | **32** (+ root)                                                         |
| Distinct file types                        | 21                                                                      |
| App source (TypeScript)                    | 35,226 lines Â· 108 files                                               |
| Tests                                      | 14,117 lines Â· 100 files                                               |
| Test : source ratio                        | 0.40 â†’ **95.58% line / 92.69% func** coverage (`bun test --coverage`) |
...
| `.github/` Â· `bench/` Â· `scripts/` Â· `.claude/` | 27    | 1,644  |

`src/` subsystems: `sim/` 17,353 (61 files) Â· `ui/` 9,007 (19 files) Â· `world.ts` 2,395 (composition
root) Â· `math/` 2,246 (12 files) Â· `audio/` 1,644 Â· `server/` 1,644 Â· `styles/` 1,644 Â· `core/` 582 Â·
...
- **Instancing:** up to 50,000 organisms collapse into **InstancedMesh pools** (â‰¤ 80 pools, one draw
  call per geometry Ã— transparency pair ” not 50,000 draws). Per-instance channels: transform matrix,
...
beat. All three are seeded + deterministic, unit-tested, and MIT-attributed to Â© 2024“2026 tsotchke (see
...
consolidation) and an **Integrated-Information Î¦ proxy** (IIT ” the participation/coherence ratio of 8 named
...
**min-cut Î¦**, **Quantum Reservoir Computing** (Fujii“Nakajima), a **Lindblad/GKSL deliberation qubit**,
...
fourth MIT-credited primitive. **Measured cost:** the whole apex beat (`SuperMind.think()`) is **â‰ˆ 272“304 Âµs/beat**
...
`prettier --check` â†’ `tsc --noEmit` (strict) â†’ `oxlint` â†’ `bun test` (**1,1617 tests, 0 fail**) â†’
`bun scripts/build.ts`. Coverage gate: line â‰¥ 0.90, function â‰¥ 0.85 (measured 95.58% line / 92.69% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1617 tests, 95.58% line / 92.69% func coverage),
and whose entire emergent intelligence weighs **â‰ˆ 14 MB ” 1/50,000th of GPT-3** at the mega ceiling. It
demonstrates that depth comes from **architecture,
determinism, and engineering discipline**, not parameter count or hardware.

**Frontier assessments (2026-06-17):** [Report I ” Whole Repository](./reports/2026-06-17-STATE-OF-THE-ART-WHOLE-REPO.md) Â· [Report II ” Super Creature](./reports/2026-06-17-STATE-OF-THE-ART-SUPER-CREATURE.md) Â· [Combined ” unified Â§III scorecard](./reports/2026-06-17-STATE-OF-THE-ART-COMBINED.md) Â· [Super Creature Research](./SUPER-CREATURE-RESEARCH.md).
