...
[![tests](https://img.shields.io/badge/tests-1794%20passing-brightgreen)](./tests)
...
- **V64–V96 · Living skin, BRUTALISM & a mind that learns (0.16.1):** the apex god-jewel skin now EVOLVES with the creature (hue / crystalline relief / ascension blaze ramp with its level); a **BRUTALISM** mode (`B` key / ▦ toolbar button) crossfades the whole cosmos — the super creatures, every instanced organism, the ground, the light rig, the sky dome and the fog — to a raw poured-concrete monolith, and back; and **online learning is wired into `SuperMind.think()` and ON by default** — every Archon now adapts its plan preferences at runtime (a bounded, deterministic, reward-reinforced per-plan bias; `setLearning(false)` freezes it), closing the long-standing "frozen weights" gap. The emergence model now spans **10 first-class angles** (empowerment, Φ, Lyapunov promoted alongside the original seven).
...
- **The apex mind closes out at ~20 coupled faculties (V98“V100)** ” a **Lindblad/GKSL open-system
  ...
  60 fps frame)** and CI-gated. **1794 tests green · 0 fail (receipts enforced) · 95.65% line / 92.84% func coverage (measured; CI gate ≥ 90 % line / ≥ 85 % function).**
