...
- 🟧 P1 `CHAOS-BIOME-50K` (V38, partial) — **shipped:** the opt-in `mega` tier (`?tier=mega`) raises the ceiling to **50,000**; profiled (`bench/scale.ts`) + optimized — √N density scaling cut 50k from 167→60 ms (2.8×), determinism golden byte-identical; live-verified at 44,977 entities. Archetypes already met (100/250 morphotypes). **Remaining:** wire the latent 70-param per-entity genome brain (`sim/genome.ts`) into the live loop; deeper neighbour-list LOD for 50k @ 60 fps.
