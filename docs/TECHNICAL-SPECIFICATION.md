...
**Gate:** 1650 tests · 95.54% line / 92.61% func (canonical synced, receipts enforced).
...
| Test : source ratio                        | 0.40 â†’ **95.54% line / 92.61% func** coverage (`bun test --coverage`) |
...
`prettier --check` â†’ `tsc --noEmit` (strict) â†’ `oxlint` â†’ `bun test` (**1,293 tests, 0 fail**) â†’
`bun scripts/build.ts`. Coverage gate: line â‰¥ 0.90, function â‰¥ 0.85 (measured 95.54% line / 92.61% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1650 tests, 95.54% line / 92.61% func coverage),
