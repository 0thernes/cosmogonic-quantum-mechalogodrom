...
**Gate:** 2,319 tests · 84.35% line / 82.05% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).  
...
| Test : source ratio                     | 0.34 → **84.35% line / 82.05% func** coverage (`bun test --coverage`) |
| Passing tests                           | **2,319** (floor; 0 failing)                                          |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**2,319 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 84.35% line / 82.05% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (2,319 tests, 84.35% line / 82.05% func coverage),
