...
> Current receipts: **v0.21.13 · canonical floor 2,507 tests · 84.64 % line / 82.21 % function**;
> latest Windows-local gate on 2026-07-11 measured **2,507 completed cases · 0 failures ·
> 2,838,975 `expect()` calls**. The portable coverage floor remains the synced canonical
...
| 3   | Test rigor & coverage     | 9.0   | A     | `2,507`-test canonical floor; latest Windows-local full receipt measured `2,507` completed cases / `0` fail and `2,838,975` `expect()` calls. Windows coverage measured `92.85%` line / `90.55%` func; published portable coverage remains the canonical `84.64%` line / `82.21%` func floor. Receipts law (`verify-receipts.ts`) makes an unmeasured number unshippable. |
