...
**Gate:** 2,372 tests · 91.91% line / 89.62% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).  
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**2,372 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 91.91% line / 89.62% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (2,372 tests, 91.91% line / 89.62% func coverage),
