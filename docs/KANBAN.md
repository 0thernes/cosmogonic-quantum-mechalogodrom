...
- 🟧 P1 `SUPERHERO-MODE` — past V34 (puzzle unlocks + reveals the 2nd creature): build the full player-as-creature mode — controls, vision modes, life/energy bars, stats, inventory, wallet, neural state, powers, camera, progression. The `cqm:superhero-unlock` event is the hook to build on.
