...
All stages must pass. Current baseline: prettier clean · tsc 0 · oxlint 0 · **2,360-test canonical floor** ·
...
**GitHub repo hygiene (one-person, main-only):**

- **Truth branch:** `main` only. Cloud-agent `cursor/*` branches are ephemeral; delete after merge:
  `git push origin --delete cursor/<name>-5433`
- **Stale fleet branches** (`swarm-*`, `audit/*`, old `claude/*`) — safe to delete when superseded by `main`:
  `git branch -r` to list · `git push origin --delete <branch>` per branch.
