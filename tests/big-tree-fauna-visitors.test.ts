...
import type {
  BigTreeFaunaActor,
  BigTreeFaunaCategory,
  BigTreeFaunaSource as BigTreeActorSource,
...
class FakeActorSource implements BigTreeActorSource {
  readonly bigTreeActorCount = 1;
  readCalls = 0;
  writeCalls = 0;
  nourishmentCalls = 0;
  restCalls = 0;
  lastNormalizedNutrition = 0;
  controlled = false;
  readonly actor: BigTreeFaunaActor;

  constructor(category: BigTreeFaunaCategory, ownerId: number) {
    this.actor = {
      ownerId,
      category,
      locomotion: 'flight',
  ...
  readBigTreeActor(index: number, out: BigTreeFaunaActor): boolean {
    ...
    Object.assign(out, this.actor);
    return true;
  }

  writeBigTreeActor(index: number, actor: Readonly<BigTreeFaunaActor>): boolean {
    if (index !== 0 || actor.ownerId !== this.actor.ownerId) return false;
    this.writeCalls++;
    this.actor.vx = actor.vx;
    this.actor.vy = actor.vy;
    this.actor.vz = actor.vz;
    this.actor.energy = actor.energy;
    return true;
  }

  nourishBigTreeActor(index: number, normalizedNutrition: number): boolean {
    if (index !== 0) return false;
    this.nourishmentCalls++;
    this.lastNormalizedNutrition = normalizedNutrition;
    this.actor.energy = Math.min(1, this.actor.energy + normalizedNutrition);
    return true;
  }

  restBigTreeActor(index: number, dt: number): boolean {
    if (index !== 0 || !(dt >= 0)) return false;
    this.restCalls++;
    return true;
  }
...
describe('BigTreeFaunaVisitors — generic canonical-fauna binding', () => {
  test('composition adapter unifies all canonical owner kinds with NHI and preserves native sinks', () => {
    const definitions: ReadonlyArray<readonly [number, BigTreeFaunaCategory, FakeActorSource]> = [
      [BIG_TREE_OWNER_SHOGGOTH, 'shoggoth', new FakeActorSource('shoggoth', 10)],
      [BIG_TREE_OWNER_TITAN, 'titan', new FakeActorSource('titan', 11)],
      [BIG_TREE_OWNER_LEVIATHAN, 'leviathan', new FakeActorSource('leviathan', 12)],
      [BIG_TREE_OWNER_PUPPET, 'puppet', new FakeActorSource('puppet', 13)],
      [BIG_TREE_OWNER_APEX, 'apex', new FakeActorSource('apex', 14)],
    ];
    const bindings = definitions.map(([ownerKind, category, source]) =>
      bindBigTreeActorSource(ownerKind as 3 | 4 | 5 | 6 | 7, category, source),
    );
    const nhi = new FakeFaunaSource([99]);
    bindings.push(bindBigTreeFauna(BIG_TREE_OWNER_NHI, nhi));

    expect(BIG_TREE_OWNER_PUPPET_MASTER).toBe(BIG_TREE_OWNER_PUPPET);
    expect(new Set(bindings.map((binding) => binding.ownerKind)).size).toBe(6);
    expect(bindings.map((binding) => binding.ownerKind)).toEqual([
      BIG_TREE_OWNER_SHOGGOTH,
      BIG_TREE_OWNER_TITAN,
      BIG_TREE_OWNER_LEVIATHAN,
      BIG_TREE_OWNER_PUPPET,
    ...
    for (let index = 0; index < definitions.length; index++) {
      const source = definitions[index]![2];
      const binding = bindings[index]!;
      const sample = {} as BigTreeFaunaVisitorSample;
      source.readCalls = 0;
      expect(binding.source.readBigTreeVisitor(0, sample)).toBe(true);
      expect(sample.ownerId).toBe(source.actor.ownerId);
      expect(sample.hunger).toBe(0.75);
      expect(sample.healthDeficit).toBeCloseTo(0.2, 12);
      expect(
        binding.source.setBigTreeVisitorIntent(
          source.actor.ownerId,
      ...
      expect(source.writeCalls).toBe(1);
      expect(source.controlled).toBe(true);
      expect(source.actor.vx).toBe(20);
      expect(source.actor.vy).toBe(0);
      expect(source.actor.vz).toBe(0);
      expect(binding.source.nourishBigTreeVisitor(source.actor.ownerId, 28)).toBe(true);
      expect(source.nourishmentCalls).toBe(1);
      expect(source.lastNormalizedNutrition).toBe(0.28);
      expect(binding.source.restBigTreeVisitor?.(source.actor.ownerId, 0.5)).toBe(true);
      expect(source.restCalls).toBe(1);
      expect(binding.source.clearBigTreeVisitorIntent(source.actor.ownerId)).toBe(true);
      expect(source.controlled).toBe(false);
    }
  });

  test('one Apex binding composes fixed autonomous bodies without duplicate owner kinds', () => {
    const first = new FakeActorSource('apex', 20);
    const second = new FakeActorSource('apex', 21);
    const binding = bindBigTreeActorSource(BIG_TREE_OWNER_APEX, 'apex', [first, second]);
    const sample = {} as BigTreeFaunaVisitorSample;

    expect(binding.source.bigTreeVisitorSlotCount).toBe(2);
    expect(binding.source.readBigTreeVisitor(0, sample)).toBe(true);
    expect(sample.ownerId).toBe(20);
    expect(binding.source.readBigTreeVisitor(1, sample)).toBe(true);
    expect(sample.ownerId).toBe(21);
    expect(binding.source.clearBigTreeVisitorIntent(20)).toBe(true);
    expect(binding.source.clearBigTreeVisitorIntent(21)).toBe(true);
  });
  ...
  test('social matching performs one source read per active visitor under bounded crowd load', () => {
    const crowdSize = 48;
    const source = new FakeFaunaSource(
      Array.from({ length: crowdSize }, (_, index) => index),
      0,
    );
    ...
    visits.addRadialSlots(BigTreeSlotKind.Socialize, crowdSize, 6);
    const tree = new FakeTreeFood();
    const adapter = new BigTreeFaunaVisitors(
      tree,
      visits,
    ...
    tree.setTime(0);
    adapter.update(0, 0, { foodAvailable: false, socialNeed: 1, curiosity: 0 });
    source.advance(0.1, 1_000);
    const before = stats();
    adapter.readStats(before);
    expect(before.activeVisitors).toBeGreaterThan(20);

    source.readCalls = 0;
    tree.setTime(0.1);
    adapter.update(0.1, 0.1, { foodAvailable: false, socialNeed: 1, curiosity: 0 });

    const after = stats();
    adapter.readStats(after);
    expect(after.activeVisitors).toBe(before.activeVisitors);
    // updateActive samples each source slot once; the exhaustive matcher consumes only cached XYZ.
    expect(source.readCalls).toBe(before.activeVisitors);
  });
});
