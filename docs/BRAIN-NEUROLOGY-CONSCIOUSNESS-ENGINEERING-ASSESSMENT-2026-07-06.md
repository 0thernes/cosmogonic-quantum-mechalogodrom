...
> vector is `[4.0, 2.2, 3.0, 3.8, 3.8, 4.5, 4.3, 3.5, 4.0]` (breadth `3.678`, rank `#1/113`, population
> z `+2.83`, peer z `+2.95`). A 30-seed organism-intelligence benchmark passed resource seeking,
> `17/17` integrated-channel causality, `5/5` excluded-channel inertness, and performance, but failed its
> preregistered 5% reversal-adaptation criterion at `3.9129%`. Shuffled-channel and entropy-matched
> classical controls did not show substrate- or quantum-specific uplift. Therefore no brain, A-Life,
> consciousness, or sentience score is increased. See the
...
| **Tsotchke corpus** (22 external slugs, 17 integrated)                             | `src/sim/tsotchke-registry.ts`, `tsotchke-brain-intake.ts`                                 | code + V2 causal receipt                                 | 8 deep + 7 wired + 2 harvest             |
...
- **Attributed math ports and deterministic adaptations (Tsotchke corpus).** Eshkol AD/GWT/VM, Moonlab Clifford/tensor/VQE, QGT, libirrep, and spin/Hopfield techniques have per-module provenance. The QRNG compatibility path is an independent seeded classical statevector adaptation, not physical entropy or a CSPRNG. The V2 causal receipt shows all 17 integrated external channels change final entity velocity and all four fences plus metadata are inert; it does not establish physical-QPU performance or quantum advantage.
...
**Result (canonical):** the V2 receipt shows **all 17 integrated external channels are causal** — each
moves final entity velocity by more than `1e-9`; the four fences plus metadata are exact zero. This is a
deterministic bounded task result, not a claim that every upstream repository is natively ported.
...
| **Tsotchke substrate**       | 22 external repos, 17/21 non-meta integrated (80.95%); every integrated channel causal in V2                                    | 4 fenced incl. OBLITERATUS; quantum-quake GPL-derived source quarantined; PINN/PIMC facade boundaries                         |
