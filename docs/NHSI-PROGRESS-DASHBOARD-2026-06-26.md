...
**Dated 22-report audit/master pair:** [`CONSOLIDATED-22-MASTER-ASSESSMENT-CURRENT-2026-07-07.md`](./CONSOLIDATED-22-MASTER-ASSESSMENT-CURRENT-2026-07-07.md) and [`CONSOLIDATED-22-FILE-AUDIT-CURRENT-2026-07-07.md`](./CONSOLIDATED-22-FILE-AUDIT-CURRENT-2026-07-07.md) preserve the 2026-07-07 publication synthesis and file-trust ledger. Their receipt numbers are historical. <!-- cqm-sync:local-measurement:start -->Current Windows evidence is `2,507 tests / 0 fail / 2,838,975 expect()` with `92.85%` line / `90.55%` function coverage.<!-- cqm-sync:local-measurement:end --> The separate `84.64% / 82.21%` portable coverage floor lives in [verification data](./VERIFICATION-ANALYTICAL-DATA.md). The pair preserves exact lab/Archon/event handles and does not claim sentience.
...
| ----------------------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **100 Faculties**       | 11 strata, coupling > count        | **100 instantiated in `FacultiesPantheon`** (stepped each beat in `world.ts` → aggregate activation = a world/percept bias), of which **~30 are DISTINCT faculties genuinely deep-wired into the apex `think()`** (each reads state AND moves the plan/drives/consciousness); the remaining ~70 are a generic-profile bias bank, plus a brutal god-layer (VOID/PHOENIX/CHAOS/REALITY/SPIRAL) wired via godPower. | **~30 deep-wired · 100 instantiated** (current code-grounded 113-system breadth `3.756`, population z `+2.954`) | `faculties-pantheon.ts`, `world.ts`, `super-mind.ts`, coupling-audit                                                     |
