...
**Gate:** 2,447 tests · 84.64% line / 82.21% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).
...
| CSS (Tailwind)      | 2     | 2,476   | 1.20 %  |
| C/C++ hdr+impl      | 8     | 1,527   | 0.74 %  |
| CSV                 | 4     | 844     | 0.41 %  |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**2,447 tests, 0 fail**) →
...
### `src/` — heaviest files (294 files · 99,555 lines total; top of the list)
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (2,447 tests, 84.64% line / 82.21% func coverage),
