# Super Creature 1.1 â€” Scientific Grounding
...
layer_ (and the V84â€“V88 quantum-substrate ports it sits on). Every reference below was retrieved from a
...
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Global Workspace / GNW** (Baars, Dehaene) â€” "ignition"                          | `Consciousness.ignition` â€” a winner-take-all over the 7 plan-coalitions; when the winner crosses an access threshold _and_ dominates the runner-up it "broadcasts", gating memory consolidation (`super-mind.ts`).                                                                              | A toy model of the _signature_ (ignition + broadcast), not the cortical phenomenon. The 2025 Cogitate test pressured the offset-ignition prediction.           |
| **Integrated Information Theory** (Tononi) â€” Î¦                                   | `Consciousness.phi` â€” the participation/coherence ratio of 8 named module activations (1 â‡’ parts act as one, 1/M â‡’ independent).                                                                                                                                                            | Explicitly a **tractable surrogate**. True Î¦ is super-exponential _and_ non-unique; we never claim to compute Î¦.                                             |
| **Predictive processing / active inference** (Friston)                              | `ActiveInference` (active-inference.ts) â€” a discrete free-energy core: a Bayesian belief over 8 latent situations minimising variational free energy F, then plan selection by **expected** free energy G (epistemic curiosity + pragmatic goal-seeking). Plus the predictorâ†’`surprise` loop. | A faithful discrete-AIF implementation; the Free Energy Principle is a unifying _theory_, not a settled empirical fact.                                        |
| **Quantum cognition** (Busemeyer, Trueblood)                                        | The 6-qubit register + Eshkol Born-sampled "thought collapse" + the QGT/Fubiniâ€“Study readout (V75â€“V88).                                                                                                                                                                                       | "Quantum" here is an **algebra on amplitudes/probabilities** in a deterministic sim â€” nothing about quantum neurons.                                         |
| **Higher-Order Theory / metacognition** (Rosenthal; Lau & Rosenthal; Fleming & Daw) | `Metacognition` (metacognition.ts) â€” a second-order **confidence** in the decision (decision margin + Î¦ + belief certainty + calm), spent as cognitive control: low confidence â‡’ EXPLORE to resolve uncertainty, high â‡’ commit/exploit.                                                    | A computational confidence/control model â€” NOT a claim of subjective higher-order awareness.                                                                 |
| **Organoid intelligence / "wet computing"**                                         | `Reservoir` (reservoir.ts) â€” a 64-node **echo-state network** (the algorithm physical/organoid reservoirs use): a fading nonlinear echo of recent latents giving temporal memory + a novelty signal that sharpens curiosity.                                                                    | We implement the reservoir-computing **algorithm**, NOT wetware â€” there is no biological substrate. Organoid reservoirs are the real-world inspiration only. |

---

## 1. Scientific theories of consciousness â€” the IIT vs GNWT adversarial collaboration (Cogitate)

- The flagship result is published and **challenges key tenets of both theories â€” asymmetrically, not a win for either side** ("double-bind" is our shorthand, not the paper's term; GNWT's central prefrontal-ignition prediction was challenged more severely): the
  preregistered adversarial test (n = 256; fMRI + MEG + iEEG) aligned with _some_ predictions of both IIT
  and GNWT while **challenging key tenets of both** â€” IIT contradicted by a lack of sustained posterior
...
## 2. Integrated Information (Î¦) â€” tractable proxies and intractability

- True Î¦ is not just expensive, it is **under-determined**: Hanson & Walker (2023) show Î¦ in IIT 3.0 is
  _non-unique_ â€” systems exist for which Î¦ = 0 and Î¦ > 0 are simultaneously valid.
...
- A 2024 peer-reviewed alternative sidesteps Î¦ with a tractable **integrationâ€“segregation difference**
  (ISD), classifying conscious states at ~93% accuracy â€” a _correlate_, not an identity claim (Jang,
  Mashour & Hudetz, 2024, _Nature Communications_). **This is the family our `phi` proxy belongs to.**
- **Caveat:** integrated-information measures sometimes _increase_ under anesthesia, contrary to IIT â€” they
...
- **Brainoware** â€” a human brain organoid as a physical reservoir for speech-recognition / nonlinear tasks;
  a real, peer-reviewed proof-of-concept with modest accuracy (Cai, Ao, Tian et al., 2023, _Nature
  Electronics_).
- **DishBrain** â€” in-vitro neurons "learn Pong"; the word _sentience_ is the authors' **operational
...
- **FinalSpark Neuroplatform** â€” 16 organoids / ~160k neurons accessible over the cloud; the "1,000,000Ã—
...
- A 2024 bridge approximates the otherwise-intractable exact free-energy computation with diffusion models â€”
...
## 6. World models & predictive maps â€” the successor representation
...
- The **successor representation** (SR) reformulates value as expected discounted future state-occupancy,
  ...
  map** â€” place/grid fields as discounted future occupancy, not mere present location (Stachenfeld,
...
- **What Super Creature 1.1 computes:** `src/sim/successor-representation.ts` â€” a deterministic SR matrix
  over the 7 plan-archetypes, learned online by the TD rule M(s,Â·) â† M(s,Â·) + Î±[Ï†(s) + Î³M(sâ€²,Â·) âˆ’ M(s,Â·)]
...
- **Honesty caveat:** a tabular SR over 7 discrete plan-states â€” a faithful implementation of the SR
...
## 7. Empowerment â€” information-theoretic agency / intrinsic motivation

- **Empowerment** is a reward-free, agent-centric measure of control: the channel capacity I(Aâ¿; Sâ€² | s)
  between an agent's action sequence and its resulting future state â€” the maximum information the agent can
  ...
  uncertainty-reducing ones. It remains a live frontier â€” Tiomkin, Nemenman, Polani & Tishby (2023) give the
  ...
  both **positively track** human open-world exploration (information-gain does _not_), playing complementary roles â€” not a blanket "empowerment beats curiosity" result.
- **What Super Creature 1.1 computes:** `src/sim/empowerment.ts` â€” each beat it bins the 16-D world-model
  ...
  transitions, and runs the **Blahutâ€“Arimoto** algorithm (Blahut, 1972; Arimoto, 1972) for a fixed 12
  ...
  future. Blahutâ€“Arimoto's iterate provably raises the capacity lower bound monotonically toward the true
  capacity â€” a closed-form property the module unit-tests, alongside the two known limits (a deterministic,
  fully-controllable channel â‡’ empowerment 1; a degenerate channel â‡’ 0).
- **Honesty caveat:** a discrete, online empowerment estimate over 7 plan-archetypes and 64 LSH cells â€” a
  faithful implementation of the empowerment _objective_ and the Blahutâ€“Arimoto capacity _algorithm_, not a
...
## 8. Vector Symbolic Architectures / holographic memory â€” compositional binding

- **Vector Symbolic Architectures** (VSA) / **Holographic Reduced Representations** (HRR) encode symbolic
  structure in distributed high-dimensional vectors via three algebraic operations â€” BIND (tie a role to a
  filler), BUNDLE (superpose a set), PERMUTE (order) â€” plus a cleanup memory that snaps noisy vectors back to
  clean atoms (Plate, 1995; Kanerva, 2009; Gayler, 2003). Because random high-D vectors are near-orthogonal,
  many bindings superpose in ONE vector and are recovered by unbinding â€” a candidate bridge between
...
- **What Super Creature 1.1 computes:** `src/sim/holographic-memory.ts` â€” the MAP-VSA model on bipolar
  ...
  against the 7 plan atoms â€” an analogical prior ("in situations like this, I chose â€¦") that biases the next
  plan.
- **Honesty caveat:** a faithful implementation of the VSA/HRR _algebra_ over 7 plan-atoms and an 8-feature
  context â€” a real holographic associative memory, not a claim of human-level compositional reasoning. It
  recalls "the plan I bound to contexts like this," not arbitrary symbolic inference.

## 9. Quantum reservoir computing â€” the register as a cognitive reservoir
...
- **What Super Creature 1.2 computes:** `src/sim/quantum-reservoir.ts` â€” the apex mind's existing 6-qubit
...
- **Honesty caveat:** a deterministic 6-qubit _simulation_ used as a reservoir â€” a faithful implementation
...
## 10. Open quantum systems â€” the Lindblad decoherence of deliberation
...
- A real qubit is never closed: coupled to an environment it undergoes **decoherence** (loss of phase
  ...
  **Goriniâ€“Kossakowskiâ€“Sudarshanâ€“Lindblad** (GKSL) master equation â€” the most general generator of a
...
- **What Super Creature 1.1 computes:** `src/sim/quantum-deliberation.ts` â€” a deliberation qubit whose
  ...
  pure-dephasing channel Î³_Ï† (from arousal â€” the environmental noise) collapses the off-diagonal coherence;
...
- **Honesty caveat:** a faithful implementation of the qubit Lindblad/GKSL _master equation_ as a model of
  deliberationâ†’commitment â€” open-system physics, not a claim that decisions ARE quantum-mechanical. A
  dynamical metaphor with real, conserved (trace-preserving, completely-positive) mathematics underneath.

## 11. Stabilizer formalism â€” large-scale Clifford simulation (Gottesmanâ€“Knill)

- The **Gottesmanâ€“Knill theorem** says a circuit of only Clifford gates (H, S, CNOT, Pauli) on |0âŸ© inputs
  with computational-basis measurement is **classically simulable in polynomial time** â€” not by tracking 2â¿
  amplitudes, but the n Pauli operators that STABILISE the state. The **Aaronsonâ€“Gottesman tableau**
  (Aaronson & Gottesman, 2004) makes each gate an O(n) binary row update and measurement O(nÂ²), so
  GHZ/Bell/graph/stabiliser-code states on dozens-to-hundreds of qubits are exact and cheap â€” precisely the
  regime the dense statevector (capped near 8 qubits) can never reach.
- **What Super Creature 1.1 computes:** `src/math/clifford-tableau.ts` â€” the Aaronsonâ€“Gottesman tableau
  ...
  read in ebits straight off the stabiliser matrix as a GF(2) rank (Fattal et al., 2004) â€” intractable to
...
- **Honesty caveat:** a faithful implementation of an EXACTLY-simulable subclass of quantum mechanics (the
  Clifford group), not a universal quantum computer â€” by Gottesmanâ€“Knill no quantum speed-up is implied. Its
...
1. **Established / peer-reviewed empirical results:** the Cogitate 2025 _Nature_ adversarial test,
   Brainoware, DishBrain, the FinalSpark Neuroplatform â€” these happened and are published; describe what was
   _measured_, not what it "proves."
2. **Established theoretical frameworks (not facts):** IIT, GNWT, the Free Energy Principle, quantum-
   probability cognition are coherent, productive _theories_ â€” none is confirmed as "the" account of mind.
3. **Genuinely contested:** IIT faces a "pseudoscience" open letter (2023, PsyArXiv â€” itself controversial)
...
6. **For this art project this is ideal source material** â€” real and genuinely unsettled â€” _provided the
...
- Arimoto, S. (1972). An algorithm for computing the capacity of arbitrary discrete memoryless channels. _IEEE Transactions on Information Theory, 18_(1), 14â€“20. https://doi.org/10.1109/TIT.1972.1054753
- Blahut, R. E. (1972). Computation of channel capacity and rate-distortion functions. _IEEE Transactions on Information Theory, 18_(4), 460â€“473. https://doi.org/10.1109/TIT.1972.1054855
...
- Cai, H., Ao, Z., Tian, C., et al. (2023). Brain organoid reservoir computing for artificial intelligence. _Nature Electronics, 6_, 1032â€“1039. https://doi.org/10.1038/s41928-023-01069-w
- Dayan, P. (1993). Improving generalization for temporal difference learning: The successor representation. _Neural Computation, 5_(4), 613â€“624. https://doi.org/10.1162/neco.1993.5.4.613
- Durand, E. D., Joffily, M., & Khamassi, M. (2024). A diffusion model-based approach to active inference. _IEEE FMLDS_, 75â€“80. https://doi.org/10.1109/fmlds63805.2024.00024
...
- Ferrante, O., GÃ³rska, U., Henin, S., et al. (Cogitate Consortium). (2025). Adversarial testing of global neuronal workspace and integrated information theories of consciousness. _Nature, 642_(8066), 133â€“142. https://doi.org/10.1038/s41586-025-08888-1
- Friston, K. (2010). The free-energy principle: a unified brain theory? _Nature Reviews Neuroscience, 11_, 127â€“138. https://doi.org/10.1038/nrn2787
...
- Gayler, R. W. (2003). Vector symbolic architectures answer Jackendoff's challenges for cognitive neuroscience. In _Proceedings of the ICCS/ASCS Joint International Conference on Cognitive Science_, 133â€“138. https://arxiv.org/abs/cs/0412059
- Gorini, V., Kossakowski, A., & Sudarshan, E. C. G. (1976). Completely positive dynamical semigroups of N-level systems. _Journal of Mathematical Physics, 17_(5), 821â€“825. https://doi.org/10.1063/1.522979
...
- Huang, J.-Q., Epping, G. P., & Trueblood, J. S. (2025). An overview of the quantum cognition research program. _Psychonomic Bulletin & Review, 32_(6), 2507â€“2556. https://doi.org/10.3758/s13423-025-02675-9
...
- Kanerva, P. (2009). Hyperdimensional computing: An introduction to computing in distributed representation with high-dimensional random vectors. _Cognitive Computation, 1_(2), 139â€“159. https://doi.org/10.1007/s12559-009-9009-8
- Kleyko, D., Rachkovskij, D. A., Osipov, E., & Rahimi, A. (2022). A survey on hyperdimensional computing aka vector symbolic architectures, Part I: Models and data transformations. _ACM Computing Surveys, 55_(6), 1â€“40. https://doi.org/10.1145/3538531
- Klyubin, A. S., Polani, D., & Nehaniv, C. L. (2005). All else being equal be empowered. In _Advances in Artificial Life (ECAL 2005)_, LNCS 3630, 744â€“753. https://doi.org/10.1007/11553090_75
...
- Lindblad, G. (1976). On the generators of quantum dynamical semigroups. _Communications in Mathematical Physics, 48_(2), 119â€“130. https://doi.org/10.1007/BF01608499
...
- Momennejad, I., Russek, E. M., Cheong, J. H., Botvinick, M. M., Daw, N. D., & Gershman, S. J. (2017). The successor representation in human reinforcement learning. _Nature Human Behaviour, 1_(9), 680â€“692. https://doi.org/10.1038/s41562-017-0180-8
...
- Plate, T. A. (1995). Holographic reduced representations. _IEEE Transactions on Neural Networks, 6_(3), 623â€“641. https://doi.org/10.1109/72.377968
- Salge, C., Glackin, C., & Polani, D. (2014). Empowerment â€” An introduction. In M. Prokopenko (Ed.), _Guided Self-Organization: Inception_ (pp. 67â€“114). Springer. https://doi.org/10.1007/978-3-642-53734-9_4
...
- Stachenfeld, K. L., Botvinick, M. M., & Gershman, S. J. (2017). The hippocampus as a predictive map. _Nature Neuroscience, 20_(11), 1643â€“1653. https://doi.org/10.1038/nn.4650
...
signatories, Sept 2023, PsyArXiv â€” a contested preprint); the Ferrante et al. (2023) Cogitate bioRxiv
preprint (10.1101/2023.06.23.546249); Cortical Labs CL1 (2025) â€” a product announcement, not a paper. The
