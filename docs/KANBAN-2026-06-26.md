...
**Status (v0.20.0):** FULL DEEP DIVE COMPLETE — Tsotchke local corpus (Eshkol COMPLETE spec + all mirrors at `Z:\[Vibe Coded (AI)]\(Tsotchke)`) + GH (22 repos) studied; registry maps all 20 projects (~16 wired, 3 fenced). Petri/soup + brutal god tier live. Gate **1,984 tests · 85.29% / 82.76%** (receipts enforced). Local==GH. "Grow What Thou Wilt."
...
- P1 `CHAOS-BIOME-50K` (V38 + V40 + V123) — **shipped:** the `mega` tier raises the ceiling to **50,000** (profiled `bench/scale.ts` + √N density scaling cut 50k 167→60 ms, golden byte-identical). **V123:** everyone **boots `phone`** (1k, fast first paint); `resolveTier` still maps ≥16-core/≥8 GB → `mega` for guidance; the perf HUD tier chip or `?tier=mega` selects 50k (verified **49,975 entities, glError 0** on capable hardware). **V42: the 70-param per-entity NN is wired** — `sim/entity-brain.ts` runs every organism's genome brain (6→6→4) on a round-robin cohort, perceiving its state + world and steering itself (bounded), determinism golden byte-identical, 7 headless tests. **Remaining:** neighbour-perception senses (currently self+global); Web Workers (ADR-0010 Stage 3) for 50k @ 60 fps; cognition-driven mutation on split.
