...
describe('A-Life code-grounded stats are computed-and-gated (no silent drift)', () => {
  ...
  test('every current narrative surface cites the current code-grounded figure (hand-edit → fail)', () => {
    ...
    const zPop = `+${cg.zPopulation.toFixed(2)}`; // e.g. "+2.88"
    ...
    // The NHSI dashboard cites the code-grounded z-population.
    expect(read('docs/NHSI-PROGRESS-DASHBOARD-2026-06-26.md')).toContain(`code-grounded z=${zPop}`);
  });
