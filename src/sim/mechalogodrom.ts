...
export class Mechalogodrom {
   ...
   */
  update(_t: number, dt: number, realDt = dt): void {
    // GOD-CLOCK CALIBRATION (owner 2026-07-14/15, FINAL: "I want the old values" — the global
    // TIME_SCALES ladder stays 0.1–5; the ×50 lives HERE, for the MECHALOGODROM + its rig only).
    // The body clock advances on the REAL frame delta with a slot-anchored rate: the 0.1× dial
    // reproduces the legacy 5× churn EXACTLY, higher dials push beyond it up to a strobe-safe ×4
    // cap (phase steps stay coherent — no aliasing flicker). Everything downstream of localT rides
    // along: shells, satellites, exterior colour panels, rings, spikes, dark-star uniforms.
    const LEGACY_5X_RATE = MECHA_TIME_SCALE * (0.3 + 0.7 * 5) * 5; // = 66.5 — the old top-slot rate
    // SUSPENDED ANIMATION: at timeScale 0 the god must NOT freeze — like every other creature it
    // stays alive in place, spinning/gimbaling/morphing on the pause visual clock. Only FUSION
    // (world progress) freezes below.
    const churn =
      this.worldTimeScale > 0
        ? LEGACY_5X_RATE * Math.min(this.worldTimeScale / 0.1, 4)
        : MECHA_TIME_SCALE * 0.3; // idle rate — continuous suspended-animation limit
    this.localT += realDt * churn;
