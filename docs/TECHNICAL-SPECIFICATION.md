...
| Test : source ratio                        | 0.40 → **93.69 % line / 90.34 % function** coverage (`bun test --coverage`) |
| Passing tests                              | **1,314** (0 failing)                                                       |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,314 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 93.69 / 90.34, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,314 tests, 93.69 % line / 90.34 % function coverage),
