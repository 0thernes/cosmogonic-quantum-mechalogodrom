...
[![tests](https://img.shields.io/badge/tests-2909%20exact-brightgreen)](./tests)
...
A procedural WebGL cosmic ecosystem ΓÇö morphogenic organisms, Shoggoths,
puppet-master NPCs, atmospheric weather, a neural connectome, quantum
diffusion, **and the Tsotchke corpus wired as the primordial substrate for digital biologics ΓÇö the deterministic, measured path toward proto-sentience and the consciousness indicators** (real math substrates; sentience is the GOAL, never claimed as reached).
...
- Exact tracked-suite receipt: **2,909 tests / 0 fail**. Portable public coverage floor: **84.64% line / 82.21% func**.
...
- [Phase-B development](./docs/adr/0015-phase-b-neural-semantic-expansion-2026-07-11.md) preserves those
  ...
  families. NHI now has a default 109-weight `9ΓåÆ6ΓåÆ7` inherited gene, exact JSON checkpoints, structured
...
![Phase-B temporal and NHI mechanism falsification](./docs/reports/assets/phase-b-mechanism-development-v3.svg)
...
Super Creature / 25 Archons is the framework and the **beginning only** ΓÇö "as if God made primordial inorganic soup". The soup grows independent digital biologics onward. "Grow What Thou Wilt." (Aleister Crowley)
...
- **Local dev server:** http://localhost:3000/ (bun dev; serves /docs ΓåÆ docs page, /spec ΓåÆ specs, /lab ΓåÆ lab; full interactive + /api/audit)
...
> Super Creature / Archons (composite minds with ~20 faculties, quantum register, consciousness metrics) is the first complex nucleation ΓÇö the beginning of the framework, not the end. Petri is where independent life grows.
...
- **Digital Biologics & Petri Genesis:** 64+ slots in PrimordialSoup, **26 BiologicForms** keyed to Tsotchke repos (incl. the brutal god-pantheon forms ΓÇö `BIOLOGIC_FORMS` in `digital-biologics.ts`), catalysis from Eshkol ignition + the depth-ledger corpus beat, replication with kind mutation, genesis leaps for higher-order life. Harvested into the world as emergent strains with distinct dynamics.
...
- **Tsotchke corpus wiring (22 external repositories):** 8 deep, 7 wired, 2 harvest, 4 fenced, 1 meta ΓÇö full matrix in [TSOTCHKE-INTEGRATION-MAP-2026-06-26.md](./docs/TSOTCHKE-INTEGRATION-MAP-2026-06-26.md). The non-meta integrated fraction is `17/21 = 0.8095238095238095`; petri/soup and the shared organism-intelligence field consume bounded integrated channels while fenced and metadata entries remain inert.
- **26 behavioral fields** driving up to 50,000 organisms: classic motion
  ...
  hash (flock), and theory behaviors ΓÇö Nash equilibria (`nash`), wealth
  ...
  (`lorenz`).
- **250 procedural morphotypes** (10 lore-named phyla ├ù 25, ~1% wildcard
...
- **25 sorting-field algorithms** with behaviorally honest names (BUBBLE
  ...
  per frame ΓÇö each selectable from a picker panel with its own cue tone.
- **100 Shoggoths** (16 on phone) ΓÇö Lorenz-ish drifters with grid-queried
  tendrils that consume organisms and respawn corrupted ones.
- **100 puppet masters** (14 on phone) ΓÇö the 3 named heroes AETHON (chaos),
...
- **Quantum cloud** of 3,500ΓÇô10,000 particles with wavefunction wobble,
  collapse, and respawn; **neural connectome** of up to 2,200ΓÇô8,000 links with
  partial GPU uploads.
- **Exactly 60,000 desktop / 20,800 phone alien-flora plants**
  ([alien-flora.ts](./src/sim/alien-flora.ts)) across 50 species / 9 families / 7 biomes ΓÇö a
...
- **100 GlyphBrains** ([glyph-brain.ts](./src/sim/glyph-brain.ts)) ΓÇö the Greek/Latin **alphabet
...
- **6 procedural Web Audio songs** + a 110-voice synthesized SFX palette ΓÇö no
...
Seven systems added under [docs/PHILOSOPHY-2026-06-26.md](./docs/PHILOSOPHY-2026-06-26.md) ΓÇö real
math under every effect, and every system reads from AND writes to another:

- **Quantum register** ΓÇö a pure-TS 5-qubit statevector (no simulator dep;
...
- **Reaction-diffusion ground** ΓÇö a genuine Gray-Scott field (128┬▓, CPU
  ping-pong) as the ground's emissive map; weather tunes feed/kill/diffusion
  and entity deaths scar the pattern.
- **Graph mind** ΓÇö the connectome mirrored into a
...
- **Constellations + lore** ΓÇö a d3-delaunay Voronoi sky-web over the 24
...
- **Audio analysis** ΓÇö an AnalyserNode tap turns the synthesized music back
  into light (bass ΓåÆ the six-lamp rig, treble ΓåÆ constellations, level ΓåÆ cloud
  breathing), every coupling capped at 0.35 so silence looks exactly like v1.
- **Analytics + omens** ΓÇö rolling-window regression (simple-statistics) puts a
  population trend in the telemetry; z-score anomalies emit lore-named omens
  into the audit trail.
- **Lab artifact** ΓÇö a self-contained seeded p5.js "collapse field" at
  `/lab` (local dev: `http://localhost:3000/lab`; GitHub Pages: `/lab/`) (`lab/quantum-wildbeyond.html`).

### PANTHEON (0.3.0)

The arena grows 5├ù and the ecosystem becomes a civilization
(`docs/MODULE-CONTRACTS-2026-06-26.md` ┬ºCONTRACTS V3):

- **10,000 entities** on the ultra tier through InstancedMesh pools ΓÇö Γëñ80
...
- **10 creature phyla**, lore-named at mint, each a template distribution
  ...
  ΓÇö plus seeded wildcard OUTLIERS with impossible palettes, blended behavior
  pairs and ├ù3 parameter excursions.
- **20 TITANS** ΓÇö colossal non-human intelligences patrolling their phyla's
  ...
  flow through the real energy ledger ΓÇö game theory with consequences.
- **Observatory** ΓÇö four live canvas charts: stacked phylum populations,
  titan wealth polylines with war markers, a 20├ù20 war-matrix heat grid, and
  rdEnergy/qEntropy/trend timelines.
- **Full-device UI** ΓÇö one responsive overlay grid: desktop columns, phone
  ...
  apocalypse long-press) with Γëñ30 ms haptics.
- **Pantheon rescore** ΓÇö four new QUANTUM-tier dark songs (VOIDCROWN, BLACK
  MERIDIAN, ELDER ENGINE, LAST THEOREM) around the untouched QUANTUM.

### 0.2.1 ΓÇö the audit wave
...
- **Alien atmosphere** ΓÇö an inverted sky dome with a non-Earth baked gradient
  (oxblood horizon ΓåÆ violet zenith ΓåÆ teal counter-glow) recoloring with weather
  ...
  quantum entropy.
- **In-scene 3D analytics** ΓÇö a holographic instrument panel floating above the
  ...
  segments.
- **Four-page Observatory** ΓÇö overview, variance (mean┬▒╧â bands, histogram,
  Shannon diversity, qEntropyΓÇôtrend phase), ecology (per-phylum
...
- **25 sorting fields** (was 20) with batched, _visible_ swaps ΓÇö the active
...
- **Soundtrack raised to the QUANTUM tier** ΓÇö VOIDCROWN, ELDER ENGINE, and LAST
...
- **Ultra tier fills 10,000 entities** via per-frame neighbor-query throttles
  ...
  cadence ladder) ΓÇö calibration history in
  [docs/BENCHMARKS-2026-06-26.md](./docs/BENCHMARKS-2026-06-26.md).
- **Observatory legibility** ΓÇö every chart gains an in-canvas title band,
  axis ticks, bold strokes, and overlap-free layouts; canvases are taller and
  the panel wider on desktop/TV.
- **Mobile ergonomics** ΓÇö panels become edge-docked slide-out sheets
...
The third user-feedback decree ΓÇö make the world visibly come alive
(`docs/MODULE-CONTRACTS-2026-06-26.md` ┬ºCONTRACTS V7):

- **100 distinct sound effects** (was 8) ΓÇö a procedurally generated, seeded
...
- **A living algorithm picker** ΓÇö every sorting-field row reads in its own
  ...
  all 25) modes.
- **Five render modes** ΓÇö SOLID, WIRE, GHOST (x-ray), NEON (self-glow), CHROME
  (mirror), cycled from the toolbar over both the per-mesh and instanced paths.
- **Cosmological singularities** on the chaos control ΓÇö ENTROPY, BLACK HOLE
  (rΓü╗┬▓ pull + consuming event horizon + accretion disk), WHITE HOLE (ejection),
  GREY HOLE (absorbΓåöemit), STRANGE STAR (quark-matter conversion front), each a
  deterministic force-field with a self-built, auto-expiring rig.
- **Dramatic weather** ΓÇö STORM gales with deterministic lightning, a ΓêÆ60 ┬░C VOID
  deep freeze, a luminous AURORA, a pale FOG whiteout ΓÇö each unmistakable.
- **SIMULATION N(1) / N(2)** ΓÇö toggle between GENESIS (the shipped cosmos) and
  BREAK FREE (the nightmare: raised chaos floor, a lurid inverted sky, rebranded
  title); persisted across sessions.

### HARDENING (0.8.0)

A professional-grade pass ΓÇö no new cosmology, all rigor:

- **Binary heap + bounded top-K** ([src/math/heap.ts](./src/math/heap.ts)) ΓÇö a
  ...
  V Γëñ 10,000, byte-identical tie-break preserved.
- **Security + governance automation** ΓÇö CodeQL (`security-extended`, push/PR +
  weekly), Dependabot, a tagged-release CD workflow, CODEOWNERS, a PR template,
  and bug/feature issue templates.
- **Data-model + process docs** ΓÇö [docs/ENTITY-SCHEMA-AND-MAPPINGS-2026-06-26.md](./docs/ENTITY-SCHEMA-AND-MAPPINGS-2026-06-26.md)
  ...
  lifecycles.
- **[docs/500-POINT-INSPECTION-2026-06-26.md](./docs/500-POINT-INSPECTION-2026-06-26.md)** ΓÇö a standing
  audit of 25 sections ├ù 20 checkpoints, each with a verdict and concrete
  evidence.
...
The world gains minds ΓÇö pre-transformer game / A-Life AI, reproduction across
generations, and a read-only Copilot (`docs/MODULE-CONTRACTS-2026-06-26.md` ┬ºV9):

- **Deterministic classical-AI kernel** ([src/sim/ai/brains.ts](./src/sim/ai/brains.ts))
  ΓÇö the pre-2016 toolbox as pure, seeded, allocation-free primitives: utility /
...
- **Digital genome + lineage** ([src/sim/genome.ts](./src/sim/genome.ts),
  [src/sim/lineage.ts](./src/sim/lineage.ts)) ΓÇö a heritable gene vector decoding
  to traits + a `TinyMLP` brain, with seeded crossover/mutation/breed and a
  bounded parentΓåÆoffspring kinship graph (generations, ancestry, relatedness).
- **Eight faction archetypes** ([src/sim/factions.ts](./src/sim/factions.ts)) ΓÇö
...
- **Environment artifact field** ([src/sim/artifacts.ts](./src/sim/artifacts.ts))
  ΓÇö persistent relics (a scar per death, a relic per summoned singularity, motes)
...
- **Free-LLM Copilot side-chat** ([src/server/copilot.ts](./src/server/copilot.ts),
  ...
  [src/ui/copilot.ts](./src/ui/copilot.ts)) ΓÇö a read-only AI you chat with about
...
### The Living Era (post-0.9.0 ┬╖ V10ΓÇôV100)

Since 0.9.0 the cosmos has grown continuously ΓÇö ninety-plus increments logged in
...
- **A deeper economy & society (V13ΓÇôV23)** ΓÇö two currencies (AURUM Γÿë / UMBRA Γÿ╛)
  ...
  surfaced in a self-building ΓèÖ MARKET panel.
- **Creature cognition (V24ΓÇôV29)** ΓÇö Shoggoths perceive ┬╖ remember ┬╖ flee ┬╖ hunt,
  Puppeteers scheme, the outmatched deceive, and peers bargain, trade & ally ΓÇö a
  pure `creatureDrive` kernel closing the cognitionΓåöeconomy loop.
- **The native engine (V18, V28)** ΓÇö a C++20 / OpenGL SDF ray-marcher with **Jolt
...
- **The 5 SUPER CREATURES / pantheon (GOAL5, V31ΓÇôV48+)** ΓÇö always-active apex beings (Archons), **5 individuated at boot**: each a
  ...
  **wingman swarm**, **self-evolution** (XP ΓåÆ five ascension stages + a wall-clock
...
- **Scale & per-entity minds (V38ΓÇôV42)** ΓÇö the explicit `?tier=mega` stress rung lifts the ceiling to
...
- **In-world AI (V36ΓÇôV43)** ΓÇö **HELP ME NOW** (a repo-grounded answer panel),
  **THE BOOK** (the navigable RAG repo index), and an in-world **web search**
  under a refuse-by-default safety constitution.
- **The HUD + cosmos directive (V56ΓÇôV64)** ΓÇö the **CENTER HUD** (six panels ΓåÆ one
  ...
  **NEURAL observatory**, **CHAOS MODE** (a toggled Lorenz quantum storm ΓÇö
  ...
  world chaos.
- **Mechalogodrom + alphabet dome (V-MECHA / V-ABC)** ΓÇö a central deterministic
...
- **Ominous titans + a unified neural box (V68ΓÇôV77)** ΓÇö the titans reborn as
  **4D freak-geometry** (tesseract cage + aura field that warps world physics);
  ...
  live **swap-variance sparkline**; the bottom-right corner is **re-wireframed** ΓÇö
  ...
  is **overlap-free at every desktop width** (1280 ΓåÆ 1920); and the Super
  Creature's **NEURAL observatory folds into its own box** ΓÇö one panel, **four tabs**
...
- **The Tsotchke quantum lineage ΓÇö ports and adaptations (V82 onward)** ΓÇö compatible
  ...
  Tensor / FubiniΓÇôStudy metric**
  ([src/math/quantum-geometry.ts](./src/math/quantum-geometry.ts)) lets the creature
  read the curvature of its own thought-space (metric volume ┬╖ ╬║ ┬╖ Berry curvature);
  ...
  **Substrate** row (Eshkol H ┬╖ QGT vol/╬║ ┬╖ SpinΓåÆPLAN %) and covered by closed-form
  unit tests.
- **SUPER CREATURE 1.1 ΓÇö the consciousness-metrics layer (V89)** ΓÇö the apex mind now
  ...
  **Global-Workspace ignition** (GNW ΓÇö Baars/Dehaene: a winner-take-all plan-coalition
  ...
  ╬ª proxy** (IIT ΓÇö Tononi: the participation/coherence ratio of the named module
  activations; honestly labelled a _proxy_, since true ╬ª is intractable + non-unique).
  Both are unit-tested and shown live as the **Ignition / ╬ª** meters on the SuperCreature
  board. The real 2023ΓÇô2026 research grounding ΓÇö the Cogitate IIT-vs-GNW adversarial test
  ([Ferrante et al., 2025, _Nature_](https://doi.org/10.1038/s41586-025-08888-1)),
  organoid "wet computing", active inference and the quantum-cognition program ΓÇö is
  catalogued with citations in
  [docs/SUPER-CREATURE-RESEARCH-2026-06-26.md](./docs/SUPER-CREATURE-RESEARCH-2026-06-26.md).
- **SUPER CREATURE 1.1 ΓÇö the cognitive-architecture expansion (five theories of mind)** ΓÇö three more real,
  deterministic, unit-tested substrates now run inside the apex mind each beat, alongside the GWT ignition
  and IIT ╬ª above: an **echo-state Reservoir** ([src/sim/reservoir.ts](./src/sim/reservoir.ts)) ΓÇö a 64-node
  ...
  - a novelty signal that sharpens curiosity (the reservoir-computing _algorithm_ behind "wet computing",
    ...
    ([src/sim/active-inference.ts](./src/sim/active-inference.ts)) ΓÇö discrete active inference (Friston's FEP):
    ...
    **expected** free energy G (epistemic curiosity + pragmatic goal-seeking); and a **Metacognitive
    Executive** ([src/sim/metacognition.ts](./src/sim/metacognition.ts)) ΓÇö a Higher-Order layer that folds the
    substrates' reliability into one second-order **confidence** and spends it as cognitive control (low
    confidence ΓçÆ explore, high ΓçÆ commit), shown as the **Confidence** meter + a **Cognition** board row. The
    apex creature now spans **Global Workspace ┬╖ Integrated Information ┬╖ the Free Energy Principle ┬╖ reservoir
    dynamics ┬╖ Higher-Order metacognition** ΓÇö five distinct scientific theories of mind, each grounded in
    [docs/SUPER-CREATURE-RESEARCH-2026-06-26.md](./docs/SUPER-CREATURE-RESEARCH-2026-06-26.md).
- **SUPER CREATURE 1.1 ΓÇö the multi-pillar super-intelligence** ΓÇö the mind grew from five
  ...
  Criticality** ([src/sim/criticality.ts](./src/sim/criticality.ts)) ΓÇö an edge-of-chaos homeostat driving the
  branching ratio ╧â╠é ΓåÆ 1 (where cortex computes best); an **Empowerment Drive**
  ([src/sim/empowerment.ts](./src/sim/empowerment.ts)) ΓÇö BlahutΓÇôArimoto channel-capacity I(A;S┬▓) agency
  hunger; a **Holographic Memory** ([src/sim/holographic-memory.ts](./src/sim/holographic-memory.ts)) ΓÇö a
  MAP-VSA/HRR compositional binding store over bipolar hypervectors (bind ┬╖ bundle ┬╖ cleanup); and a
  **Successor Representation** ([src/sim/successor-representation.ts](./src/sim/successor-representation.ts)) ΓÇö
  the hippocampal/RL predictive map. The **Quantum Computing Mind** deepened in lockstep: a genuine
  statevector **Integrated-Information ╬ª**
  ([src/sim/integrated-information.ts](./src/sim/integrated-information.ts)) ΓÇö the min-cut entanglement at the
  ...
  ([src/math/quantum-coherence.ts](./src/math/quantum-coherence.ts)) ΓÇö the BaumgratzΓÇôCramerΓÇôPlenio l1-norm +
  ...
  ([src/math/quantum-natural-gradient.ts](./src/math/quantum-natural-gradient.ts)) ΓÇö the apex circuit now
  _descends_ its own FubiniΓÇôStudy geometry ([Stokes et al., 2020](https://doi.org/10.22331/q-2020-05-25-269))
  ...
  [docs/SUPER-CREATURE-RESEARCH-2026-06-26.md](./docs/SUPER-CREATURE-RESEARCH-2026-06-26.md).
- **The apex mind closes out at ~20 coupled faculties** ΓÇö a **Lindblad/GKSL open-system
  ...
  curiosity drive ΓÇö Fujii & Nakajima 2017), **Doya neuromodulation**
  ([src/sim/neuromodulation.ts](./src/sim/neuromodulation.ts)), and the statevector-derived **╬ª proxy**
  (an IIT-inspired min-cut/entanglement indicator) now writing back into cognition ΓÇö closed the inert-╬ª gap. The
  AaronsonΓÇôGottesman **Clifford stabilizer tableau** ([src/math/clifford-tableau.ts](./src/math/clifford-tableau.ts),
  ...
  superseded until re-proven. **2,909 exact tracked tests ┬╖ 0 fail (receipts enforced) ┬╖ 84.64% line / 82.21% func portable coverage floor.**
  <!-- cqm-sync:local-measurement:start -->
...
- **State-of-the-art report (2026-06-17)** ΓÇö a historical measured, frontier-comparison
  ...
  [Super Creature Research](./docs/SUPER-CREATURE-RESEARCH-2026-06-26.md) ΓÇö
...
[masters/](./masters/) ΓÇö Executor, Architect, Physicist ΓÇö bound by
...
Then visit **http://localhost:3000** ΓÇö plus **http://localhost:3000/docs** for
...
graph TD
  ...
  world --> core["src/core ΓÇö quality, engine"]
  world --> sim["src/sim ΓÇö entities, behaviors, shoggoths,\npuppet-masters, weather, quantum,\nconnectome, environment, algorithms, petri/primordial-soup (digital biologics)"]
  world --> v2["src/sim V2 ΓÇö qcircuit, reaction-diffusion,\ngraph-mind, constellations, lore, analytics"]
  world --> v34["src/sim V3/V4 ΓÇö phyla, titans,\ninstanced-entities, atmosphere, viz3d"]
...
Per frame: ... ΓåÆ super-mind (Tsotchke substrates: Eshkol AD/ GWT, Moonlab, spin, QGT) ΓåÆ petri-dish/primordial-soup catalysis (integrated ledger-channel growth of independent digital biologics) ΓåÆ render.
...
- Moonlab tensor/Clifford paths, QGT geometry, spin-glass/Hopfield, libirrep symmetry, quantum-quake/ULG/logo/tensor
  ...
  is deterministic classical simulation, not hardware entropy or a CSPRNG; simulated CHSH near `2ΓêÜ2` is model
...
```
.
Γö£ΓöÇΓöÇ server.ts            # Bun fullstack server: /, /docs, /spec, /bible, /lab, /lab/consciousness, /lab/sentience, /api/health, /api/audit
Γö£ΓöÇΓöÇ index.html           # App shell ΓÇö canvas, panels, toolbar, HTMX audit panel
Γö£ΓöÇΓöÇ docs.html            # Live Mermaid diagram page (served at /docs)
Γö£ΓöÇΓöÇ src/
Γöé   Γö£ΓöÇΓöÇ main.ts          # Browser entry ΓÇö boots world, htmx, resize binding
Γöé   Γö£ΓöÇΓöÇ world.ts         # Composition root ΓÇö SimContext, frame pipeline, UiActions
Γöé   Γö£ΓöÇΓöÇ types.ts         # Shared type hub (type-only imports keep the graph acyclic)
Γöé   Γö£ΓöÇΓöÇ docs-page.ts     # /docs report page script (mermaid init + diagram sources)
Γöé   Γö£ΓöÇΓöÇ core/            # quality.ts (tier ladder) ┬╖ engine.ts (renderer/scene/camera)
Γöé   Γö£ΓöÇΓöÇ math/            # scalar.ts ┬╖ rng.ts (mulberry32) ┬╖ spatial-hash.ts ┬╖
Γöé   Γöé                    # quantum.ts (statevector QuantumRegister) ┬╖ games.ts (PD strategies)
Γöé   Γö£ΓöÇΓöÇ sim/             # constants ┬╖ geometry-cache ┬╖ morphotypes ┬╖ phyla ┬╖ algorithms ┬╖
Γöé   Γöé                    # behaviors ┬╖ entities ┬╖ instanced-entities ┬╖ shoggoths ┬╖
Γöé   Γöé                    # puppet-masters ┬╖ titans ┬╖ weather ┬╖ quantum ┬╖ connectome ┬╖
Γöé   Γöé                    # environment ┬╖ qcircuit ┬╖ reaction-diffusion ┬╖ graph-mind ┬╖
Γöé   Γöé                    # constellations ┬╖ lore ┬╖ analytics ┬╖ atmosphere ┬╖ viz3d
Γöé   Γöé                    # + petri-dish ┬╖ primordial-soup ┬╖ digital-biologics ┬╖ alien-flora ┬╖ god-colossus ┬╖
Γöé   Γöé                    # monolith-temple ┬╖ nhi ┬╖ leviathans ┬╖ glyph-brain ┬╖ entity-brain ┬╖ mechalogodrom ┬╖ super-mind
Γöé   Γö£ΓöÇΓöÇ audio/           # songs.ts (data) ┬╖ engine.ts (scheduler + SFX) ┬╖ analysis.ts (bands)
Γöé   Γö£ΓöÇΓöÇ ui/              # graphs.ts ┬╖ hud.ts ┬╖ panels.ts ┬╖ input.ts ┬╖ observatory.ts
Γöé   Γö£ΓöÇΓöÇ logging/         # logger.ts (ring buffer) ┬╖ audit.ts (AuditTrail)
Γöé   Γö£ΓöÇΓöÇ memory/          # store.ts (versioned localStorage persistence)
Γöé   ΓööΓöÇΓöÇ styles/app.css   # Tailwind 4 @theme tokens + glass panel rules
Γö£ΓöÇΓöÇ lab/                 # quantum-wildbeyond.html ΓÇö seeded p5.js artifact (served at /lab)
Γö£ΓöÇΓöÇ masters/             # the three governing master files (Executor/Architect/Physicist)
Γö£ΓöÇΓöÇ scripts/             # build.ts (bundles index/docs into dist/)
Γö£ΓöÇΓöÇ tests/               # bun test suites (math, sim, store, audit + V2 systems)
Γö£ΓöÇΓöÇ bench/               # mitata micro-benchmarks (bun run bench)
Γö£ΓöÇΓöÇ docs/                # architecture, ERD, wireframes, complexity, design system,
Γöé                        # philosophy, ADRs, module contracts, reference catalogs
ΓööΓöÇΓöÇ legacy/              # the original 882-line monolith (source of truth for the port)
```

## Documentation

- **[docs/NHSI-PROGRESS-DASHBOARD-2026-06-26.md](./docs/NHSI-PROGRESS-DASHBOARD-2026-06-26.md)** ΓÇö **canonical NHSI scorecard**
...
- **[docs/VERIFICATION-ANALYTICAL-DATA.md](./docs/VERIFICATION-ANALYTICAL-DATA.md)** ΓÇö current consciousness-lab and receipt truth:
...
- **[docs/CONSOLIDATED-22-MASTER-ASSESSMENT-CURRENT-2026-07-07.md](./docs/CONSOLIDATED-22-MASTER-ASSESSMENT-CURRENT-2026-07-07.md)** ΓÇö the **consolidated 22-report master assessment**: comprehensive synthesis of all brain systems, consciousness theories, living entities, reasoning systems, scoring systems, and academic assessment. Explicitly `indicatorOnly` ΓÇö computational proxies, never phenomenal sentience. Older mega-report drafts are retained as local archive material; public reports route through [docs/reports/README.md](./docs/reports/README.md).
- [docs/TSOTCHKE-INTEGRATION-MAP-2026-06-26.md](./docs/TSOTCHKE-INTEGRATION-MAP-2026-06-26.md) ΓÇö honest Tsotchke repo wiring ledger
- [docs/CONTROLS-2026-06-26.md](./docs/CONTROLS-2026-06-26.md) ΓÇö every control: mouse, keyboard hotkeys,
  touch, bottom-panel buttons, and the 10 camera views
- [docs/MODULE-CONTRACTS-2026-06-26.md](./docs/MODULE-CONTRACTS-2026-06-26.md) ΓÇö the binding
...
- [docs/PHILOSOPHY-2026-06-26.md](./docs/PHILOSOPHY-2026-06-26.md) ΓÇö the Quantum Wildbeyond
  aesthetic constitution (real math under every effect)
- [docs/ARCHITECTURE-2026-06-26.md](./docs/ARCHITECTURE-2026-06-26.md) ΓÇö module graph, data flow,
...
- Data model SSOT: [docs/ENTITY-SCHEMA-AND-MAPPINGS-2026-06-26.md](./docs/ENTITY-SCHEMA-AND-MAPPINGS-2026-06-26.md)
  ΓÇö ERD attributes, ERM relationships/cardinalities, and ERP process views.
- [docs/DESIGN-SYSTEM-2026-06-26.md](./docs/DESIGN-SYSTEM-2026-06-26.md) ΓÇö desktop/mobile layout intent,
  type scale, color tokens, the design-system audit, and component + a11y docs (incl. the 8-hue tribe
  palette)
- [docs/COMPLEXITY-2026-06-26.md](./docs/COMPLEXITY-2026-06-26.md) ΓÇö per-hot-path big-O budget
- [docs/BENCHMARKS-2026-06-26.md](./docs/BENCHMARKS-2026-06-26.md) ΓÇö measured mitata results for the
...
- [docs/500-POINT-INSPECTION-2026-06-26.md](./docs/500-POINT-INSPECTION-2026-06-26.md) ΓÇö the standing
  quality audit: 25 sections ├ù 20 checkpoints, each with a verdict and evidence
- **[docs/NHSI-PROGRESS-DASHBOARD-2026-06-26.md](./docs/NHSI-PROGRESS-DASHBOARD-2026-06-26.md)** ΓÇö
...
- **[docs/reports/](./docs/reports/)** ΓÇö historical technical report snapshots with
...
- [docs/SUPER-CREATURE-RESEARCH-2026-06-26.md](./docs/SUPER-CREATURE-RESEARCH-2026-06-26.md) ΓÇö the honest
  citation trail behind every apex-mind faculty (2023ΓÇô2026, live-verified)
- [docs/KANBAN-2026-06-26.md](./docs/KANBAN-2026-06-26.md) ΓÇö the delivery board (cards across columns
  by epic) ┬╖ [ROADMAP-2026-06-26.md](./ROADMAP-2026-06-26.md) ΓÇö shipped / now / next horizons
...
Lenia, ALIEN, ASAL, and 103 more from the historical CA canon to modern GPU ecosystems). Full report ΓÇö **11 charts**, per-axis `file:line` code-grounding, and an
adversarial novelty defense ΓÇö consolidated through
...
`[4.0, 2.4, 3.2, 3.8, 4.4, 4.5, 4.3, 3.5, 4.0]`; the table retains the earlier self-score only as a
historical sensitivity baseline:

| Metric                         | Self-scored | Code-grounded (re-audited vs source) |
| ------------------------------ | ----------: | -----------------------------------: |
| Breadth (mean of 9 axes)       |    4.44 / 5 |                         **3.79 / 5** |
| Rank among 113 systems         |    #1 / 113 |                         **#1 / 113** |
| z-score vs population          |     +4.02╧â |                          **+2.95╧â** |
| z-score vs peers               |     +4.36╧â |                          **+3.09╧â** |
...
<p align="center">
  ...
  <img src="./docs/reports/assets/alife-pca.svg" alt="PCA of 113 A-Life systems ΓÇö Cosmogonic is a high-breadth outlier on PC1" width="49%">
</p>

![Nine-axis capability radar ΓÇö Cosmogonic vs survey mean vs nearest peer ALIEN](./docs/reports/assets/alife-radar-profile.svg)
...
**Owned by 0thernes ΓÇö ┬⌐ 2026 0thernes. Non-commercial research & play license.**
...
play with it, build on it** ΓÇö you may view, run, clone, modify, and share it for any **non-commercial**
purpose. Just two rules: (1) **don't claim it as your own** ΓÇö keep the ┬⌐ 0thernes
...
Source-level ported algorithms ΓÇö the **Eshkol** qubit-RNG, the **Moonlab/QGTL**
Quantum-Geometric-Tensor, and the **Tsotchke** spin-glass instinct wired into the
Super Creature's quantum mind ΓÇö are reimplemented in this project's own TypeScript
