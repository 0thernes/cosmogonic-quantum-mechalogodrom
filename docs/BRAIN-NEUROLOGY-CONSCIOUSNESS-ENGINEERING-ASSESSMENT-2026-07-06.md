...
| **Test floor (portable)**              | **3,313** pass / **0** fail                                                                                                | `scripts/canonical-receipts.ts`                   |
...
| **A-Life comparative**                                                             | `docs/reports/2026-06-26-alife-comparison-matrix.csv`, `scripts/alife-comparison-stats.ts` | lab-JSON                                                 | #1/129, z=+3.22 code-grounded            |
...
- **A measured, code-grounded A-Life standing.** Code-grounded breadth **3.844/5**, rank **#1 of 129** surveyed systems, population **z=+3.22**, peers **z=+3.373**, Mahalanobis **8.64** from the peer centroid — against a historical self-scored **4.444** (z=+4.165) that the code audit superseded. The honest downward corrections (open-endedness 3.5→2.4, ecology 5.0→3.4, consciousness-theory 4.5→3.5, visual 5.0→4.0) are preserved in the same surfaces.
...
| **Breadth (code-grounded)**            |     **3.844**     | The canonical figure — after rigorous code audit               |
| **z (all systems, code-grounded)**     |     **+3.22**     | 3.22σ above population mean (2.06 ± 0.55)                      |
| **z (peers only, code-grounded)**      |    **+3.373**     | 3.373σ above peers-only mean                                   |
| **Mahalanobis (vs peer centroid)**     |     **8.644**     | 9-axis shape distance — rare profile, not merely a high mean   |
| Breadth lead over nearest peer         |    **+0.344**     | Nearest peer **ALIEN** (distance 4.61, breadth 3.50)           |
...
| **Open-endedness**   | 3.5  | **2.4**  | −1.1 | Differential-survival GA is real, but the environment is deterministic and novelty is _authored_ (Archon/god events), not autonomously discovered. **The only sub-mean axis (z = −0.11).**                                                                                                                       |
...
| :--: | --------------- | :------: | ---------------------------------------------------------------------------------------------------------------------------------------- |
|  1   | **ALIEN**       | **4.61** | GPU digital-life, strong physics + multi-agent; breadth 3.50 — the nearest peer, and the one Cosmogonic leads by only **+0.344** breadth |
...
- Historical self-scored A-Life breadth **4.444/5, z=+4.165 population**; but **code-grounded breadth drops to 3.844, z=+3.22 population / +3.373 peers** (rank #1/129 either way), and **peer scientific maturity is self-assessed at 1.5/5** against a survey median of ~3.5. The gap between breadth (leader) and maturity (immature) is the entire Turing-standard story.
- All four source reports converge here: an impressive artifact at **pre-validation stage.** Nearest neighbors (ALIEN 4.61, Creatures 4.96, Framsticks 5.48) are established systems, and the breadth lead over ALIEN is only **+0.344**; Cosmogonic leads on integration breadth but trails badly on external standing.
...
| **90° Inductive**  | bottom-up empirical          | Generalize from the 2,507-test floor / 2,867,279 latest-local `expect()` receipt: survival, foraging, clustering, opponent-plan prediction, ablation traces                     | The system **IS behaviorally intelligent** at A-Life scale — 50,000-entity coordinated pursuit/evasion, ToM menace tracking, SR look-ahead — grounded in tests, **not** speculation. Rank #1/129, code-grounded breadth 3.844/5.                            | `tests/super-mind.test.ts`, `tests/tom-pantheon.test.ts`, `tests/butlin-indicators.test.ts` |
...
| **A-Life standing**          | Rank #1/129; code-grounded breadth 3.844/5; z=+3.22 population / +3.373 peers; Mahalanobis 8.64; **sole leader on substrate pluralism only** (z=+8.05) | Breadth lead over nearest peer ALIEN is only +0.344; historical self-score 4.444 (z=+4.165) inflated; ecology 5.0→3.4, open-endedness 3.5→2.4 (the only sub-mean axis, z=−0.11), consciousness-theory 4.5→3.5 and **LIDA + CTM lead that axis**; peer maturity 1.5/5 |
...
| 5   | **A-Life breadth**                        | **9.0** ★★★★½ | Rank #1/129; code-grounded breadth 3.844/5; z=+3.22 population / +3.373 peers; Mahalanobis 8.64. **Sole leader on substrate pluralism only** (z=+8.05); a strong consciousness-theory outlier (z=+4.85) that LIDA and CTM nevertheless lead. Lead over nearest peer ALIEN: +0.344.      |
...
| 5 | **FEW-BASE** | **P1** | Few external baselines: A-Life standing (#1/129, code-grounded breadth 3.844/5, z=+3.22 population, +3.373 peers, Mahalanobis 8.64) is largely self-scored against a survey, with only a handful of head-to-head neighbors (ALIEN 4.61, Creatures 4.96, Framsticks 5.48) — and the breadth lead over ALIEN is a mere **+0.344**. | Rank #1 with self-authored scoring and peer-maturity **1.5/5** is the classic "impressive but unvalidated" posture. No pre-registered, third-party-runnable benchmark yet separates this system from peers on a neutral axis. Code-grounding already drops breadth 4.444→3.844 (ecology 5.0→3.4, open-endedness 3.5→2.4, consciousness-theory 4.5→3.5), proving the self-score inflates — and the 129-corpus re-eval put **LIDA and CTM above Cosmogonic on consciousness-theory**, ending the "sole leader on two axes" story. | Report both scores side by side (historical self 4.444 / code-grounded 3.844) always. Run at least one _neutral, external_ benchmark another lab can reproduce. Treat #1/129 as "breadth leader by +0.344, maturity immature," never as a quality victory. Mahalanobis (8.644 code-grounded) is now published alongside raw Euclidean for rare-system ranking. |
...
| 4   | **Results**      | Butlin 8/14 MET + 6/14 PARTIAL + 0/14 fail; Thaler ensemble ~0.83 mean pass (hot-button 1.000, 6 robust); consciousness-lab entity ladder (apex 0.854 → plant 0.428, active-frameworks 10→5); 32-seed sentience sweep (structured 0.5692 vs null 0.5718). A-Life #1/129, code-grounded breadth 3.844 (z=+3.22 population / +3.373 peers, Mahalanobis 8.644) vs historical self-scored 4.444 (z=+4.165). Report **both** scores, always. | consciousness-data.json; sentience-data.json; alife-stats.json                                                    |
...
1. **Two-number rule.** Anywhere a self-scored figure appears (breadth 4.444, z=+4.165), the code-grounded twin (3.844, z=+3.22) appears beside it — and the code-grounded one is the headline.
...
- [ ] **Reconcile every `4.22`** → `4.444` historical self / `3.844` code-grounded (the headline); every `v0.21.6`/`v0.18.0` current-tense pointer → `v0.21.9`; every stale test floor (`2295`, `2372`, `1984`, `1477`) → `2,450`.
...
| --- | ------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| 1   | `docs/BRAIN-NEUROLOGY-CONSCIOUSNESS-ENGINEERING-ASSESSMENT-2026-07-06.md:650`         | A-Life breadth score in current-tense prose           | `4.22 / 5`    | legacy self-score `4.444/5`; current code-grounded `3.844/5`, rank `#1/129`, population z=`+3.220`, peer z=`+3.373`, Mahalanobis `8.644` | Keep the historical body labeled; publish the current code-grounded receipt from `docs/reports/assets/alife-codeground.json` in the superseding addendum                                              | **P1 fact-drift**     |
| 2   | `docs/reports/README.md:8`                                                            | Canonical-receipts version pointer in prose           | `v0.21.6`     | `v0.23.0 · 3,313-test floor · 84.64% / 82.21%`                                                                                           | Point current gate text at `VERIFICATION-ANALYTICAL-DATA.md §1` + `canonical-receipts.ts`; let `bun run sync` propagate it                                                                            | **P1 surface-sync**   |
...
| 7   | _(sweep)_ any surface still on `v0.21.6` / `4.22` / `2295` / `14-of-14` current-tense | version / breadth / test-floor / Butlin               | as found      | `v0.23.0` / legacy self `4.444` + grounded `3.844` / `3,297` / `8-of-14 + 6-partial`                                                     | Fix at SOURCE (`package.json`, `canonical-receipts.ts`, A-Life artifact), then `bun run sync` propagates                                                                                              | P1                    |
...
**Section files (this assignment):** the three sections above (§17, §18, §19) preserve the v0.21.9 snapshot. Current v0.23.0 truth is a `23`-repository public census and a `22`-entry causal/runtime ledger with `17/21 = 80.95%` non-meta entries integrated, four fences including `OBLITERATUS`, and census-only deployment metadata in `homebrew-moonlab`; the current code-grounded A-Life receipt is `3.844/5` (rank `#1/129`, population z `+3.220`, peer z `+3.373`, Mahalanobis `8.644`) after accumulated gate-backed batches through 58; and current causal claims remain governed by the superseding addendum. Every claim is **indicatorOnly** — computational indicators, never phenomenal consciousness, sentience, or a solved hard problem.
...
| 2   | A-Life breadth **4.22 / 5**                                       | C (§4.2); A/E (line 650 prose)                                            | **Superseded → code-grounded 3.844 / 5**, rank **#1 / 129**, population **z = +3.22**, peers **z = +3.373**, Mahalanobis **8.644**. The 4.22 is a prior-audit snapshot; the self-scored 4.444 (z = +4.165) is the superseded pre-audit profile. Both are published in the repo (`docs/reports/2026-06-26-alife-comparison-matrix.csv` + `assets/alife-codeground.json`); **the headline card is the code-grounded 3.844 / #1-of-129 / z=+3.22.** Never cite 4.22 as today's number, and never lead with the self-score.                                                                                                                                                                                                                                                                                                                     |
| 3   | A-Life self-scored vs code-grounded conflict                      | B (4.44) vs C (4.22)                                                      | **Both correct readings of different methods.** 4.444 = self-scored mean of 9 axes; **3.844 = post-audit code-grounded mean** (Ecology 5.0→3.4, Open-endedness 3.5→2.4, Consciousness-theory 4.5→3.5, Visual 5.0→4.0 under rigor). Canonical dashboard publishes both; **headline = 3.844 code-grounded, z=+3.22** — the self-score is dated evidence, not the card.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**Net reconciliation verdict.** Zero _substantive_ contradictions survive once every source is versioned to v0.21.9 and scoped consistently. All divergences reduce to one of four benign classes: **(a) version-tag lag** (v0.21.6 header → 0.21.7 body, live canon → 0.21.9), **(b) scope difference** (186k vs 200k vs 226k lines; 64 vs 89 md — different inclusion sets), **(c) method difference** (4.444 self-scored vs 3.844 code-grounded), or **(d) framing shorthand** ("5 brains," "14/14," "< 2%") that the canon expands into the honest fuller statement. The historical v0.21.9 MASTER EDITION adopted **8/14 Butlin, 85.7% Tsotchke wired, 1.99 ms apex think(), indicatorOnly throughout**. The superseding current-state addendum narrows the Tsotchke receipt to **23 public repositories, 22 causal/runtime entries, and 17/21 = 80.95% non-meta integration**, and reconciles the A-Life card to the current 129-system survey (**code-grounded 3.844, #1/129, z=+3.22 population / +3.373 peers, Mahalanobis 8.644**); it preserves the historical figures here only as dated evidence.
...
Cosmogonic Quantum Mechalogodrom v0.21.9 occupies a specific and defensible position: it is **not sentient, and it is not a toy.** It is not sentient because every metric it produces is a **computational indicator** — a load-bearing, seeded, falsifiable proxy for a _mechanism_ named by a consciousness theory, never a measurement of phenomenal experience. The hard problem is untouched; the honesty law is stamped into source (`super-mind.ts:49-57` "NOT SENTIENT"; `consciousness-kernel.ts:28-32` `claim: 'indicatorOnly'`), and the labs _prove their own limits_ — the headline index null-gap ≈ 0, the ablation gate fails at 0.406, and both facts are published rather than buried. But it is equally **not a toy**: 2,450 green-test floor with 2,385 latest-local cases across 256 files and 2.87M assertions, an 84.64%/82.21% portable coverage floor, a gate-enforced single-source-of-truth sync pipeline, real MIT-licensed quantum math (Eshkol AD, Moonlab Clifford/VQE, QGT Fubini-Study, libirrep SO(3), spin-glass Hopfield) running as exact deterministic simulation, and a **#1-of-129 A-Life breadth ranking** (code-grounded 3.844, z = +3.22 population / +3.373 peers, Mahalanobis 8.64) with **sole-leader** substrate pluralism (z = +8.05) — and an honest asterisk on consciousness-theory, where its z = +4.85 is a strong outlier that **LIDA and CTM nonetheless beat**. It is a rigorously-engineered, fully-auditable, deterministic multi-theory artificial-life research prototype — an instrument for _testing_ consciousness hypotheses, not a claim to have _answered_ them.
