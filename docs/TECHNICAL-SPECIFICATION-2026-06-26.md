...
**Gate:** 1,477 tests · 90.48% line / 87.82% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).  
...
| Test : source ratio                     | 0.34 → **90.48% line / 87.82% func** coverage (`bun test --coverage`) |
...
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 90.48% line / 87.82% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,477 tests, 90.48% line / 87.82% func coverage),
