...
**Gate:** 1648 tests · 95.77% line / 92.87% func (canonical synced, receipts enforced).
...
| Test : source ratio                        | 0.40 → **95.77% line / 92.87% func** coverage (`bun test --coverage`) |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,1648 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 95.77% line / 92.87% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1648 tests, 95.77% line / 92.87% func coverage),
