...
**Gate:** 2,162 tests · 95.71% line / 92.69% func (canonical synced via verify-receipts 2026-06-21, receipts law enforced).  
...
| Test : source ratio                        | 0.40 → **95.71% line / 92.69% func** coverage (`bun test --coverage`) |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**2,162 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 95.71% line / 92.69% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (2,162 tests, 95.71% line / 92.69% func coverage),
