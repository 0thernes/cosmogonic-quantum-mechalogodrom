...
| 3   | Test rigor & coverage     | 9.0   | A     | `2,295`-test canonical floor; latest local full receipt measured `2,347` pass / `0` fail and `2,866,344` `expect()` calls. Published coverage remains the canonical `84.41%` line / `82.11%` func floor. Receipts law (`verify-receipts.ts`) makes an unmeasured number unshippable. |
