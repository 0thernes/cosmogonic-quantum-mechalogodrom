...
**Epistemic contract (from roadmap XML):** *If it is not measured, it is not real. UNKNOWN remains UNKNOWN.*
...
```mermaid
flowchart TB
  subgraph population["Population scale — LIVE"]
    EB["EntityBrain<br/>70 params × 50k max"]
    C["Connectome<br/>2.2k–8k links"]
    GM["GraphMind<br/>Louvain + PageRank"]
  end

  subgraph apex["Apex tier — LIVE"]
    SM["SuperMind × 5<br/>~10,081 weights"]
    SB["SuperBody + SuperCreature"]
    PD["Petri / Primordial Soup"]
  end

  subgraph society["Pantheon society — LIVE"]
    PS["20 light Archons<br/>Eshkol VM echoes"]
    MF["MindField 25×8 stigmergy"]
    FP["FacultiesPantheon<br/>100 design / ~30 deep"]
    TP["TomPantheon<br/>25 ToM organs"]
  end

  subgraph visual["Visual / scaffold tier"]
    GB["GlyphBrain × 100<br/>~25k each — visual only"]
    MB["MechalogodromBrain<br/>5M designed / ~120k live"]
    AB["ApexBrain 101st<br/>11 weird organs"]
    CK["ConsciousnessKernel<br/>10 theories — lab only"]
  end

  subgraph thaler["Thaler substrate"]
    TM["70-param Creativity Machines<br/>9 constitutive markers"]
  end
...
```mermaid
flowchart LR
  Claim --> Source["Code / canonical constant / lab JSON"]
  Source --> Test["Focused test or receipt"]
  Test --> Surface["Docs / README / dashboard"]
  Surface --> Gate["sync:check / verify:facts / verify:receipts"]
```
...
*These are scrutiny **levels**, not literal award readiness.*
...
- No public reproducibility bundle (pinned data, seeds, plots, hardware notes) — *partially addressed by `scripts/reproduce.ts`*
...
```mermaid
flowchart TD
  Dataset["Real connectome dataset"] --> Provenance["License, checksum, citation"]
  Provenance --> Graph["Typed neural graph"]
  Graph --> Dynamics["Neural dynamics"]
  Sensors["Physics body sensors"] --> Dynamics
  Dynamics --> Motors["Motor neurons / actuators"]
  Motors --> Body["Physics body"]
  Body --> Sensors
  Dynamics --> Metrics["Behavior, lesions, controls, ablations"]
```
...
1. **Unknown unknowns in mechanism space:** The repo implements more distinct consciousness *indicator families* than almost any A-Life system surveyed — but **indicator convergence ≠ experience**. The sentience lab's null-gap ≈ 0 on the headline index is a **feature**: it proves the headline scalar is not a magic sentience detector.

2. **Phenomenon of the phenomenon:** The coupled consciousness kernel measures *emergence as excess over independent mean* — a second-order quantity about *relations among indicators*, not about qualia. That is the correct epistemic level for software.
...
*Pass 1 compiled 2026-07-06 · Cosmogonic Quantum Mechalogodrom v0.21.7 · 0thernes Corp*  
*License: Non-commercial research & play (© 0thernes, commercial rights reserved)*
