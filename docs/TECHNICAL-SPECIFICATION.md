...
**Gate:** 1399 tests Â· 94.38% line / 91.26% func (canonical synced).
...
| Test : source ratio                        | 0.40 â†’ **94.38 % line / 91.26 % function** coverage (`bun test --coverage`) |
| Passing tests                              | **1,400** (0 failing)                                                         |
...
`prettier --check` â†’ `tsc --noEmit` (strict) â†’ `oxlint` â†’ `bun test` (**1,400 tests, 0 fail**) â†’
`bun scripts/build.ts`. Coverage gate: line â‰¥ 0.90, function â‰¥ 0.85 (measured 94.38 / 91.26, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,400 tests, 94.38 % line / 91.26 % function coverage),
