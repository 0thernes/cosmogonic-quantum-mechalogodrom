...
| Test : source ratio                        | 0.40 â†’ **94.59 % line / 91.47 % function** coverage (`bun test --coverage`) |
...
`bun scripts/build.ts`. Coverage gate: line â‰¥ 0.90, function â‰¥ 0.85 (measured 94.59 / 91.47, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1442 tests, 94.59 % line / 91.47 % function coverage),
