...
| Test : source ratio                        | 0.40 → **95.58% line / 92.69% func** coverage (`bun test --coverage`) |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,1617 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 95.58% line / 92.69% func, `bun test --coverage`).
