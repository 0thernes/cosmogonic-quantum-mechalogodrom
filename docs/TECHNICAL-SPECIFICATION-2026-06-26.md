...
**Gate:** 1,984 tests · 85.29% line / 82.76% func (canonical synced via verify-receipts 2026-06-26, receipts law enforced).  
...
| Test : source ratio                     | 0.34 → **85.29% line / 82.76% func** coverage (`bun test --coverage`) |
...
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 85.29% line / 82.76% func, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,984 tests, 85.29% line / 82.76% func coverage),
