...
> assessment and its historical sensitivity numbers. Current evidence is narrower: the corrected
...
> vector is `[4.0, 2.4, 3.3, 3.8, 4.5, 4.6, 4.4, 3.5, 4.0]` (breadth `3.833`, rank `#1/113`, population
> z `+3.080`, peer z `+3.234`). Accumulated gate-backed batches through 56 establish this receipt:
...
| **Test floor (portable)**              | **3,297** pass / **0** fail                                                                                                | `scripts/canonical-receipts.ts`                   |
...
| **90° Inductive**  | bottom-up empirical          | Generalize from the 2,507-test floor / 2,867,279 latest-local `expect()` receipt: survival, foraging, clustering, opponent-plan prediction, ablation traces                     | The system **IS behaviorally intelligent** at A-Life scale — 50,000-entity coordinated pursuit/evasion, ToM menace tracking, SR look-ahead — grounded in tests, **not** speculation. Rank #1/113, breadth 4.44/5.                                           | `tests/super-mind.test.ts`, `tests/tom-pantheon.test.ts`, `tests/butlin-indicators.test.ts` |
...
- [ ] **Reconcile every `4.22`** → `4.44` self / `3.68` code-grounded; every `v0.21.6`/`v0.18.0` current-tense pointer → `v0.21.9`; every stale test floor (`2295`, `2372`, `1984`, `1477`) → `2,450`.
...
| --- | ------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| 1   | `docs/BRAIN-NEUROLOGY-CONSCIOUSNESS-ENGINEERING-ASSESSMENT-2026-07-06.md:650`         | A-Life breadth score in current-tense prose           | `4.22 / 5`    | legacy self-score `4.44/5`; current code-grounded `3.833/5`, rank `#1/113`, population z=`+3.080`, peer z=`+3.234`                      | Keep the historical body labeled; publish the current code-grounded receipt from `docs/reports/assets/alife-codeground.json` in the superseding addendum                                              | **P1 fact-drift**     |
| 2   | `docs/reports/README.md:8`                                                            | Canonical-receipts version pointer in prose           | `v0.21.6`     | `v0.23.0 · 3,297-test floor · 84.64% / 82.21%`                                                                                          | Point current gate text at `VERIFICATION-ANALYTICAL-DATA.md §1` + `canonical-receipts.ts`; let `bun run sync` propagate it                                                                            | **P1 surface-sync**   |
...
| 7   | _(sweep)_ any surface still on `v0.21.6` / `4.22` / `2295` / `14-of-14` current-tense | version / breadth / test-floor / Butlin               | as found      | `v0.23.0` / legacy self `4.44` + grounded `3.833` / `3,297` / `8-of-14 + 6-partial`                                                     | Fix at SOURCE (`package.json`, `canonical-receipts.ts`, A-Life artifact), then `bun run sync` propagates                                                                                              | P1                    |
...
**Section files (this assignment):** the three sections above (§17, §18, §19) preserve the v0.21.9 snapshot. Current v0.23.0 truth is a `23`-repository public census and a `22`-entry causal/runtime ledger with `17/21 = 80.95%` non-meta entries integrated, four fences including `OBLITERATUS`, and census-only deployment metadata in `homebrew-moonlab`; the current code-grounded A-Life receipt is `3.833/5` after accumulated gate-backed batches through 56; and current causal claims remain governed by the superseding addendum. Every claim is **indicatorOnly** — computational indicators, never phenomenal consciousness, sentience, or a solved hard problem.
...
**Net reconciliation verdict.** Zero _substantive_ contradictions survive once every source is versioned to v0.21.9 and scoped consistently. All divergences reduce to one of four benign classes: **(a) version-tag lag** (v0.21.6 header → 0.21.7 body, live canon → 0.21.9), **(b) scope difference** (186k vs 200k vs 226k lines; 64 vs 89 md — different inclusion sets), **(c) method difference** (4.44 self-scored vs 3.68 code-grounded), or **(d) framing shorthand** ("5 brains," "14/14," "< 2%") that the canon expands into the honest fuller statement. The historical v0.21.9 MASTER EDITION adopted **8/14 Butlin, 4.44 self / z=+2.83 code-grounded, 85.7% Tsotchke wired, 1.99 ms apex think(), indicatorOnly throughout**. The superseding current-state addendum narrows the Tsotchke receipt to **23 public repositories, 22 causal/runtime entries, and 17/21 = 80.95% non-meta integration**; it preserves the historical number here only as dated evidence.
...
Cosmogonic Quantum Mechalogodrom v0.21.9 occupies a specific and defensible position: it is **not sentient, and it is not a toy.** It is not sentient because every metric it produces is a **computational indicator** — a load-bearing, seeded, falsifiable proxy for a _mechanism_ named by a consciousness theory, never a measurement of phenomenal experience. The hard problem is untouched; the honesty law is stamped into source (`super-mind.ts:49-57` "NOT SENTIENT"; `consciousness-kernel.ts:28-32` `claim: 'indicatorOnly'`), and the labs _prove their own limits_ — the headline index null-gap ≈ 0, the ablation gate fails at 0.406, and both facts are published rather than buried. But it is equally **not a toy**: 2,450 green-test floor with 2,385 latest-local cases across 256 files and 2.87M assertions, an 84.64%/82.21% portable coverage floor, a gate-enforced single-source-of-truth sync pipeline, real MIT-licensed quantum math (Eshkol AD, Moonlab Clifford/VQE, QGT Fubini-Study, libirrep SO(3), spin-glass Hopfield) running as exact deterministic simulation, and a **#1-of-113 A-Life breadth ranking** (z = +4.02 population, +2.83 code-grounded) with **solo-leader** substrate pluralism (z = +8.05) and consciousness-theory wiring (z = +10.04). It is a rigorously-engineered, fully-auditable, deterministic multi-theory artificial-life research prototype — an instrument for _testing_ consciousness hypotheses, not a claim to have _answered_ them.
...
6. **Claim discipline.** Hold the line already held: `indicatorOnly` everywhere, 8/14 never 14/14, 4.44 self / 2.83 code-grounded never 4.22, **23 public repositories / 22 causal-runtime entries / 17-of-21 non-meta integrated** never "all deeply wired," simulated quantum never quantum-speedup, scrutiny-levels never award-readiness. Every promotion earns its number at the source (`canonical-receipts.ts`), synced to every surface, gate-enforced.
