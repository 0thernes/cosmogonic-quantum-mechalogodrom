...
| Test : source ratio                        | 0.40 → **94.45 % line / 91.47 % function** coverage (`bun test --coverage`) |
| Passing tests                              | **1,259** (0 failing)                                                       |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,259 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 94.45 / 91.47, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,259 tests, 94.45 % line / 91.47 % function coverage),
