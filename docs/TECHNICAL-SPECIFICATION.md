...
| Test : source ratio                        | 0.40 → **94.61 % line / 91.19 % function** coverage (`bun test --coverage`) |
| Passing tests                              | **1,050** (0 failing)                                                       |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,050 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 94.61 % / 91.19 %, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,050 tests, 94.61 % line coverage),
