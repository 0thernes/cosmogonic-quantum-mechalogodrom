...
**Updated:** 2026-07-11 · Tsotchke: **22 external repositories** (`8 deep`, `7 wired`, `2 harvest`, `4 fenced`, `1 meta`; non-meta integrated fraction `17/21 = 0.8095238095238095`). `classical-contrast` is a live internal control and is not counted as an external repository. After batches 21–25, the code-grounded A-Life vector is `[4.0, 2.4, 3.2, 3.8, 4.3, 4.5, 4.3, 3.5, 4.0]`: breadth `3.778`, rank `#1/113`, population z `+2.990`, peer z `+3.131`, and peer maturity `1.5/5`. The gate now drift-locks that receipt; the `2.4` Open-endedness score is backed by differential-survival GA, while the `4.3` Cognition/Learning score is backed by exact Eshkol-AD policy learning, live flora-gradient chemotaxis, and a genuine multi-layer perceptron (backprop through a tanh hidden layer — solves XOR, which a linear unit provably cannot) that every digital biologic trains ONLINE into a forward self-model whose prediction error collapses in the live petri (GATE-MLP + GATE-SELFMODEL, ablation-verified). The audit sync allowlist gap is closed. The [organism-intelligence causal audit](./reports/2026-07-10-OPERATIONAL-ORGANISM-INTELLIGENCE-CAUSAL-AUDIT.md) passed synthetic goal-only and corpus-conditioned effects, `6.1213%` fixed-family adaptation, row causality, exclusion, numerical safety, three-process performance stability, and matched coverage for every named consumer class. It failed uniform random-action baseline separation and aggregate-mapping specificity. V3 itself authorizes no additional organism-intelligence, consciousness, or sentience uplift; the A-Life axis changes come from the separate gate-backed mechanisms named above.
...
**Dated 22-report audit/master pair:** [`CONSOLIDATED-22-MASTER-ASSESSMENT-CURRENT-2026-07-07.md`](./CONSOLIDATED-22-MASTER-ASSESSMENT-CURRENT-2026-07-07.md) and [`CONSOLIDATED-22-FILE-AUDIT-CURRENT-2026-07-07.md`](./CONSOLIDATED-22-FILE-AUDIT-CURRENT-2026-07-07.md) preserve the 2026-07-07 publication synthesis and file-trust ledger. Their receipt numbers are historical. <!-- cqm-sync:local-measurement:start -->Current Windows evidence is `2,613 tests / 0 fail / 3,022,764 expect()` with `92.97%` line / `90.86%` function coverage.<!-- cqm-sync:local-measurement:end --> The separate `84.64% / 82.21%` portable coverage floor lives in [verification data](./VERIFICATION-ANALYTICAL-DATA.md). The pair preserves exact lab/Archon/event handles and does not claim sentience.
