...
Wave 1 (Foundation): Audit + plan + contract amendments + provenance everywhere + enhanced godform/world with full corpus archetypes. (This doc + reports)
Wave 2 (Math Layer): Extend/ new leaves for Eshkol AD (nested gradients for topdown/empowerment), full Moonlab Clifford + QGT, better QRNG, libirrep reps for 5 forms + quantum states.
Wave 3 (Sim Layer): Wire into SuperMind (new Eshkol organ or AD subnet), SuperBody (more morph from quake/tensor), memory/narrative using arena ideas.
Wave 4 (World/Physics): Integrate quantum-quake elements or QGE primitives into native or world sim for creature interactions; local percepts richer.
Wave 5 (UI/Bridge + Web): ULG ideas into super-panel/observatory; optional WASM bridge notes.
Wave 6 (Native + Perf): Selective C from mirrors into native/ (via facade).
Wave 7 (Tests/Bench/Receipts): Goldens, new benches (tensor contraction, AD tape, irrep), update receipts law.
Wave 8 (Docs/Audit/Close): Full ERM update, CORPUS_INTEGRATION_REPORT, final gate, clean tree.
