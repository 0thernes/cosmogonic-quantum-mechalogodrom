...
```mermaid
flowchart TD
  World["World / Petri / Entities"] --> Sensors["Percepts and telemetry"]
  Sensors --> SuperMind["SuperMind: 30 organs, 25 variants, 5 stages"]
  Sensors --> EntityBrains["Entity TinyMLP / MemoryRing / adapters"]
  Sensors --> ApexBrain["ApexBrain weird-organ engine"]
  Sensors --> MechBrain["Mechalogodrom fusion brain"]

  Tsotchke["Tsotchke corpus: Eshkol, Moonlab, QGT, spin, libirrep, RNGs, PINN/PIMC/quake"] --> SuperMind
  Tsotchke --> ApexBrain
  Tsotchke --> Petri["Digital biologics / primordial soup"]

  SuperMind --> Plans["Plan/drives: hunt, flee, dominate, deceive, spawn, explore, rest"]
  ApexBrain --> Plans
  MechBrain --> Plans
  EntityBrains --> World
  Plans --> Bodies["SuperBody / Archons / world action"]
  Bodies --> World

  SuperMind --> Labs["Consciousness and Sentience Labs"]
  ApexBrain --> Labs
  MechBrain --> Labs
  EntityBrains --> Labs
  Labs --> Docs["Truth surfaces and JSON receipts"]
```
...
```mermaid
flowchart LR
  Claim["Claim"] --> Source["Code / canonical constant / lab JSON"]
...
flowchart TD
  ...
  Body --> Sensors
  Dynamics --> Metrics["Behavior, stability, lesions, controls"]
```
