...
> vector is `[4.0, 2.4, 3.2, 3.8, 4.1, 4.5, 4.3, 3.5, 4.0]` (breadth `3.756`, rank `#1/113`, population
> z `+2.954`, peer z `+3.090`). Batches 21–26 gate-lock and correct this receipt: differential-survival
...
| **Test floor (portable)**              | **2,521** pass / **0** fail                                                                                          | `scripts/canonical-receipts.ts`                         |
...
| **A-Life breadth**                     | code-grounded **3.756/5** · rank **#1 / 113** · z = **+2.954** population / **+3.090** peers; legacy self-score 4.44 | `docs/reports/assets/alife-codeground.json`             |
...
| --- | ------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| 1   | `docs/BRAIN-NEUROLOGY-CONSCIOUSNESS-ENGINEERING-ASSESSMENT-2026-07-06.md:650`         | A-Life breadth score in current-tense prose           | `4.22 / 5`    | legacy self-score `4.44/5`; current code-grounded `3.756/5`, rank `#1/113`, population z=`+2.954`, peer z=`+3.090` | Keep the historical body labeled; publish the current code-grounded receipt from `docs/reports/assets/alife-codeground.json` in the superseding addendum | **P1 fact-drift**     |
| 2   | `docs/reports/README.md:8`                                                            | Canonical-receipts version pointer in prose           | `v0.21.6`     | `v0.21.13 · 2,521-test floor · 84.64% / 82.21%`                                                                    | Point current gate text at `VERIFICATION-ANALYTICAL-DATA.md §1` + `canonical-receipts.ts`; let `bun run sync` propagate it                               | **P1 surface-sync**   |
...
| 7   | _(sweep)_ any surface still on `v0.21.6` / `4.22` / `2295` / `14-of-14` current-tense | version / breadth / test-floor / Butlin               | as found      | `v0.21.13` / legacy self `4.44` + grounded `3.756` / `2,507` / `8-of-14 + 6-partial`                               | Fix at SOURCE (`package.json`, `canonical-receipts.ts`, A-Life artifact), then `bun run sync` propagates                                                 | P1                    |
...
**Section files (this assignment):** the three sections above (§17, §18, §19) preserve the v0.21.9 snapshot. Current v0.21.13 truth is `17/21 = 80.95%` non-meta external repositories integrated, with four fences including `OBLITERATUS`; the current code-grounded A-Life receipt is `3.756/5` after the batch-21–25 GA, Eshkol-AD learning, and flora-chemotaxis updates; and current causal claims remain governed by the superseding addendum. Every claim is **indicatorOnly** — computational indicators, never phenomenal consciousness, sentience, or a solved hard problem.
