<!-- reviewed: 2026-07-07 | v0.21.7 MASTER brain/neuro/consciousness assessment | merges 4 AI reports | canonical: docs/VERIFICATION-ANALYTICAL-DATA.md -->
...
`bun run metrics` snapshot (2026-07-07), excluding `node_modules`, `dist`, `coverage`, native build output, and `.git`. **Total tracked authored: 730 files · 193,174 lines.**

| Extension               |   Count |                Lines | Role                                                    |
| ----------------------- | ------: | -------------------: | ------------------------------------------------------- |
| `.ts` (TypeScript)      | **584** |              135,813 | app source + tests + scripts + bench (no `.tsx`/`.jsx`) |
| `.md` (Markdown)        |  **64** |               17,287 | living docs (non-legacy tracked)                        |
| `.html`                 |   **8** |               12,982 | diagrams / labs / spec surfaces                         |
...
| ------------------ | ----: | ------------: | ------------------------------------------------------------------------------------------ |
| `src/`             |   288 |        97,135 | app source (`.ts` only): `main.ts`, `server.ts`, sim, math, core, workers                  |
| `tests/`           |   255 |        33,887 | the 2,360-test floor / 2,866,612 latest-local `expect()` receipt                           |
| `docs/`            |    65 |        15,680 | living narrative + ADRs + dated reports (64 non-legacy MDs synced)                         |
| `scripts/`         |    27 |         5,721 | build / verify / sync / bench (gate-critical: `canonical-receipts.ts`, `sync-surfaces.ts`) |
...
| `lab/`             |     5 |        13,206 | consciousness + sentience falsifier labs (HTML + JSON)                                     |
| `masters/`         |     3 |           478 | XML steering personas (EXECUTOR / ARCHITECT / PHYSICIST)                                   |
...
| Root files         |    27 |        11,288 | `README`, `CHANGELOG`, `CLAUDE.md`, `THIRD-PARTY-NOTICES`, config and root metadata        |
...
- **A gate-enforced, deterministic artifact — with the full gate currently RED.** The 8-stage `bun run check` (prettier → tsc strict → oxlint → verify:receipts → sync:check → verify:facts → build) is designed to hold every surface honest, and the four measurement sub-gates pass right now: **typecheck, lint, sync:check, and verify:receipts are green** — canonical receipts floor **2,360 tests / 0 fail** across **255 test files**; the latest Windows-local receipt is **2,378 completed cases / 0 fail / 2,866,612 `expect()` calls**. The portable coverage floor remains **84.64 % line / 82.21 % func** (Windows checkout **92.01 % / 89.65 %**), version pinned at **0.21.7** in `package.json` and propagated to every surface by `scripts/sync-surfaces.ts`. **But the full `check` is red**: `format:check` fails on the unformatted report forks and — the P0 blocker — `bun run build` fails at `src/styles/app.css:0` (a verified toolchain regression, not content; see §II.9.1). Any claim that `bun run check` is fully green today is stale. Codebase scale (git-tracked, 2026-07-07): **730 tracked files · 584 `.ts` (583 excl. 1 `.d.ts`) · 135,813 TS lines · 8 C/C++ · 255 `*.test.ts`**.
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------- |
| Test suite                 | **2,360-test floor / 0 fail** across **255 test files**                                                                                                                                            | `scripts/canonical-receipts.ts` |
| Assertion density          | **2,866,612** `expect()` calls                                                                                                                                                                     | measured 2026-07-07             |
| Coverage (portable floor)  | **84.64 % line / 82.21 % func**                                                                                                                                                                    | canonical ledger (Ubuntu)       |
| Coverage (Windows receipt) | **92.01 % line / 89.65 % func**                                                                                                                                                                    | this checkout                   |
...
| Codebase scale             | **730 tracked files** (584 `.ts`, 135,813 TS LOC; 193,174 tracked authored LOC)                                                                                                                    | `bun run metrics`               |
...
| **90° Inductive**  | bottom-up empirical          | Generalize from the 2,360-test floor / 2,866,612 latest-local `expect()` receipt: survival, foraging, clustering, opponent-plan prediction, ablation traces                     | The system **IS behaviorally intelligent** at A-Life scale — 50,000-entity coordinated pursuit/evasion, ToM menace tracking, SR look-ahead — grounded in tests, **not** speculation. Rank #1/113, breadth 4.44/5.                                           | `tests/super-mind.test.ts`, `tests/tom-pantheon.test.ts`, `tests/butlin-indicators.test.ts` |
...
| 3   | **Test / receipt integrity**              | **8.7** ★★★★½ | 2,360-test floor / 0 fail · 2,866,612 latest-local `expect()` · 255 files; 84.64% line / 82.21% func portable floor gate-enforced. Deep and honest (floor is a floor, not a headline). |
...
The canonical test corpus (**2,360-test floor / 0 fail · 255 files** at v0.21.7, with a latest-local **2,866,612 expect()** receipt) is the _floor_, not the ceiling: it proves the mechanisms run deterministically. This benchmark suite proves whether they are _load-bearing indicators_ — a strictly stronger, and currently only partially satisfied, bar.
...
| 8   | **Appendix**     | Canonical receipts (v0.21.7 · 2,360-test floor / 0 fail · 255 files · latest-local 2,866,612 expect() · 84.64%/82.21% portable, 92.01%/89.65% Windows); file inventory (730 tracked); full coupling matrix W; complete lexicon / honesty attestation; reproduction commands (`bun run check`, `bun run bench`).                                                                                                                         | canonical-receipts.ts; VERIFICATION-ANALYTICAL-DATA.md                                                            |
...
**Section files (this assignment):** the three sections above (§17, §18, §19) are the deliverable text, returned inline as required — no separate report file was written. All numbers reconcile to canonical **v0.21.7** (2,360-test floor / 0 fail · 84.64%/82.21% portable · 92.01%/89.65% Windows · 730 tracked files · Butlin 8/14 MET + 6/14 PARTIAL · A-Life 4.44 self / 3.68 code-grounded · Tsotchke 18/21 wired = 85.7%). Every claim is **indicatorOnly** — computational indicators, never phenomenal consciousness, sentience, or a solved hard problem.
...
| 18  | Test suite size                                                   | A/E, B, C, D                                                              | **Confirmed current split → 2,360-test floor / 0 fail · 255 files · latest-local 2,866,612 expect().** All four reports agree once versioned to 0.21.7 (B explicitly confirms). Coverage: **84.64% line / 82.21% func** portable floor; **92.01% / 89.65%** Windows receipt.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| SUPER-REPORT ULTIMATE-MEGA (manifesto)              |    232 | **C+ / 6.0**                 | **"~400 TS files" (→584), "~50,000 lines" (→135,813 .ts / 193,174 tracked authored), ApexBrain conflation, "7 C/C++" (→8)** | the manifesto-of-the-gods voice + the brain×scrutiny checkmark matrix concept                                                                                                              | supersede (keep voice only) |
...
| **Total LOC / files**                  | "~50,000 lines / ~400 TS files" (ULTIMATE)    | **135,813 .ts LOC · 584 .ts files · 730 tracked authored files · 193,174 tracked authored LOC**                                                                                                                                                              | `bun run metrics`                                                                 |
