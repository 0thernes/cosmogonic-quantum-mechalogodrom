...
import { nhiAscension } from '../src/sim/nhi-body';

describe('nhiAscension (pure)', () => {
  test('ground → 0, top → 1, mid → 0.5; clamps beyond the column', () => {
    expect(nhiAscension(0)).toBe(0);
    expect(nhiAscension(240)).toBeCloseTo(1, 6);
    expect(nhiAscension(120)).toBeCloseTo(0.5, 6);
    expect(nhiAscension(1000)).toBe(1); // clamp above
    expect(nhiAscension(-50)).toBe(0); // below ground → clamp
  });

  test('monotonic non-decreasing; always within [0,1]', () => {
    let prev = -1;
    for (let y = -20; y <= 300; y += 10) {
