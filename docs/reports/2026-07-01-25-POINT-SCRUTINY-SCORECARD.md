...
> Current receipts: **v0.22.0 · canonical floor 3,271 tests · 84.64 % line / 82.21 % function**;
> latest Windows-local gate on 2026-07-14 measured **3,271 completed cases · 0 failures ·
> 3,551,782 `expect()` calls**. The portable coverage floor remains the synced canonical
...
| 3   | Test rigor & coverage     | 9.0   | A     | `3,271`-test canonical floor; latest Windows-local full receipt measured `3,271` completed cases / `0` fail and `3,551,782` `expect()` calls. Windows coverage measured `93.57%` line / `91.56%` func; published portable coverage remains the canonical `84.64%` line / `82.21%` func floor. Receipts law (`verify-receipts.ts`) makes an unmeasured number unshippable. |
