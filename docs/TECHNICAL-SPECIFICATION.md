...
| Test : source ratio                        | 0.40 → **97.38 % line / 93.35 % function** coverage (`bun test --coverage`) |
| Passing tests                              | **1,165** (0 failing)                                                       |
...
`prettier --check` → `tsc --noEmit` (strict) → `oxlint` → `bun test` (**1,165 tests, 0 fail**) →
`bun scripts/build.ts`. Coverage gate: line ≥ 0.90, function ≥ 0.85 (measured 97.38 % / 93.35 %, `bun test --coverage`).
...
**bit-reproducible from one seed**, ships through a **full CI/CD gate** (1,165 tests, 97.38 % line coverage),
...
**Frontier assessments (2026-06-17):** [Report I — Whole Repository](./reports/2026-06-17-STATE-OF-THE-ART-WHOLE-REPO.md) · [Report II — Super Creature](./reports/2026-06-17-STATE-OF-THE-ART-SUPER-CREATURE.md) · [Super Creature Research](./SUPER-CREATURE-RESEARCH.md).
